...
The temporary \`cosmetic-convergence-3-5.css\` file was an incubator, not a presentation authority. Its surviving rules are absorbed into the existing owners: semantic workspace closure (Processi/Evidenze and shared procedure-frame geometry), A6-UX3 (operational RN/EC/GRC flow and actions), A6-UX4 (context/reference, scope decision and Standard Browser), enterprise workspace (Home/EP/Admin/dialogs), and workspace chrome (AI indicator). The temporary stylesheet is no longer mounted.
