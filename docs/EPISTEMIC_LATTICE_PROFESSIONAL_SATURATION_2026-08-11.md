# EP-01 Professional-use saturation — 2026-08-11

## Baseline

This work starts from `main@6aeff3a5fc4fe286619d41c787162e5a18975a8c`, the merge commit of PR #70.

EP-01 already has the right authority boundary: it is cross-cutting, does not become an eighth business procedure, reads the canonical append-only semantic history, presents Explore/Flat/Graph from one projection, keeps AI derivations proposed, and restricts access to admin/auditor.

The remaining problem is not the existence of the lattice. It is whether different professionals can use the same lattice for different legitimate jobs without silently changing truth, losing claim boundaries, or needing prior knowledge of the internal data model.
...
## Canonical professional-use taxonomy

The bounded screenshot/simulation space contains exactly twelve professional lenses. They are interaction presets, not authorities:

1. `compliance-lead` — global concentration, open review signals and cross-procedure trace.
2. `internal-auditor` — predecessor/basis/producer/revision lineage and raw verification.
3. `dpo-privacy` — privacy incidents, declared GDPR references, evidence and unresolved questions without applicability inference.
4. `security-manager` — incidents, technical objects, controls, actions and risks.
5. `risk-manager` — risk scenarios, human ratings, treatment/action links and review lineage.
6. `control-owner` — control/object changes, mapping evidence and corrective work.
7. `assurance-reviewer` — evidence used in assurance responses and the boundary between record, answer and approval.
8. `legal-231-reviewer` — explicit registered D.Lgs. 231/2001 art. 24-bis traces only; never an inferred offence/legal classification.
9. `it-operations` — operational changes, incidents, technical assets and action lineage.
10. `supplier-procurement` — supplier/contract/DPA monitoring, coverage and evidence requests.
11. `quality-manager` — procedures, corrective actions, evidence consistency and repeatability.
12. `executive-sme` — bounded overview of where evidence/work concentrates, never a compliance score.

Actual RBAC remains only `admin`, `auditor`, `user`. Professional lenses never grant permissions. `user` remains denied EP-01.
...
- exactly twelve professional lenses, each with purpose, working question, preferred mode, procedure scope and claim guardrail;
- lenses are returned inside the canonical EP-01 projection and never change `projectionSha256`;
- lens filtering can only remove/orient atoms from the current projection, never synthesize atoms;
- business-thread context is searchable and visible in atom detail when recorded;
- proposed/reviewed/rejected states stay distinct under every lens;
- 231/privacy lenses contain explicit non-inference boundaries;
- executive lens exposes no compliance/maturity percentage or legal red/green verdict;
- user role still receives no EP-01 data or navigation.
