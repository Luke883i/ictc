...
assert.equal(MEANINGFUL_HANDOFFS.length,24);assert.ok(MEANINGFUL_HANDOFFS.every(edge=>edge.predicate&&edge.predicate!=='cross-procedure-draft-from'),'handoffs must carry intent-specific predicates');assert.equal(new Set(MEANINGFUL_HANDOFFS.map(edge=>edge.predicate)).size>=12,true);assert.equal(MAX_CROSS_PROCEDURE_LINEAGE_DEPTH,8);
