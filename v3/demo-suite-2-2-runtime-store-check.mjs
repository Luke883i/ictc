...
let store=null,restarted=null;
try{
  store=new RuntimeStore(root);await store.init();
  ...
  const first=store.snapshot(),firstRevision=first.revision;
  ...
  restarted=new RuntimeStore(root);await restarted.init();
  const beforeIdempotent=restarted.snapshot().revision;
  ...
  const posture=demoSuite22Projection(restarted.snapshot());
  assert.equal(posture.enabled,true);assert.equal(posture.positiveRecords,188);assert.equal(posture.stressFixtures,512);
  console.log(JSON.stringify({ok:true,control:'DEMO-SUITE-2.2-RUNTIME-STORE',revision:firstRevision,stateDigest:DEMO_SUITE_22_EXPECTED_DIGEST,positiveRecords:posture.positiveRecords,stressFixturesTestOnly:posture.stressFixtures,idempotentRestart:true},null,2));
}finally{
