...
function sourceLaws(s){
  ...
  return Object.freeze({
    ...
    queueInsideRegistry:s.operational.includes("if(queue&&queue.parentElement!==details)details.insertBefore(queue,host)"),
    ...
    decisionFrameOutsideOwnedBlock:s.harmonizationBase.includes("const primary=root.querySelector(':scope > [data-editorial-slot=\"primary\"]')")&&s.harmonizationBase.includes("if(primary&&frame.previousElementSibling!==primary)primary.after(frame)")&&!s.harmonizationBase.includes("rail.after(frame)"),
...
const SOURCE_MUTANTS=Object.freeze([
  ...
  ['queue-outside-registry',s=>({...s,operational:s.operational.replace("if(queue&&queue.parentElement!==details)details.insertBefore(queue,host)","if(queue&&queue.parentElement!==details)details.after(queue)")})],
  ...
  ['decision-frame-breaks-owned-block',s=>({...s,harmonizationBase:s.harmonizationBase.replace("if(primary&&frame.previousElementSibling!==primary)primary.after(frame)","const rail=root.querySelector(':scope > .procedure-support-rail');if(rail)rail.after(frame)")})],
