export const A6_UX1_CONTRACT=Object.freeze({
  ...
  evidenceBoundary:'Automated source/model plus exact-head Chromium flow geometry; human assistive-technology/usability remains external evidence.'
});
...
export function validateA6Ux1({css,styles,registry,workflow,browser}){
 ...
 for(const [token,code] of [['--a6-ux1-footer-min:44px','DESKTOP_MIN'],['--a6-ux1-footer-min:52px','MOBILE_MIN'],['env(safe-area-inset-bottom,0px)','SAFE_AREA'],['--a6-ux1-footer-reserve:0px','ZERO_RESERVE'],['scroll-padding-bottom:0px','ROOT_SCROLL_PADDING'],['padding-bottom:0!important','BODY_RESERVE'],['position:static!important','FLOW_POSITION'],['inset:auto!important','FLOW_INSET'],['height:auto!important','AUTO_HEIGHT'],['min-height:calc(var(--a6-ux1-footer-min) + var(--a6-ux1-footer-safe-bottom))!important','MIN_HEIGHT']])check(has(css,token),code,token);
 check(!has(css,'position:fixed!important'),'NO_FIXED_OVERLAY','fixed footer overlay resurrected');
 ...
 for(const token of ['getBoundingClientRect','bodyPaddingBottom','scrollPaddingBottom','normal-flow-non-overlap','reflow-1440-390-320','root-trap-negative'])check(has(browser,token),'BROWSER_ORACLE',token);
