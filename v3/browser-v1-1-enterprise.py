...
try:
 with sync_playwright() as pw:
  ...
  PHASE='feature-flag-enforcement'; page.locator('#stableProfileMenu summary').click(); page.locator('#openAdminCenter').click(); expect(page.locator('#procedurePolicyList input[name="coverage"]')).to_be_checked(); page.locator('#procedurePolicyList input[name="coverage"]').uncheck(); page.locator('#procedurePolicyForm button[type="submit"]').click(); expect(page.locator('#procedurePolicyCount')).to_contain_text('6/7 attive'); page.locator('#adminCenter [data-admin-close]').click(); page.locator('.service-nav [data-service="processes"]').click(); expect(page.locator('#procedureHub .stable-process-card')).to_have_count(6); assert page.locator('#procedureHub [data-process-code="MC-01"]').count()==0
  ...
  restored=api_eval(page,"const r=await fetch('/api/admin/procedures',{method:'PUT',headers,body:JSON.stringify({features:{coverage:true}})}); return {status:r.status,body:await r.json()};"); assert restored['status']==200,restored; page.reload(wait_until='networkidle'); page.locator('.service-nav [data-service="processes"]').click(); expect(page.locator('#procedureHub .stable-process-card')).to_have_count(7)

  PHASE='monitoring-action-cleanup'; page.locator('#procedureHub [data-process-code="RN-01"] .primary').click(); assert page.locator('button:visible').evaluate_all("els=>els.filter(x=>!(x.innerText||x.getAttribute('aria-label')||'').trim()).length")==0; if page.locator('.contribute-card').count():
