...
assert.match(lifecycle,/if\(flushing\)\{replay=true;return cycle;\}/,'reentrant requests must replay in the current flush instead of queueing an echo microtask');
...
assert.match(lifecycle,/function publishConvergence\(\)/,'lifecycle must expose one convergence publisher');
assert.match(lifecycle,/dataset\.experienceCycle=String\(cycle\)/,'final convergence must expose a monotonic cycle on the document root');
assert.match(lifecycle,/ictc:experience-converged/,'final convergence must emit one explicit semantic boundary event');
assert.match(lifecycle,/publishConvergence\(\);\s*return cycle;/,'convergence may be published only after replay exhaustion');
...
assert.match(ci,/FAILED_BROWSER_SCRIPT=.*browser-failure-provenance\.mjs/,'canonical browser rail must retain structured per-script provenance in logs/artifacts');
assert.doesNotMatch(ci,/ictc\/browser-failure\//,'canonical browser rail must not multiply root failures into per-script commit statuses');
...
console.log(JSON.stringify({ok:true,root:EXPERIENCE_ROOT.id,participants:EXPECTED_EXPERIENCE_PARTICIPANTS.length,phaseOrder:EXPECTED_EXPERIENCE_PARTICIPANTS.map(item=>item.phase),timingAuthority:'forbidden-in-final-participants',coalescingAuthority:'experience-lifecycle-only',convergenceSignal:'monotonic-experience-cycle-after-replay-exhaustion',browserGateAuthority:'ci.yml/browser-journeys',browserFailureTopology:'single-authoritative-rail-status+structured-log-provenance',manualDiagnostics:['uiux-onto-epistemic','ui-standard'],replayGuard:MAX_EXPERIENCE_REPLAY_CYCLES,packagePrivate:true}));
