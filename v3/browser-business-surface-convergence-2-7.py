...
try:
 with sync_playwright() as pw:
  ...
  PHASE='home';metrics=p.locator('#homePulse .home-business-metric');expect(metrics).to_have_count(6);expect(p.locator('#homePulse .metric-explain')).to_have_count(6);expect(p.locator('#homeView')).not_to_contain_text('punteggio di conformità',use_inner_text=True);no_overflow(p,'home-desktop')
