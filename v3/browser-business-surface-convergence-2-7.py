...
def goto_proof(page):
 page.goto(f'{BASE}/?view=proof',wait_until='networkidle');ready(page);expect(page.locator('#proofView')).to_be_visible();page.wait_for_selector('#proofContent:not([hidden])');page.wait_for_function("()=>document.querySelectorAll('#proofView [data-proof-tab]').length===6")
...
try:
 with sync_playwright() as pw:
  ...
  PHASE='evidence-tabs';goto_proof(p);tabs=p.locator('#proofView [data-proof-tab]');expect(tabs).to_have_count(6);expect(p.locator('#proofTitle')).to_have_text('Evidenze ICTC')
