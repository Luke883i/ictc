...
def audit_one(page,code,pid):
 global PHASE
 host=open_process(page,code,pid)
 PHASE=f'{code}-attention-slot';slot=page.locator(f'{host} > [data-procedure-attention-slot="{pid}"]');expect(slot).to_have_count(1)
 PHASE=f'{code}-worklist-visible';section=slot.locator(':scope > [data-procedure-worklist]');expect(section).to_be_visible()
 owner=slot.get_attribute('data-attention-slot-owner');assert owner==OWNER[pid],(pid,owner,OWNER[pid])
 PHASE=f'{code}-first-operational-order';frame=page.locator(f'{host} > .procedure-frame');expect(frame).to_have_count(1);assert frame.evaluate('(node,slot)=>node.nextElementSibling===slot',slot.element_handle()),f'{pid}: attention is not first operational block after compact identity';assert section.get_attribute('data-placement-owner')==owner,(pid,section.get_attribute('data-placement-owner'),owner)
 PHASE=f'{code}-bounded-controls';facets=section.locator('[data-worklist-facet]');facet_count=facets.count();assert facet_count<=1,(pid,'facet-count',facet_count);facet_key=None
 if facet_count: facet_key=facets.first.get_attribute('data-worklist-facet');assert facet_key and facet_key!='state',(pid,facet_key)
 immediate=section.locator('.procedure-worklist-filters input[type="search"],.procedure-worklist-filters select[data-worklist-facet]');assert immediate.count()<=2,(pid,'immediate-controls',immediate.count());reveal=section.locator(':scope > details.procedure-worklist-reveal');expect(reveal).to_have_count(1);assert reveal.get_attribute('open') is None,f'{pid}: terminal/quiescent reveal must be collapsed by default'
 PHASE=f'{code}-default-work';items=section.locator('[data-work-item-id]');count=items.count();assert count>0,f'{pid}: DEMO must expose actionable A1 work'
 for i in range(min(count,40)):
  item=items.nth(i);assert item.get_attribute('data-work-item-actionable')=='true',f'{pid}: inactive row leaked into default worklist';kind=item.get_attribute('data-work-item-kind') or '';state=item.get_attribute('data-work-item-state') or '';visible=item.inner_text().lower();assert f'{kind} · {state}'.lower() not in visible,(pid,kind,state,visible[:200]);assert item.locator('.procedure-work-business-kind').inner_text().strip();assert item.locator('.procedure-work-situation').inner_text().strip();assert item.locator('[data-work-item-target]').count()<=1,(pid,'duplicate-primary',i)
...
try:
 with sync_playwright() as pw:
  ...
  request=ctx.request;PHASE='runtime-health';health=api_get(request,'/api/health');assert health['status']==200,health;PHASE='runtime-identity';identity=api_get(request,'/api/admin/identity');assert identity['status']==200,identity
  ...
  if EXPECTED_SHA: assert build.get('sha')==EXPECTED_SHA,(EXPECTED_SHA,build);assert build.get('exact') is True,build;assert build.get('dirty') is False,build
  ...
  PHASE='aggregate-coverage';assert len(RESULTS)==7;assert all(r['target']['resolution']!='unresolved' for r in RESULTS);assert len({r['procedureId'] for r in RESULTS})==7
  PHASE='aggregate-network-failures';assert not FAILURES,FAILURES
  PHASE='aggregate-page-errors';assert not PAGE_ERRORS,PAGE_ERRORS
  PHASE='aggregate-write-boundary';writes=[x for x in NETWORK if x['method']!='GET'];assert not writes,writes
  report={'ok':True,'slice':'S4-A1','profile':'worklist-presentation-authority','expectedBuildSha':EXPECTED_SHA or None,'runtimeIdentity':build_identity,'procedures':RESULTS,'metrics':{'procedureCoverage':'7/7','firstOperationalAttention':'7/7','localFacetSemantics':'7/7 source/runtime projection; browser controls cardinality-aware 0..1','rawKindStateDefault':0,'genericProcessOpener':0,'exactTargetResolved':'7/7','sharedGlobalPlacementAuthority':0,'immediateControlMax':2,'unintendedWrites':0},'network':NETWORK,'networkFailures':FAILURES,'pageErrors':PAGE_ERRORS,'claimBoundary':'Exact-checked-out-head server-backed browser evidence for representative DEMO work items on seven canonical procedures. It proves bounded DOM placement and target resolution, not one million browser executions, human usability, accessibility certification, or enterprise-ready status.'};(ART/'browser-s4-a1-worklist-authority.json').write_text(json.dumps(report,indent=2,ensure_ascii=False),encoding='utf8');print(json.dumps({'ok':True,'slice':'S4-A1','procedures':7,'exactTargets':7,'build':build,'writes':0}),flush=True);ctx.close();browser.close()
