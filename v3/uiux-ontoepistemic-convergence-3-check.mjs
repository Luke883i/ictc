...
assert.ok(src.slots.includes("SUPPORT_NAMES=new Set(['advanced-context'])")&&src.slots.includes("SYNTHETIC=new Set(['reference','metrics','attention','advanced-context','evidence','technical','boundary'"),'R3 references must be an independent editorial slot before work, not header copy or advanced context');
