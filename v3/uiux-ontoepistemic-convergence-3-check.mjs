...
const src={
 ...
 workflow:r('../.github/workflows/s4-a6-ux4-semantic-surface.yml'),browser:r('./browser-s4-a6-ux4-semantic-surface.py'),browserFinetune:r('./browser-procedure-finetuning-1-4-base.py'),common:r('./public/ui/common.js'),actions:r('./public/ui/actions.js'),grcbase:r('./public/ui/grc-workspace-base.js'),worklist:r('./public/ui/procedure-worklist.js'),p2:r('./public/enduser-composition-p2.css'),standard:r('./public/ui/standard-browser.js'),proof:r('./public/ui/proof-workspace-3-2.js'),stable:r('./public/ui/stable-shell.js'),model:r('./runtime/model.mjs'),server:r('./server.mjs'),anatomyJs:r('./public/ui/procedure-anatomy.js'),adminws:r('./public/ui/admin-workspace-3-2.js')
};
...
assert.ok(src.browser.includes("'readOnlyRecordCount':read_only_cards.count()")&&src.browser.includes("cards.every(card=>card.dataset.readOnly==='true')"),'O1 auditor AP oracle must settle on all-record read-only coverage before asserting');
assert.ok(src.browserFinetune.includes('[data-open-monitor-manager]')&&src.browserFinetune.includes('#monitorManagerDialog')&&src.browserFinetune.includes('[data-a6-registry-count]')&&!src.browserFinetune.includes('data-a6-registry-total')&&!src.browserFinetune.includes('[data-rn-open-scheduler]'),'O1 RN monitor browser oracle must follow the canonical monitor manager and primary count owner');
