...
const js = [await readFile(new URL('app.js', publicRoot), 'utf8'),...await Promise.all(uiFiles.map(name => readFile(new URL(`ui/${name}`, publicRoot), 'utf8')))].join('\n');
...
console.log(`ui-contract-check: ok (${uiFiles.length} UI modules, role-correct recovery and protected evidence)`);
