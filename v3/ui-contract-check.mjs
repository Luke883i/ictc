...
for(const required of ['data-download-evidence','downloadProtected','sourceDecisionReason','pauseReason','closureNote','Riprova analisi AI','formulationVersions','observations'])assert.ok(js.includes(required)||html.includes(required),required);
for(const forbidden of ['tenantSelect','reviewer','semantic graph','control mapping','owner confirmation','prompt(','confirm('])assert.ok(!`${html}\n${js}`.toLowerCase().includes(forbidden.toLowerCase()),forbidden);
assert.ok(js.includes('x-ictc-expected-revision'));
assert.ok(css.includes('prefers-reduced-motion'));
assert.ok(uiFiles.length>=5);
console.log(`ui-contract-check: ok (${uiFiles.length} UI modules, three canonical roles, role-correct recovery and protected evidence)`);
