...
export function validateP6(contract){
  ...
  const ai=contract?.aiStatus||{};if(ai.visualMode!=='icon-only'||ai.fixedLabel!==false||ai.readyTooltip!=='AI disponibile'||!String(ai.unconfiguredTooltip||'').startsWith('AI non configurata')||!String(ai.keyMissingTooltip||'').startsWith('Chiave AI non disponibile')||ai.readyIcon!=='sparkles'||ai.unconfiguredIcon!=='info'||ai.keyMissingIcon!=='triangle-alert'||ai.roleVisibleElsewhere!==true)errors.push('ai-status');
