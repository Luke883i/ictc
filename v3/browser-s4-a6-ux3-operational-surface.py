...
def admin_desktop(browser):
 ...
 PHASE='ec';ec=open_process(page,'EC-01');ereg=ec.locator('[data-a6-registry="incidents"]');expect(ereg).to_be_visible();expect(ereg).to_have_attribute('open','');expect(ereg.locator('[data-a6-search="incidents"]')).to_be_visible();expect(ereg.locator('[data-a6-state="incidents"]')).to_have_value('open');expect(ereg).to_have_attribute('data-a6-unbound-records','0');no_overflow(page,'desktop:ec')
