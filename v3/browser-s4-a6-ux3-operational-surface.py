...
def admin_desktop(browser):
 ...
 rn=open_process(page,'RN-01');PHASE='rn-assert';reg=rn.locator(':scope > [data-rn-monitoring-secondary][data-a6-registry="monitoring"]');expect(reg).to_have_count(1);expect(reg.locator(':scope > summary')).to_be_visible();mission_total=int(reg.get_attribute('data-a6-registry-total') or '0');
 if mission_total>0: expect(reg).to_have_attribute('open','');expect(reg.locator('[data-a6-search="monitoring"]')).to_be_visible()
 else: expect(reg).not_to_have_attribute('open','');expect(reg.locator('[data-a6-state="monitoring"]')).to_have_value('current');expect(reg).to_have_attribute('data-a6-unbound-records','0');expect(rn.locator('[data-open-contribution] strong')).to_have_text('Aggiungi fonte o materiale');no_overflow(page,'desktop:rn')
