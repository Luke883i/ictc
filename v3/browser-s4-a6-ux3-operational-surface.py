...
def admin_desktop(browser):
 ...
 ec=open_process(page,'EC-01');PHASE='ec-assert';ereg=ec.locator('[data-a6-registry="incidents"]');expect(ereg).to_be_visible();expect(ereg).to_have_attribute('open','');expect(ereg.locator('[data-a6-search="incidents"]')).to_have_count(0);expect(ereg.locator('[data-a6-state="incidents"]')).to_have_count(0);expect(ereg.locator('[data-seq-queue-search="incidents"]')).to_be_visible();expect(ereg.locator('[data-seq-queue-state="incidents"]')).to_be_visible();expect(ereg.locator(':scope > summary [data-a6-registry-count]')).to_contain_text(' di ');expect(ereg).to_have_attribute('data-a6-unbound-records','0');expect(ereg.locator('.incident-card.p2-record-row').first).to_have_attribute('data-record-grammar','row-list');no_overflow(page,'desktop:ec')
