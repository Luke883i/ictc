...
def open_process(page,code):
 global PHASE
 processes(page).click();card=page.locator(f'#procedureHub [data-process-code="{code}"]');expect(card).to_be_visible();card.locator(':scope > footer .procedure-primary').click();pid,root=CODES[code];PHASE=f'{code}-owner';page.wait_for_function("x=>{const r=document.querySelector(x.root);return !!(r&&r.offsetParent!==null&&r.dataset.a6Ux3Operational==='a6-ux3'&&r.dataset.a6OperationalOwner===x.owner)}",arg={'root':root,'owner':pid});return page.locator(root)
...
def admin_desktop(browser):
 ...
 if priorities.count():expect(priorities.first.locator('.a6-process-icon')).to_have_count(1)
 no_overflow(page,'desktop:home');footer_clear(page,'desktop:home')
 PHASE='rn';rn=open_process(page,'RN-01');reg=rn.locator('[data-a6-registry="monitoring"]');expect(reg).to_be_visible();expect(reg).not_to_have_attribute('open','');reg.locator(':scope > summary').click();expect(reg).to_have_attribute('open','');expect(reg.locator('[data-a6-search="monitoring"]')).to_be_visible();expect(reg.locator('[data-a6-state="monitoring"]')).to_have_value('active');expect(reg).to_have_attribute('data-a6-unbound-records','0');expect(rn.locator('[data-open-contribution] strong')).to_have_text('Aggiungi fonte o materiale');no_overflow(page,'desktop:rn')
 PHASE='ec';ec=open_process(page,'EC-01');ereg=ec.locator('[data-a6-registry="incidents"]');expect(ereg).to_be_visible();expect(ereg).not_to_have_attribute('open','');ereg.locator(':scope > summary').click();expect(ereg).to_have_attribute('open','');expect(ereg.locator('[data-a6-search="incidents"]')).to_be_visible();expect(ereg.locator('[data-a6-state="incidents"]')).to_have_value('open');expect(ereg).to_have_attribute('data-a6-unbound-records','0');no_overflow(page,'desktop:ec')
 PHASE='ao';ao=open_process(page,'AO-01');expect(ao.locator('[data-seq-ao-search]')).to_have_count(1);expect(ao.locator('[data-seq-ao-filter]')).to_have_count(1);expect(ao.locator('[data-seq-ao-search]')).to_have_attribute('data-a6-stable-record-search','');expect(ao.locator('[data-seq-ao-filter]')).to_have_attribute('data-a6-stable-state-filter','');no_overflow(page,'desktop:ao')
 PHASE='mc';mc=open_process(page,'MC-01');expect(mc.locator('[data-framework-card]')).to_have_count(21);expect(mc.locator('.finetune-concept-drilldown:visible')).to_have_count(0);scopes=mc.locator('.market-scope-editor');expect(scopes).to_have_count(21);scope=scopes.first;expect(scope.locator(':scope > summary')).to_be_visible();scope.locator(':scope > summary').click();expect(scope).to_have_attribute('open','');position=scope.evaluate("e=>getComputedStyle(e).position");assert position=='fixed',position;scope.locator(':scope > summary').click();expect(scope).not_to_have_attribute('open','');entries=mc.locator('[data-open-standard-browser]');expect(entries).to_have_count(21);expect(entries.first).to_have_text('Comprendi standard');entries.first.click();dialog=page.locator('#standardBrowserDialog');expect(dialog).to_be_visible();expect(dialog.locator('[data-standard-nodes]')).to_be_visible();expect(dialog.locator('[data-standard-detail]')).to_be_visible();nodes=dialog.locator('[data-standard-node-select]');
 if nodes.count():nodes.first.click();expect(dialog.locator('[data-standard-detail] h3')).to_be_visible()
 expect(dialog.locator('[data-standard-use]')).to_have_count(0);dialog.locator('button[aria-label="Chiudi"]').click();expect(dialog).not_to_be_visible();no_overflow(page,'desktop:mc');footer_clear(page,'desktop:mc')
