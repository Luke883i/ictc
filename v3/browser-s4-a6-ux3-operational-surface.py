...
def open_process(page,code):
 global PHASE
 processes(page).click();card=page.locator(f'#procedureHub [data-process-code="{code}"]');expect(card).to_be_visible();card.locator(':scope > footer .procedure-primary').click();pid,root=CODES[code];PHASE=f'{code}-owner'
 ready="""x=>{const r=document.querySelector(x.root),h=document.documentElement,w=r?.querySelector(':scope > [data-procedure-attention-slot="'+x.owner+'"] [data-procedure-worklist]');return !!(r&&r.offsetParent!==null&&r.dataset.a6Ux3Operational==='a6-ux3'&&r.dataset.a6OperationalOwner===x.owner&&h.dataset.a6Ux4Semantic==='a6-ux4'&&w?.dataset.a6Ux4Mount==='semantic-bridge')}"""
 page.wait_for_function(ready,arg={'root':root,'owner':pid});return page.locator(root)
...
def admin_desktop(browser):
 ...
 page=ctx.new_page();page.set_default_timeout(30000);errors=[];writes=[];ensure_onboarded(page,BASE,'admin');page.on('pageerror',lambda e:errors.append(str(e)));page.on('request',lambda r:writes.append({'method':r.method,'url':r.url}) if r.url.startswith(BASE+'/api/') and r.method!='GET' else None)
 PHASE='home';page.goto(BASE+'/?view=home',wait_until='networkidle');page.wait_for_function("()=>document.documentElement.dataset.a6Ux3Operational==='a6-ux3'");home=page.locator('#homeView');expect(home).to_have_attribute('data-a6-operational-owner','home');expect(page.locator('#homeTitle')).to_have_text('Integrated Compliance Tower Control');expect(page.locator('#homePriorities')).to_have_attribute('data-a6-operational-queue','decision-then-priority');priorities=page.locator('#homePriorities .home-business-priority');
 PHASE='home-cognitive-order';order=page.evaluate("""()=>{const hero=document.querySelector('#homeView .home-hero'),intro=hero?.querySelector(':scope > .home-intro'),decision=hero?.querySelector(':scope > .home-next'),queue=hero?.querySelector(':scope > #homePriorities');const children=[...(hero?.children||[])];const box=e=>{const r=e.getBoundingClientRect();return{x:r.x,y:r.y,right:r.right,bottom:r.bottom,width:r.width,height:r.height}};return{dom:[children.indexOf(intro),children.indexOf(decision),children.indexOf(queue)],intro:box(intro),decision:box(decision),queue:box(queue)}}""");assert order['dom']==[0,1,2],order;assert order['queue']['y']>=max(order['intro']['bottom'],order['decision']['bottom'])-2,order;RESULTS.append({'oracle':'home-cognitive-order','metrics':order})
 ...
 rn=open_process(page,'RN-01');PHASE='rn-assert';reg=rn.locator(':scope > [data-rn-monitoring-secondary][data-a6-registry="monitoring"]');expect(reg).to_have_count(1);expect(reg).not_to_have_attribute('open','');expect(reg.locator(':scope > summary')).to_be_visible();expect(reg.locator('[data-a6-search="monitoring"]')).to_have_count(0);expect(reg.locator('[data-a6-state="monitoring"]')).to_have_count(0);expect(reg).to_have_attribute('data-a6-unbound-records','0');reg.locator(':scope > summary').click();expect(reg).to_have_attribute('open','');expect(reg.locator('[data-seq-queue-search="monitoring-jobs"]')).to_be_visible();expect(reg.locator('[data-seq-queue-state="monitoring-jobs"]')).to_be_visible();expect(reg.locator(':scope > summary [data-a6-registry-count]')).to_contain_text(' di ');expect(rn.locator('[data-open-contribution] strong')).to_have_text('Aggiungi fonte o materiale');no_overflow(page,'desktop:rn')
 ec=open_process(page,'EC-01');ereg=ec.locator('[data-a6-registry="incidents"]');PHASE='ec-registry-visible';expect(ereg).to_be_visible();PHASE='ec-registry-open';expect(ereg).to_have_attribute('open','');PHASE='ec-retired-filter-absent';expect(ereg.locator('[data-a6-search="incidents"]')).to_have_count(0);expect(ereg.locator('[data-a6-state="incidents"]')).to_have_count(0);PHASE='ec-canonical-search';expect(ereg.locator('[data-seq-queue-search="incidents"]')).to_be_visible();PHASE='ec-canonical-state';expect(ereg.locator('[data-seq-queue-state="incidents"]')).to_be_visible();PHASE='ec-single-count';expect(ereg.locator(':scope > summary [data-a6-registry-count]')).to_contain_text(' di ');PHASE='ec-bound-records';expect(ereg).to_have_attribute('data-a6-unbound-records','0');PHASE='ec-row-grammar';ec_rows=ereg.locator('.incident-card');
 if ec_rows.count():
  for i in range(ec_rows.count()):expect(ec_rows.nth(i)).to_have_attribute('data-record-grammar','row-list')
 else:expect(ereg.locator('.empty')).to_be_visible()
 PHASE='ec-overflow';no_overflow(page,'desktop:ec')
