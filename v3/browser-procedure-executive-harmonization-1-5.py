...
def check_frame(page,code):
 id=IDS[code]; root=root_for(code); frame=page.locator(f'{root} > .procedure-decision-frame[data-executive-procedure="{id}"]'); expect(frame).to_have_count(1); expect(frame).to_be_visible(); assert frame.get_attribute('data-procedure-guidance-authority')=='procedure-guidance-projection'; detail=frame.locator(':scope > details.composition-process-context'); expect(detail).to_have_count(1); expect(detail.locator(':scope > summary')).to_have_text('Contesto decisionale'); assert detail.get_attribute('open') is None; assert frame.evaluate("e=>{const work=e.previousElementSibling;return !!work}"); return frame
try:
 with sync_playwright() as pw:
  ...
  PHASE='hub'; expect(page.locator('#procedureHub .executive-process-card')).to_have_count(7); assert 'da decidere' not in page.locator('#procedureHub').inner_text(); assert 'Nessuna attenzione aperta' not in page.locator('#procedureHub').inner_text()
  for code in LABELS:
  ...
  PHASE='mobile'; mobile=browser.new_context(viewport={'width':390,'height':844}); mobile.add_init_script("localStorage.setItem('ictc-role','admin');localStorage.setItem('ictc-service','processes')"); m=mobile.new_page(); m.goto(BASE+'/?view=processes',wait_until='networkidle'); no_overflow(m); openp(m,'AR-01'); check_frame(m,'AR-01'); no_overflow(m); mobile.close()
  out={'ok':True,'profile':'procedure-executive-harmonization-1.5+semantic-composition-3.1','procedures':list(LABELS),'canonicalMetricSignalsPreservedButHidden':True,'presentationEvidenceSibling':True,'decisionContextProgressiveDisclosure':True,'workBeforeDecisionContext':True,'mobileOverflow':False,'evidenceClass':'E2 server-backed browser; not human usability, legal review or independent assurance'}; (ART/'browser-procedure-finetuning-1-4-executive.json').write_text(json.dumps(out,indent=2),encoding='utf8'); print('browser-procedure-executive-harmonization-1-5: complete',flush=True); ctx.close(); browser.close()
