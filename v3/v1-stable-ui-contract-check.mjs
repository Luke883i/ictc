...
assert.match(active,/installStableShell/);assert.match(active,/renderProcedureHub/);assert.match(active,/filter\(item=>item\.code&&item\.id!=='evidence'\)/);assert.match(active,/stable-process-card/);assert.match(active,/installStableProfileMenu/);assert.match(active,/adminButton\.hidden=true/);assert.match(active,/settingsButton\.hidden=true/);
...
assert.match(styles,/stable-experience\.css/);assert.match(css,/stable-header-inner/);assert.match(css,/height:52px/);assert.match(css,/home-hero/);assert.match(css,/procedure-hub/);assert.match(css,/stable-profile-menu/);assert.match(css,/stable-legal-footer/);assert.match(css,/position:fixed/);
...
console.log('v1-stable-ui-contract-check: ok (3 surfaces, 7 processes, 5 disclosure levels, thin canonical shell and persistent legal footer)');
