...
try:
    with sync_playwright() as pw:
        ...
        expect(page.locator('.proof-reading-card')).to_have_count(3); expect(page.locator('.proof-section')).to_have_count(5); expect(page.locator('#proofMethodTitle')).to_have_text('Come ICTC dimostra la propria postura')
        expect(page.locator('.proof-reading-grid')).to_contain_text('Osservabile'); expect(page.locator('.proof-reading-grid')).to_contain_text('Da completare'); expect(page.locator('.proof-reading-grid')).to_contain_text('Confine'); no_overflow(page); min_height(page, '.proof-section > summary')
        ...
        PHASE = 'evidence-disclosure'; open_rn(page); menu = page.locator('.evidence-export-menu').first; expect(menu).to_be_visible(); summary = menu.locator(':scope > summary'); summary.focus(); summary.press('Enter'); expect(menu).to_have_attribute('open', ''); expect(menu.locator('[data-evidence-download]')).to_have_count(4); min_height(page, '.evidence-export-menu > summary'); min_height(page, '.evidence-export-menu [data-evidence-download]')
