...
def ready(page):
    ...
    page.wait_for_function("()=>{const r=document.querySelector('#proofView'),c=document.querySelector('#proofContent');return document.documentElement.dataset.nativeSemanticLattice==='3.2.0'&&r?.dataset.localCompositionOwner==='proof-workspace-3-2.js'&&r?.dataset.semanticWorkspaceClosure==='3.2.1'&&!!c?.querySelector(':scope > details[data-proof-workspace=\"epistemic-investigation\"]')&&!!c?.querySelector(':scope > details[data-proof-workspace=\"trace-reconstruction\"]')&&!!c?.querySelector(':scope > details.proof-section[data-information-role=\"evidence\"]')&&!!c?.querySelector(':scope > details[data-composition-detail=\"proof-reading\"]')}")
...
def assert_investigation_first(page):
    ...
    investigation=page.locator('#proofContent > details[data-proof-workspace="epistemic-investigation"]');trace=page.locator('#proofContent > details[data-proof-workspace="trace-reconstruction"]');decisions=page.locator('#proofContent > details.proof-section[data-information-role="evidence"]');reading=page.locator('#proofContent > details[data-composition-detail="proof-reading"]')
    for node in [investigation,trace,decisions,reading]:expect(node).to_have_count(1);expect(node).not_to_have_attribute('open','')
    expect(investigation.locator(':scope > summary')).to_contain_text('Reticolo epistemico');expect(trace.locator(':scope > summary')).to_contain_text('Ricostruisci un elemento di lavoro');expect(decisions.locator(':scope > summary')).to_contain_text('Decisioni e tracciabilità');expect(reading.locator(':scope > summary b')).to_have_text('Criteri di lettura e sintesi tecnica')
