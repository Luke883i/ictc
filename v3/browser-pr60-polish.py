...
def ready(page):
    ...
    page.wait_for_function("()=>{const r=document.querySelector('#proofView'),c=document.querySelector('#proofContent');return document.documentElement.dataset.nativeSemanticLattice==='3.2.0'&&r?.dataset.localCompositionOwner==='proof-workspace-3-2.js'&&!!c?.querySelector(':scope > details.proof-section[data-information-role=\"evidence\"]')&&!!c?.querySelector(':scope > details[data-composition-detail=\"proof-reading\"]')}")
