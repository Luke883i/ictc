...
def ready(page):
 page.wait_for_function("()=>document.documentElement.dataset.businessSurfaceConvergence==='2.7.0'");page.wait_for_selector('#proofContent:not([hidden])');page.wait_for_function("()=>document.querySelectorAll('#proofView [data-proof-tab]').length===6")
def open_rn(page):
...
try:
 with sync_playwright() as pw:
  ...
  PHASE='evidence-first-view';page.goto(BASE+'/?view=proof',wait_until='networkidle');ready(page);expect(page.locator('#proofView')).to_be_visible();expect(page.locator('#proofTitle')).to_have_text('Evidenze ICTC');expect(page.locator('.proof-semantic-qualifier')).to_have_text('Dati osservabili, metodo e limiti della prova');tabs=page.locator('#proofView [data-proof-tab]');expect(tabs).to_have_count(6);expect(page.locator('[data-proof-tab="summary"]')).to_have_attribute('aria-selected','true');assert page.locator('#proofView [data-proof-panel]:visible').count()==1;expect(page.locator('[data-proof-panel="summary"] .proof-reading-card')).to_have_count(3);expect(page.locator('#proofMethodTitle')).to_be_visible();min_height(page,'.proof-scope-tabs [role="tab"]');no_overflow(page)
  PHASE='evidence-refresh-ownership';initial_requests=len(proof_requests);assert initial_requests>=1,proof_requests;page.evaluate("()=>{for(let i=0;i<8;i++)document.dispatchEvent(new CustomEvent('ictc:rendered'));}");page.wait_for_timeout(180);assert len(proof_requests)==initial_requests,(initial_requests,len(proof_requests),proof_requests);revision=int(page.locator('html').get_attribute('data-ictc-projection-revision') or 0);page.evaluate("r=>document.dispatchEvent(new CustomEvent('ictc:projection-committed',{detail:{revision:r,actorRole:'admin'}}))",revision+1);page.wait_for_function('(count)=>performance.getEntriesByType("resource").filter(x=>x.name.includes("/api/standard-proof")).length>=count',arg=initial_requests+1);page.wait_for_function('(r)=>Number(document.querySelector("#proofView")?.dataset.loadedRevision||0)>=r',arg=revision+1)
  PHASE='evidence-keyboard-tabs';summary=page.locator('[data-proof-tab="summary"]');summary.focus();summary.press('ArrowRight');expect(page.locator('[data-proof-tab="decisions"]')).to_be_focused();expect(page.locator('[data-proof-tab="decisions"]')).to_have_attribute('aria-selected','true');expect(page.locator('[data-proof-panel="decisions"]')).to_be_visible();assert page.locator('#proofView [data-proof-panel]:visible').count()==1;page.locator('[data-proof-tab="decisions"]').press('End');expect(page.locator('[data-proof-tab="export"]')).to_be_focused();expect(page.locator('[data-proof-panel="export"]')).to_be_visible();page.screenshot(path=str(ART/'ux-pr60-evidence-desktop.png'),full_page=True)
  ...
  PHASE='mobile-evidence';mobile_ctx=browser.new_context(viewport={'width':390,'height':844});mobile_ctx.add_init_script("localStorage.setItem('ictc-role','admin');localStorage.setItem('ictc-service','proof')");mobile=mobile_ctx.new_page();mobile.set_default_timeout(30000);mobile.goto(BASE+'/?view=proof',wait_until='networkidle');ready(mobile);expect(mobile.locator('#proofTitle')).to_have_text('Evidenze ICTC');expect(mobile.locator('.proof-semantic-qualifier')).to_have_text('Dati osservabili, metodo e limiti della prova');expect(mobile.locator('[data-proof-tab]')).to_have_count(6);assert mobile.locator('#proofView [data-proof-panel]:visible').count()==1;min_height(mobile,'.proof-scope-tabs [role="tab"]');no_overflow(mobile);mobile.screenshot(path=str(ART/'ux-pr60-evidence-mobile.png'),full_page=True);mobile_ctx.close()
  out={'ok':True,'profile':'pr60-evidence-polish-2.7','evidence':{'stableRouteHeading':'Evidenze ICTC','semanticQualifier':'Dati osservabili, metodo e limiti della prova','tabs':6,'singleVisiblePanel':True,'proofMethodVisible':True,'genericRenderRefetches':0,'projectionCommitRefresh':True},'evidenceDownloads':downloaded,'keyboard':{'tabs':True,'escapeReturnsFocus':True},'touchTargetsMin':44,'mobileOverflow':False,'evidenceClass':'E2-server-backed-browser; automated UI evidence, not independent human usability or AT assessment'};(ART/'browser-pr60-polish.json').write_text(json.dumps(out,indent=2),encoding='utf8');print('browser-pr60-polish: complete',flush=True);ctx.close();browser.close()
