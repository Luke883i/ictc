...
async function warmupFleet(){const warm=await Promise.all(Array.from({length:64},(_,index)=>commit(100000+index,{tenantId:`warm-${String(index%4).padStart(2,'0')}`,port:ports[index%2],commandId:`warm-${index}`,subjectId:`risk-warm-${index}`,actorId:`warm-user-${index}`,payload:{warmup:true,index}}))),failures=warm.filter(item=>item.status!==200);assert.deepEqual(failures.slice(0,3),[],'steady-state warm-up must have no failed writes');return{writes:warm.length,failures:failures.length};}
async function fleet1000({checkStatus=true,checkLatency=true}={}){const warmup=checkLatency?await warmupFleet():null,fleet=await Promise.all(Array.from({length:1000},(_,index)=>commit(index))),failures=fleet.filter(item=>item.status!==200),p95=percentile(fleet.map(item=>item.durationMs),.95),p99=percentile(fleet.map(item=>item.durationMs),.99);if(checkStatus)assert.deepEqual(failures.slice(0,3),[],'1000-client fleet canary must have no failed point writes');if(checkLatency)assert.ok(p95<5000,`fleet canary p95 pathological after steady-state warm-up: ${p95}`);return{writes:1000,failures:failures.length,warmupWrites:warmup?.writes||0,p95Ms:Number(p95.toFixed(2)),p99Ms:Number(p99.toFixed(2))};}
