...
async function fleet1000(){const fleet=await Promise.all(Array.from({length:1000},(_,index)=>commit(index))),failures=fleet.filter(item=>item.status!==200),p95=percentile(fleet.map(item=>item.durationMs),.95),p99=percentile(fleet.map(item=>item.durationMs),.99);assert.deepEqual(failures.slice(0,3),[],'1000-client fleet canary must have no failed point writes');assert.ok(p95<5000,`fleet canary p95 pathological: ${p95}`);return{writes:1000,failures:failures.length,p95Ms:Number(p95.toFixed(2)),p99Ms:Number(p99.toFixed(2))};}
