...
try:
    with sync_playwright() as pw:
        ...
        page.locator('#incidentDialog textarea[name="originalNarrative"]').fill('Evento di convergenza UI usato come source per verificare readiness asincrona e provenance del draft collegato.')
        page.locator('#incidentDialog input[name="awarenessAt"]').fill('2026-08-11T08:45')
        page.locator('#incidentDialog button[type="submit"]').click()
        expect(page.locator('#incidentDialog')).not_to_be_visible()
        expect(page.locator('#incidentList .incident-card').first).to_be_visible()

        PHASE='draft-readiness'
        cross=page.locator('.procedure-cross-tools[data-cross-tools="incidents"] [data-cross-create]')
        expect(cross).to_be_visible(); cross.click()
        dialog=page.locator('#crossCreateDialog'); expect(dialog).to_be_visible()
        target=page.locator('#crossTarget'); submit=page.locator('[data-cross-submit]')
        expect(target).to_be_enabled(); expect(submit).to_be_enabled()
        targets=target.locator('option').evaluate_all("xs=>xs.map(x=>x.value).filter(Boolean)")
        assert 'actions' in targets,targets
        assert 'incidents' not in targets,targets
        target.select_option('actions')
        expect(submit).to_be_enabled()
        page.locator('#crossTitle').fill('Azione draft da convergenza UI')
        page.locator('#crossDescription').fill('Draft collegato: ownership, priorita e chiusura restano checkpoint AP-01.')
        submit.click(); expect(dialog).not_to_be_visible()

        PHASE='compression-reversibility'
        processes(page).click()
        meta=page.locator('#epistemicMetaCard'); expect(meta).to_be_visible(); meta.locator('[data-service="epistemic"]').click()
        expect(page.locator('#epistemicView')).to_be_visible()
        compression=page.locator('#epistemicCompression'); expect(compression).to_be_visible()
        rev=int(compression.get_attribute('data-state-revision') or '0')
        digest=compression.get_attribute('data-projection-digest') or ''
        ...
        page.locator('#epistemicProcedureFilter').select_option('')

        PHASE='graph-geometry'
        page.locator('[data-epistemic-mode="graph"]').click(); graph_ready(page)
        visual=page.locator('.epistemic-graph-canvas circle.epistemic-graph-node').first
        hit=page.locator('.epistemic-graph-canvas circle.epistemic-graph-hit').first
        assert float(visual.get_attribute('r') or 0)==10
        assert float(hit.get_attribute('r') or 0)==22
        hb=hit.bounding_box(); vb=visual.bounding_box()
        assert hb and vb and hb['width']>=43.5 and vb['width']<hb['width'],(hb,vb)
        ...
        capture['on']=True
        page.mouse.move(baseline['center']['x'],baseline['center']['y']); page.mouse.down(); page.mouse.move(baseline['center']['x']+56,baseline['center']['y']+36,steps=6); page.mouse.up(); page.wait_for_timeout(80)
        capture['on']=False
        after=page.evaluate("""id=>{const g=[...document.querySelectorAll('.epistemic-graph-canvas g[data-epistemic-node]')].find(x=>x.dataset.epistemicNode===id);const line=[...document.querySelectorAll('.epistemic-graph-edge')].find(l=>l.dataset.graphFrom===id||l.dataset.graphTo===id);if(!g||!line)return null;const from=line.dataset.graphFrom===id;return{transform:g.getAttribute('transform')||'',x:Number(line.getAttribute(from?'x1':'x2')),y:Number(line.getAttribute(from?'y1':'y2'))};}""",drag_id)
        assert after and after['transform'].startswith('translate('),after
        assert abs(after['x']-baseline['x'])>20 and abs(after['y']-baseline['y'])>15,(baseline,after)
        ...
        page.locator('[data-graph-reset]').click(); expect(page.locator('[data-graph-zoom-label]')).to_have_text('100%')
        capture['on']=False; assert not writes,writes
        no_overflow(page)
        page.screenshot(path=str(ART/'ux-ui-epistemic-convergence-desktop.png'),full_page=True)

        PHASE='mobile-containment'
        ...
        m=mc.new_page(); m.goto(BASE+'/?view=processes',wait_until='networkidle')
        m.locator('#epistemicMetaCard [data-service="epistemic"]').click(); expect(m.locator('#epistemicCompression')).to_be_visible(); no_overflow(m)
        m.locator('[data-epistemic-mode="graph"]').click(); graph_ready(m); no_overflow(m)
        heights=m.locator('.epistemic-graph-controls button').evaluate_all("xs=>xs.map(x=>x.getBoundingClientRect().height)")
        assert heights and min(heights)>=43.5,heights
