...
assert.match(security, /CodeQL[^\n]{0,180}(?:conditional|condizionale)/i, 'SECURITY must preserve the conditional CodeQL boundary');
assert.match(security, /not executed|non eseguito/i, 'SECURITY must declassify skipped CodeQL as not executed');
assert.match(security, /first-party security SAST/i, 'SECURITY must name the unconditional first-party SAST rail');
assert.match(security, /runtime security DoD/i, 'SECURITY must name the runtime security DoD rail');
assert.match(security, /(?:process-local|in-process)[^\n]{0,180}(?:distributed|multi-host)/i, 'SECURITY must preserve the local-vs-distributed abuse-control boundary');
for (const flag of ['ICTC_ALLOW_PRIVATE_AI', 'ICTC_ALLOW_INSECURE_AI']) assert.ok(security.includes(flag), `SECURITY missing AI network opt-in ${flag}`);
