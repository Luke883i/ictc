...
assert.equal(pkg.scripts.test, 'npm run test:current');assert.equal(pkg.private,true);assert.match(testing, /npm test` esegue `test:current`/);assert.match(testing, /exact PR HEAD/i);assert.match(development, /state\.sqlite/);
...
assert.ok(readme.includes('docs/START_HERE.md'), 'README must expose the newcomer route');assert.ok(agents.includes('docs/START_HERE.md'), 'AGENTS must expose the newcomer route');for (const token of ['README.md', 'AGENTS.md', 'docs/authority-matrix.yaml', 'docs/11_ARCHITECTURE.md', 'npm test']) assert.ok(startHere.includes(token), `START_HERE missing route token ${token}`);
assert.match(startHere, /mappa[^\n]{0,80}non[^\n]{0,80}autorità/i, 'START_HERE must not present itself as a new authority');assert.match(startHere, /documenti storici[^\n]{0,120}non[^\n]{0,80}autorità corrente/i, 'START_HERE must distinguish design lineage from current authority');
console.log(JSON.stringify({ok:true,check:'documentation-authority',authorities:['sqlite-state-persistence','package:test','release-identity-v2','security-workflow-boundary'],onboardingRoute:'docs/START_HERE.md',files:normative.map(([name]) => name)}));
