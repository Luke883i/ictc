...
const baseline={durableReplay:true,commandConflict:true,safeUnknownAction:true,aiProposal:true,historicalFailClosed:true,externalIdentity:true},keys=Object.keys(baseline),discovered=new Set(),holdout=new Set();let killed=0,lastNovelAt=-1;
for(let i=0;i<MUTATIONS;i++){const key=keys[(rng()+i)%keys.length],family=detect({...baseline,[key]:false});if(family){killed++;if(i<DISCOVERY){if(!discovered.has(family))lastNovelAt=i;discovered.add(family);}else if(!discovered.has(family))holdout.add(family);}}
assert.equal(killed,MUTATIONS);assert.deepEqual([...discovered].sort(),[...FAMILIES].sort());assert.equal(holdout.size,0);
console.log(JSON.stringify({ok:true,profile:'runtime-stabilization-2.9',simulations:SIMULATIONS,mutations:MUTATIONS,killed,killRate:1,normalizedFailureFamilies:discovered.size,lastNovelAt,holdout:{from:DISCOVERY,to:MUTATIONS-1,newNormalizedFamilies:0},simulationSignature:signature>>>0,limitations:['Deterministic model-level saturation over the declared operators; it is not proof that the failure vocabulary is complete.','Mutation kill rate is bounded repository evidence, not legal/compliance assurance, deployment assurance or a probability of correctness.']}));
