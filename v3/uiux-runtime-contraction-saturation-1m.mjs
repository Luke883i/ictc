...
const baseline=()=>({presentation:{hiddenCompat:0,legacyWrite:0,legacyClass:0},behavior:{unique:true,double:0,intercept:0,order:0},compat:{mounted:0,css:0,listener:0,domWrite:0,duplicate:0,uniqueLeft:0,gateRequires:0,browserRequires:0,currentSelector:0},future:{legacyDependency:0,rootEdit:0,genericAction:true}});
const families=[
 ...
 ['double-listener',1,s=>s.behavior.double++],['duplicate-owner',1,s=>s.behavior.unique=false],['interception',1,s=>s.behavior.intercept++],['mount-order',1,s=>s.behavior.order++],
...
function violations(s,disabled=-1){const out=[];if(disabled!==0&&(s.presentation.hiddenCompat||s.presentation.legacyWrite||s.presentation.legacyClass))out.push(LAWS[0]);if(disabled!==1&&(!s.behavior.unique||s.behavior.double||s.behavior.intercept||s.behavior.order))out.push(LAWS[1]);if(disabled!==2&&(s.compat.mounted||s.compat.css||s.compat.listener||s.compat.domWrite||s.compat.duplicate||s.compat.uniqueLeft||s.compat.gateRequires||s.compat.browserRequires||s.compat.currentSelector))out.push(LAWS[2]);if(disabled!==3&&(s.future.legacyDependency||s.future.rootEdit||!s.future.genericAction))out.push(LAWS[3]);return out;}
