...
const PRODUCT_CI_FAMILIES=Object.freeze([
 ...
 'optional-owner','optional-surface','pre-convergence-read','aggregate-before-leaf','stateful-historical-oracle','duplicate-observational-gate',
]);
const CI_PRODUCT_FAMILIES=Object.freeze([
 ...
 'geometry-in-semantic-oracle','legacy-status-authority','hidden-browser-source','unbounded-browser-parallelism','current-profile-lineage-drift','duplicate-observational-gate',
]);
...
function baseBoundary(){return {
 ...
 boundedParallelism:true,currentProfile:true,duplicateObserverAuthority:false,
};}
...
assert.ok(ci.includes('max-parallel: 4')&&ci.includes('ICTC_STATE_DIR="$RUNNER_TEMP/ictc-browser-${{ matrix.id }}-state"'),'required browser CI must remain bounded and isolated');
assert.ok(!ci.match(/browser-journeys:[\s\S]*?epistemic-professional-browser:/)?.[0].includes('browser-information-value.py'),'information-value must not duplicate required browser authority');
assert.ok(ci.includes('information-value-diagnostic:')&&ci.includes('name: diagnostic / information-value-3-2')&&ci.includes('continue-on-error: true'),'information-value must remain executable as non-authoritative diagnostic evidence');
assert.ok(census.includes('failureRank(run)')&&census.includes('browserSourcePrefix'),'census must retain leaf/source provenance');
assert.ok(census.includes("const diagnosticPrefixes=Object.freeze(['diagnostic / '])")&&census.includes("acceptanceAuthority:'required-checks-only'")&&census.includes("diagnosticPolicy:'reported-not-gating'")&&census.includes("authority(run)==='required'"),'census must report diagnostics without turning them into exact-head acceptance authority');
