...
function estimateUsage(envelope, prompt, inputJson, output, settings) {
  ...
  const inputTokens = Number(provider.prompt_tokens ?? provider.input_tokens ?? Math.ceil((prompt.length + inputJson.length) / 4));
  const outputTokens = Number(provider.completion_tokens ?? provider.output_tokens ?? Math.ceil(canonicalJson(output).length / 4));
  const inputRate = Number(settings.governance?.inputCostPerMillion || 0);
  const outputRate = Number(settings.governance?.outputCostPerMillion || 0);
  return {
    inputTokens,
    outputTokens,
    totalTokens: inputTokens + outputTokens,
    estimatedCostUsd: Number(((inputTokens * inputRate + outputTokens * outputRate) / 1000000).toFixed(6)),
    source: provider.prompt_tokens != null || provider.input_tokens != null ? 'provider' : 'estimated'
  };
}

export async function callJson(settings, purpose, systemPrompt, payload, options = {}) {
  if (stateProvider) assertAiBudget(stateProvider());
  const endpoint = asString(settings.llm.endpoint, 4000);
  const model = asString(settings.llm.model, 500);
  ...
  const inputJson = canonicalJson(payload);
  const body = {
    model,
  ...
  if (!response.ok) throw Object.assign(new Error(`Provider AI: ${response.status}`), { status: 502, code: 'ai-provider-error', details: responseText.slice(0, 1000) });
  let envelope;
...
function jobProfile(mission) {
  return {
    ...
    changeTypes: mission.changeTypes || ['new-law', 'amendment', 'repeal', 'guidance', 'effective-date'],
    resultLimit: Number(mission.resultLimit || 50),
    limitation: 'La novelty identifica candidati rispetto alla baseline dichiarata; non dimostra completezza, applicabilita o vigenza.'
  };
}
...
export function enrichContribution(settings, contribution) {
  return callJson(settings, 'contribution-enrichment', settings.prompts.contributionEnrichment, {
    ...
    contribution: {
      ...
      attachments: contribution.attachments.map(item => ({ name: item.name, mime: item.mime, bytes: item.bytes, sha256: item.sha256 }))
    }
  });
}
...
export function analyzeIncident(settings, incident) {
  return callJson(settings, 'incident-analysis', settings.prompts.incidentAnalysis, {
    ...
    outputContract: {
      ...
      assumptions: ['string'],
      signals: ['ongoing|personal-data|malicious|cross-border|technical-detection'],
      timeline: [{ at: 'ISO date or empty', event: 'string', source: 'user|attachment|inference' }],
