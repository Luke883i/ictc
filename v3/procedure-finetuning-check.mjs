...
const anchors=await read('./public/ui/procedure-control-anchors-1-4.js');
...
assert.equal(contract.commonSubstrate.orientationOrder.join('|'),'object|why-here|epistemic-state|human-decision|evidence|next-action|handoff');
...
for(const token of ['dataset.journeyProcess','dataset.journeyStage','dataset.journeyIntent','dataset.journeyAuthority','dataset.journeyEvidenceEffect']) assert.ok(ui.includes(token)||anchors.includes(token),token);
...
assert.match(anchors,/if\(node\.dataset\.journeyProcess\)continue/);
...
assert.ok(active.indexOf('installProcedureControlAnchors')<active.indexOf('installVisualEpistemicRuntime'),'control anchors must precede final visual normalizer');
...
assert.match(standards,/does not validate|must not be read as authoritative normative wording/i);
console.log(JSON.stringify({ok:true,authority:contract.authority,scope:contract.scope,journeyStages:Object.fromEntries(contract.scope.map(code=>[code,contract.processes[code].journey.length])),natureAttacks:contract.natureFalsification.attacks.length,controlAnchorFields:contract.commonSubstrate.controlAnchor.requiredAttributes,residualControlAnchoring:true}));
