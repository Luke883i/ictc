...
startJourneyShell().catch(error => {
  ...
  if (main) {
    main.innerHTML = `<section class="welcome"><span class="eyebrow">Runtime non disponibile</span><h1>ICTC non può aprire la workspace</h1><p>${String(error.message || error)}</p><button id="retryBoot" type="button">Riprova</button></section>`;
