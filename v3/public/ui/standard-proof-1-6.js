...
const roleIntroductions = {
  admin: 'Vedi insieme cosa governi, quali controlli sono verificati qui e quali prove devono arrivare dal deployment.',
  user: 'Scopri cosa puoi registrare, cosa resta originale e quali decisioni richiedono un ruolo autorizzato.',
  auditor: 'Ricostruisci architettura, autorita, standard, evidenze e limiti in sola lettura.'
};
...
function normalizeStaticLabels() {
  ...
  if (brand) brand.textContent = 'Decisioni, fonti ed evidenze';
  ...
  document.title = 'ICTC 1.6 · Standard proof';
}

function proofMarkup() {
  return `
    <section id="proofView" class="view proof-view" hidden tabindex="-1" aria-labelledby="proofTitle">
      <section class="proof-hero" aria-describedby="proofLead">
        <div class="proof-hero-copy">
          <p class="eyebrow">ICTC 1.6 · Standard proof</p>
          <h1 id="proofTitle">Capire il sistema. Verificare le promesse.</h1>
          <p id="proofLead">Una guida unica per persone operative, amministratori, auditor e tecnici. Ogni affermazione collega pratica, evidenza e limite.</p>
      ...
      <nav class="proof-index" aria-label="Indice della guida ICTC">
        <a href="#proofStart">In breve</a>
        <a href="#proofArchitecture">Architettura</a>
        <a href="#proofJourneys">Flussi</a>
        <a href="#proofStandards">Standard</a>
        <a href="#proofGlossary">Glossario</a>
        <a href="#proofLimits">Limiti</a>
      </nav>
      ...
      <div id="proofContent" hidden>
        <section id="proofStart" class="proof-section" aria-labelledby="proofStartTitle">
          <header><p class="eyebrow">Orientamento</p><h2 id="proofStartTitle">ICTC in tre risposte</h2></header>
          <div id="proofAnswers" class="proof-answer-grid"></div>
        </section>

        <section id="proofArchitecture" class="proof-section" aria-labelledby="proofArchitectureTitle">
          <header><p class="eyebrow">Infrastruttura</p><h2 id="proofArchitectureTitle">Otto strati, una sola fonte di verità</h2><p>La vista spiega il sistema; non replica dati operativi né introduce un secondo runtime.</p></header>
          <ol id="proofStack" class="proof-stack"></ol>
        </section>

        <section id="proofJourneys" class="proof-section" aria-labelledby="proofJourneysTitle">
          <header><p class="eyebrow">Logica</p><h2 id="proofJourneysTitle">Dal dato grezzo alla prova</h2></header>
          <div id="proofJourneyGrid" class="proof-journey-grid"></div>
        </section>

        <section id="proofStandards" class="proof-section" aria-labelledby="proofStandardsTitle">
          <header><p class="eyebrow">Riferimenti</p><h2 id="proofStandardsTitle">Standard e pratiche di ispirazione</h2><p>“Allineato” descrive una pratica osservabile; non significa certificato o legalmente conforme.</p></header>
          <div id="proofStandardList" class="proof-standard-list"></div>
        </section>

        <section id="proofGlossary" class="proof-section" aria-labelledby="proofGlossaryTitle">
          <header><p class="eyebrow">Ontologia</p><h2 id="proofGlossaryTitle">Parole che non sono sinonimi</h2></header>
          <dl id="proofGlossaryList" class="proof-glossary"></dl>
        </section>

        <section id="proofLimits" class="proof-section proof-limits" aria-labelledby="proofLimitsTitle">
          <header><p class="eyebrow">Confine</p><h2 id="proofLimitsTitle">Cosa questa prova non dimostra</h2></header>
          <p id="proofBoundary"></p>
          <div class="proof-limit-grid">
            <div><h3>Evidenze disponibili</h3><ul id="proofEvidenceKinds"></ul></div>
            <div><h3>Attestazioni mancanti</h3><ul id="proofDeploymentGaps"></ul></div>
          </div>
        </section>
      </div>
    </section>`;
...
function adminProofMarkup() {
  return `<section id="adminProofPanel" class="admin-panel admin-proof-panel" aria-labelledby="adminProofTitle">
    <div class="admin-proof-head"><div><p class="eyebrow">Standard proof</p><h3 id="adminProofTitle">Postura e confini</h3><p>Una sintesi di ciò che il runtime dimostra e di ciò che richiede attestazioni esterne.</p></div><button id="openProofFromAdmin" type="button">Apri guida e prova</button></div>
    <div id="adminProofMetrics" class="metric-grid" aria-live="polite"><div class="metric"><b>—</b><small>Caricamento prova</small></div></div>
  </section>`;
}
...
function ensureSurface() {
  ...
  if (nav && !$('#openStandardProof')) {
    nav.insertAdjacentHTML('beforeend', '<button id="openStandardProof" type="button" data-proof-service="proof" aria-current="false">Guida e prova</button>');
  }
  const main = $('#main');
...
function setProofVisibility(visible, focus = true) {
  ...
  $('#openStandardProof')?.setAttribute('aria-current', visible ? 'page' : 'false');
  if (visible) {
...
function navigateToProof({ focus = true } = {}) {
  state.service = 'proof';
  storageSet('ictc-service', 'proof');
  setProofVisibility(true, focus);
  loadProof().catch(() => {});
}
...
function renderProof(data) {
  ...
  $('#proofDeploymentGaps').innerHTML = posture.deployment.blockers.length
    ? posture.deployment.blockers.map(item => `<li><b>${esc(item.label)}</b><span>${esc(item.evidence)}${item.action ? ` · ${esc(item.action)}` : ''}</span></li>`).join('')
    : '<li>Nessun gap di deployment aperto nel profilo corrente.</li>';
...
function renderAdminProof(data) {
  ...
  const posture = data.proof.posture;
  root.innerHTML = `
    <div class="metric"><b>${esc(data.release)}</b><small>Release attiva</small></div>
    <div class="metric"><b>${esc(posture.runtime.verified)}/${esc(posture.runtime.total)}</b><small>Controlli runtime</small></div>
    <div class="metric"><b>${esc(posture.deployment.blockers.length)}</b><small>Gap deployment</small></div>
    <div class="metric"><b>${data.proof.integrity.ok ? 'Coerente' : 'Da verificare'}</b><small>Catena evidenze</small></div>`;
}

async function loadProof(force = false) {
  if (!force && cachedProof && cachedRole === state.role) {
    renderProof(cachedProof);
    return cachedProof;
  }
  ...
  loadingPromise = api('/api/standard-proof').then(data => {
    renderProof(data);
    return data;
  }).catch(error => {
    $('#proofLoading').hidden = true;
    ...
    $('#proofView')?.removeAttribute('aria-busy');
    loadingPromise = null;
  });
...
function bindSurface() {
  ...
  document.addEventListener('click', event => {
    ...
    if (service || homeAction) setProofVisibility(false, false);
  }, true);
}

export function installStandardProof16Experience() {
  installStable14Experience();
  ensureSurface();
  bindSurface();
  document.addEventListener('ictc:rendered', () => {
    normalizeStaticLabels();
    if (state.service === 'proof') {
      setProofVisibility(true, false);
      loadProof(true).catch(() => {});
    } else {
      setProofVisibility(false, false);
    }
  });
}
