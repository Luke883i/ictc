...
export function renderIncidentWorkspace() {
  ...
  const readOnly = (state.data?.actor?.role || state.role) === 'auditor';
  const nextQuestion = question ? (readOnly ? `<section class="question-compass" data-incident-readonly-question="true"><p class="eyebrow">Informazione mancante · sola consultazione</p><h3>${esc(question.label)}</h3>${question.suggestedValue ? `<div class="ai-suggestion"><b>Suggerimento AI non adottato</b><p>${esc(question.suggestedValue)}</p><small>La proposta resta distinta da una risposta umana.</small></div>` : ''}<div class="why-grid"><div><b>Perché serve</b><br>${esc(question.whyNow)}</div><div><b>Come sarebbe usata</b><br>${esc(question.evidenceUse)}</div></div><div class="boundary">Il profilo Auditor può consultare il quesito e la sua base, ma non registrare o correggere la risposta.</div></section>` : `<section class="question-compass"><p class="eyebrow">Domanda successiva · ${incident.questions.length} informazioni mancanti</p><h3>${esc(question.label)}</h3>${question.suggestedValue ? `<div class="ai-suggestion"><b>Suggerimento AI da verificare</b><p>${esc(question.suggestedValue)}</p><small>Viene registrato solo dopo la tua conferma o correzione.</small></div>` : ''}<div class="why-grid"><div><b>Perché serve ora</b><br>${esc(question.whyNow)}</div><div><b>Come sarà usata</b><br>${esc(question.evidenceUse)}</div></div>${answerValue(question)}<div class="question-actions"><button class="unknown" type="button" data-answer-unknown="${esc(question.id)}">Non disponibile</button><button class="primary" type="button" data-answer-question="${esc(question.id)}">Conferma o correggi</button></div></section>`) : '';
  ...
  const review = current ? `<section><p class="eyebrow">Confronto con l’originale</p><h3>Verifica la versione corrente</h3><div class="draft-review"><section><h4>Parole aggiunte</h4><p>${addedDiff(incident.originalNarrative, incident.finalNarrative || current.narrative)}</p></section><section><label>Formulazione finale<textarea id="finalNarrative" rows="12" ${readOnly || ['submitted','closed'].includes(incident.state) ? 'disabled' : ''}>${esc(incident.finalNarrative || current.narrative)}</textarea></label>${!readOnly && !['submitted','closed'].includes(incident.state) ? '<label class="confirm-row"><input id="confirmIncident" type="checkbox"> Confermo di aver verificato fatti, informazioni non disponibili e formulazione corrente.</label>' : ''}</section></div></section>` : !question ? (readOnly ? `<section class="lens-panel" data-incident-readonly-formulation="true"><p class="eyebrow">Formulazione</p><h3>Consultazione della formulazione</h3><p>Il profilo Auditor non crea bozze né modifica la versione corrente.</p></section>` : `<section class="lens-panel"><p class="eyebrow">Formulazione</p><h3>Le informazioni minime sono disponibili</h3><p>Puoi chiedere una bozza all’AI oppure scrivere una versione umana. Entrambe saranno salvate prima dell’invio.</p><label>Versione manuale<textarea id="manualNarrative" rows="8"></textarea></label></section>`) : '';
  ...
  if(readOnly) for(const node of $('#workspaceBody').querySelectorAll('[data-retry-incident],#questionValue,[data-answer-question],[data-answer-unknown]')) node.remove();
  const actions = [evidenceButton(incident.evidenceUrl)];
  if (!readOnly && !question && !current && ['intake','clarifying','review'].includes(incident.state)) {
  ...
  if (!readOnly && current && ['clarifying','review'].includes(incident.state)) {
  ...
  if (!readOnly && incident.state === 'submitted' && state.data.actor.role === 'admin') actions.push(`<label class="closure-inline">Motivo chiusura<input id="closureNote" placeholder="Esito amministrativo e limiti residui"></label><button class="primary" type="button" data-close-incident="${esc(incident.id)}">Chiudi evento</button>`);
