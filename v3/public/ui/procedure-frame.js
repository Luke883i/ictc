...
function procedureLocation(){if(state.service==='monitoring')return'monitoring';if(state.service==='incidents')return'incidents';if(state.service==='grc')return activeGrcId();return null;}
...
function frameModel(id){const meta=contract(id)||{},item=operational(id)||{},attention=Number(item.attentionCount||0),total=totalFromMetrics(item);return{
  ...
  attention,
  total,
  stateLabel:attention>0?'Richiede attenzione':'In ordine',
...
export function renderProcedureExperience(){renderProcedureHub();mountNativeFrame('monitoring','#monitoringView');mountNativeFrame('incidents','#incidentsView');if($('#grcWorkspace'))mountGrcFrame();updateBackLabels();sanitizeEndUserControls();}
function openPrimary(id){if(id==='monitoring'){const button=$('#openContribution')||document.querySelector('[data-open-contribution]');button?.click();return;}if(id==='incidents'){$('#openIncident')?.click();return;}if(id==='coverage'){const panel=document.querySelector('#grcWorkspace .market-mapping-panel');if(panel){panel.open=true;panel.scrollIntoView({block:'nearest',behavior:'smooth'});panel.querySelector('select,input,textarea,button')?.focus();return;}}const disclosure=$('#grcPrimaryForm');if(disclosure){disclosure.open=true;disclosure.scrollIntoView({block:'nearest',behavior:'smooth'});disclosure.querySelector('input,select,textarea,button')?.focus();}}
export function installProcedureFrame(){if(installed)return;installed=true;document.addEventListener('click',event=>{const back=event.target.closest?.('[data-nav-back]');if(back){event.preventDefault();event.stopImmediatePropagation();navigateBack();return;}const primary=event.target.closest?.('[data-procedure-primary]');if(primary){event.preventDefault();event.stopImmediatePropagation();openPrimary(primary.dataset.procedurePrimary);return;}const secondary=event.target.closest?.('[data-procedure-secondary="monitoring-plan"]');if(secondary){event.preventDefault();event.stopImmediatePropagation();const form=$('#missionForm');if(form){form.hidden=false;form.scrollIntoView({block:'nearest',behavior:'smooth'});form.querySelector('textarea,input,select')?.focus();}return;}},true);document.addEventListener('ictc:rendered',()=>queueMicrotask(renderProcedureExperience));document.addEventListener('ictc:surface-changed',()=>queueMicrotask(renderProcedureExperience));renderProcedureExperience();}
