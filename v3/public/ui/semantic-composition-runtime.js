...
import { ENDUSER_SURFACE_GRAMMAR, LOCAL_COMPOSITION_OWNERS, NATIVE_SEMANTIC_LATTICE_VERSION, SURFACE_ARCHETYPE_FAMILY } from './native-semantic-lattice-3-2.js';
...
function annotateSurface(id,blueprint){const root=document.querySelector(blueprint.root);if(!root)return;root.dataset.semanticComposition=SEMANTIC_COMPOSITION_VERSION;root.dataset.nativeSemanticLattice=NATIVE_SEMANTIC_LATTICE_VERSION;root.dataset.surfaceArchetype=SURFACE_ARCHETYPE_FAMILY[id]||SURFACE_ARCHETYPE_FAMILY[blueprint.procedure]||'unclassified';if(ROOT_COUNTS[blueprint.root]===1){root.dataset.compositionSurface=id;root.dataset.surfaceGrammar=ENDUSER_SURFACE_GRAMMAR[id]||'unclassified';}root.dataset.informationRole=DEFAULT_ROLE[blueprint.kind]||'context';root.dataset.localCompositionOwner=LOCAL_COMPOSITION_OWNERS[id]||LOCAL_COMPOSITION_OWNERS[blueprint.procedure]||'native-surface';}
