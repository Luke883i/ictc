...
function canonicalFor(id){return state.data?.procedureRegistry?.procedures?.find(item=>item.id===id)||null;}
function businessProcedureIds(){return new Set((state.data?.procedureRegistry?.procedures||[]).map(item=>item.id));}
...
function renderProcedureHub(){const host=$('#procedureHub');if(!host||!state.data)return;const businessIds=businessProcedureIds(),procedures=(state.data.procedures||[]).filter(item=>businessIds.has(item.id));host.dataset.procedureHub='canonical-business-registry-derived';host.innerHTML=procedures.map(item=>{const canonical=canonicalFor(item.id),label=canonical?.label||item.label,code=canonical?.code||item.code,purpose=canonical?.purpose||item.description||'',claim=canonical?.claimBoundary||item.claimBoundary||'',count=Number(item.attentionCount||0),metrics=(item.metrics||[]).slice(0,4),status=count>0?`${count} da vedere`:'In ordine';let action='';if(item.service==='grc')action=`<button class="primary" type="button" data-service="grc" data-grc-process="${esc(item.id)}">${esc(item.actionLabel||'Apri')}</button>`;else if(['monitoring','incidents'].includes(item.service))action=`<button class="primary" type="button" data-service="${esc(item.service)}">${esc(item.actionLabel||'Apri')}</button>`;return `<article class="stable-process-card" data-procedure-id="${esc(item.id)}" data-process-code="${esc(code)}"><header><div><span class="process-code">${esc(code)}</span><h2>${esc(label)}</h2></div><span class="stable-status ${count>0?'attention':'ready'}">${esc(status)}</span></header>${metrics.length?`<div class="stable-metrics">${metrics.map(m=>`<span><b>${esc(m.value)}</b><small>${esc(m.label)}</small></span>`).join('')}</div>`:''}<footer>${action}<details><summary>Scopo e limiti</summary><p>${esc(purpose)}</p>${claim?`<small>${esc(claim)}</small>`:''}</details></footer></article>`;}).join('')||'<div class="empty">Nessun processo operativo per il profilo corrente.</div>';}
function renderStableChrome(){ensureStableShell();removeHiddenDuplicateEntries();renderProcedureHub();updateStableProfileMenu();}
export function installActiveExperience(){if(installed)return;installed=true;installStableShell();installProofSurface();installTraceExplorer();installGrcWorkspace();installSurfaceRouter();installProcedureMarketUx();installProcedureAnatomy();installBindings();installAdminCenter();installProcedureAdmin();installEnterpriseExperience();installFi01Reference();installGlobalTools();installAiOptionalControls();installProcessHandoffs();installRiskActionFidelity();installStableProfileMenu();updateStableProfileMenu();renderStableChrome();document.addEventListener('ictc:rendered',renderStableChrome);}
