...
['coherence-debt-drop',m=>{delete m.repositoryCoherenceDebt}],
['coherence-class',m=>{m.repositoryCoherenceDebt.classification='generic-debt'}],
['coherence-authority-widen',m=>{m.repositoryCoherenceDebt.createsNewAuthority=true}],
['coherence-finding-drop',m=>{m.repositoryCoherenceDebt.findings=m.repositoryCoherenceDebt.findings.filter(x=>x.id!=='F8')}],
['coherence-f5-internalize',m=>{const x=m.repositoryCoherenceDebt.findings.find(x=>x.id==='F5');x.closureClass='repository-internal'}],
['coherence-f5-unlink',m=>{m.repositoryCoherenceDebt.findings.find(x=>x.id==='F5').linkedGap=null}],
['coherence-f6-pass-launder',m=>{const x=m.repositoryCoherenceDebt.findings.find(x=>x.id==='F6');x.status='passed';x.blocking=true}],
['coherence-f7-runtime-launder',m=>{m.repositoryCoherenceDebt.findings.find(x=>x.id==='F7').limitation='Current runtime defect.'}],
['coherence-f8-consistency-launder',m=>{m.repositoryCoherenceDebt.findings.find(x=>x.id==='F8').limitation='Fresh means semantically consistent.'}],
['coherence-negative-proof',m=>{m.repositoryCoherenceDebt.negativeControls[0].universalAbsenceProof=true}],
['coherence-falsifier-drop',m=>{m.repositoryCoherenceDebt.findings[0].falsifier=''}]
];
