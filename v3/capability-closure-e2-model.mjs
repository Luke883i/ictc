...
export function validateClosureModel(model,ctx={}){
  ...
  if(ctx.authority){ck(ctx.authority?.planningState?.nextSerialSlice==='UIUX-CONVERGE-0'&&ctx.authority?.planningState?.noNewSerialBridge===true,'AUTHORITY_PLAN');const c5=(ctx.authority?.conditionalSlices||[]).find(x=>x.id==='C5-SEMANTIC-OWNER-COMPRESSION');ck(Boolean(c5)&&String(c5.mustResolveBefore).includes('UIUX-CONVERGE-0'),'AUTHORITY_C5');}
