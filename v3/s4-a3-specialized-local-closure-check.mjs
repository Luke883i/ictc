...
assert.ok(proof.includes("proofReadingOrder='facts>decisions>evidence-basis>trace>epistemic>external>integrity>method>export'"),'Proof must publish decision/evidence-first ordering');
assert.ok(proof.includes('initializeDisclosure(detail,detail===decisions)'),'Proof decisions must be the initial open decision surface');
...
console.log(JSON.stringify({ok:true,slice:'S4-A3',authority:'source-contract',checks:['MC primary intent=1 + reference secondary','EP search-first + progressive technical controls','EP selected-focus graph <=24 nodes/48 edges','Proof facts/decisions/evidence before technical reconstruction','Proof domain resolution idempotent after local relabeling','Disclosure state preserved','No new MutationObserver authority'],claimBoundary:'Source-contract evidence only; browser/runtime acceptance remains separate.'}));
