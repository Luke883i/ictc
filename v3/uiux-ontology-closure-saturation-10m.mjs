...
function sourceLaws(s){
  ...
  return {
    ...
    orientationTransient:s.frame.includes('data-orientation-transient="true"')&&s.frame.includes("addEventListener('pointerover'")&&s.frame.includes("addEventListener('focusin'")&&s.frame.includes("event.key!=='Escape'")&&s.frame.includes("(hover: hover) and (pointer: fine)"),
    ...
    proofReadingGuide:s.proof.includes('data-proof-reading-guide')&&s.proof.includes('WORKSPACE_COPY.proof.readingGuide')&&['Cosa osserva','Cosa non prova','Come leggere gli stati','Fonti e provenienza','Quando serve verifica esterna'].every(token=>s.native.includes(token)),
...
const SOURCE_MUTANTS=[
  ...
  ['proof-reading-guide-lost',s=>({...s,proof:s.proof.replace('data-proof-reading-guide','data-proof-reading-guide-retired')})],
