...
import { createControlTestHandler } from './runtime/control-test-handler.mjs';
...
function runtimePosture(){const state=store.snapshot();const base={integrity:store.verifyChain(),safeBinding:true,identityProvider:process.env.ICTC_IDENTITY_MODE==='trusted-header',identityStrategy:state.settings.identity?.strategy||'legacy-role-header',attachmentTrust:attachmentTrustPosture(process.env)};return readinessRuntimeFromDeployment(base,deploymentEvidencePosture(process.env));}
const handlers=[createWorkOrchestration({store,permissions}),createProcessLandscapeHandler({store,permissions}),createManualMonitoringHandler({store,permissions}),createManualAssuranceHandler({store,permissions}),createManualIncidentHandler({store,permissions}),createProcessHandoffHandler({store,permissions}),createRiskActionFidelityHandler({store,permissions}),createInsightRuntime({store,permissions}),createWorkbenchProjection({store,permissions}),createStandardLibraryHandler({store,permissions}),createProcedureInvariantHandler({store,permissions}),createControlTestHandler({store,permissions}),createGrcRuntime({store,permissions}),createMonitoringJobHandler({store,permissions}),createUserMonitoringHandler({store,runMission:monitoring.runMission}),monitoring.handle,createContributionHandler({store,permissions}),createIncidentMarketHandler({store,permissions}),createIncidentHandler({store,permissions}),createEvidenceHandler({store:evidenceStore,permissions}),createAdminHandler({store,permissions,posture:runtimePosture})];
