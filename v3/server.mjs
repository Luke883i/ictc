...
async function handleApi(request,response,url,actor){
  const pathname=url.pathname; const method=request.method||'GET';
  if(method==='GET'&&pathname==='/api/health'){
    json(response,200,{ok:true,service:'ictc',version:VERSION,readiness:'ready',services:['monitoring','incidents','administration'],roles:ROLES,integrity:store.verifyChain(),enterprise:enterpriseReadiness(store.snapshot(),{integrity:store.verifyChain(),safeBinding:true,dependencyAudit:true})});return;
  }
  if(method==='GET'&&pathname==='/api/bootstrap'){json(response,200,visibleState(actor,store,VERSION));return;}
