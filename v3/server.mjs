...
import { demoSuite30Enabled, demoSuite30Projection, ensureDemoSuite30 } from './runtime/demo-suite-3-0.mjs';
...
async function initializeRuntimeStore(store){await store.init();await ensureEnterpriseState(store);await ensurePrivacyState(store);await ensureDemoSuite22(store);await ensureDemoSuite30(store);return store;}
...
function demoPosture(state){const demo=state?.settings?.demoSuite30||demoSuite30Enabled()?demoSuite30Projection(state):demoSuite22Projection(state);return demo.enabled?{...demo,schedulerEnabled:false,schedulerMode:'disabled-in-demo'}:demo;}
...
async function handleApi(request,response,url,actor){
  ...
  if(method==='PATCH'&&pathname==='/api/profile/onboarding'){requirePermission(actor,'read',permissions);const currentUser=(snapshot.users||[]).find(item=>item.id===actor.id),accepted=currentUser?.onboarding?.acceptedAt;if(accepted){json(response,200,{result:{schemaVersion:'1.0.0',authority:'user-record',startedAt:currentUser.onboarding.startedAt||null,progress:100,elapsedMs:Math.max(0,Number(currentUser.onboarding.elapsedMs||0)),acceptedAt:accepted,accepted:true,readOnly:true},receipt:null,replayed:true});return;}const input=await bodyJson(request),progress=Math.max(0,Math.min(100,Number(input.progress||0))),elapsedMs=Math.max(0,Number(input.elapsedMs||0)),accept=input.accept===true,envelope=await store.mutate(actor,'profile.onboarding.updated',{type:'user',id:actor.id},{progress,elapsedMs,accept},draft=>{const user=(draft.users||[]).find(item=>item.id===actor.id);if(!user)throw httpError(409,'Profilo utente non disponibile per registrare onboarding','onboarding-user-missing');const current=user.onboarding||{},nextProgress=Math.max(Number(current.progress||0),progress),nextElapsed=Math.max(Number(current.elapsedMs||0),elapsedMs);if(accept&&nextProgress<100)throw httpError(409,'Completa il percorso prima di accettare','onboarding-incomplete');user.onboarding={schemaVersion:'1.0.0',startedAt:current.startedAt||now(),progress:accept?100:nextProgress,elapsedMs:nextElapsed,acceptedAt:accept?now():null};return{...user.onboarding,authority:'user-record',accepted:Boolean(user.onboarding.acceptedAt),readOnly:Boolean(user.onboarding.acceptedAt)};},commandFrom(request));json(response,200,envelope);return;}
...
async function schedulerTickCurrentTenant(){
  const before=store.snapshot();if(demoPosture(before).enabled)return;const tickAt=now();try{await sweepClockReviewNeeds({store,actor:{id:'system:review-clock',role:'system'},validAsOf:tickAt,command:{id:`review-clock-${tickAt}`}});}catch(error){observability.log('review_clock_error',{tenantId:tenantAuthority.current().tenantId,code:error.code||'error'});}const state=store.snapshot();if(!procedureEnabled(state,'monitoring'))return;const due=state.missions.filter(item=>item.state==='active'&&item.nextRunAt&&new Date(item.nextRunAt)<=new Date());for(const mission of due){const workKey=`monitoring:${mission.id}`;let claim;try{claim=store.persistence.claimSchedulerWork({workKey,dueAt:mission.nextRunAt,ownerId:schedulerOwnerId,leaseMs:schedulerLeaseMs,at:tickAt});if(!claim.claimed){observability.log('scheduler_claim_held',{tenantId:tenantAuthority.current().tenantId,missionId:mission.id,workKey,fence:claim.fence});continue;}await store.withSchedulerClaim(claim,()=>monitoring.runMission(mission.id,{id:'scheduler',role:'admin',identityMode:'system',identityStrategy:'system',permissions:[...permissions.admin]},{id:`scheduler-${mission.id}-${mission.nextRunAt}`}));}catch(error){observability.log('scheduler_error',{tenantId:tenantAuthority.current().tenantId,missionId:mission.id,code:error.code||'error'});}finally{if(claim?.claimed)try{store.persistence.releaseSchedulerWork(claim,{at:now()});}catch(error){observability.log('scheduler_release_error',{tenantId:tenantAuthority.current().tenantId,missionId:mission.id,code:error.code||'error'});}}}
}
async function schedulerTick(){if(demoSuite22Enabled()||demoSuite30Enabled())return;for(const tenantId of tenantAuthority.tenantIds())await tenantAuthority.runTenant(tenantId,schedulerTickCurrentTenant);}
