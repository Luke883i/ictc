...
const handlers = [
  createWorkbenchProjection({ store, permissions }),
  createMonitoringJobHandler({ store, permissions }),
  monitoring.handle,
...
function proofFor(actor) {
  ...
  return standardProofProjection({ actor, version: VERSION, readiness: enterpriseReadiness(store.snapshot(), posture), integrity: posture.integrity });
}
...
async function handleApi(request, response, url, actor) {
  ...
  if (method === 'GET' && pathname === '/api/health') {
    ...
    json(response, 200, {
      ...
      service: 'ictc',
      version: VERSION,
      readiness: enterprise.level,
      services: ['monitoring', 'incidents'],
      supportSurfaces: ['standard-proof'],
      controlPlane: 'administration',
      roles: ROLES,
      integrity: store.verifyChain(),
      enterprise
    });
  ...
  if (method === 'GET' && pathname === '/api/bootstrap') {
    ...
    projected.experience.release = '1.8-enterprise-workbench';
  ...
  if (method === 'GET' && pathname === '/api/standard-proof') {
    ...
    json(response, 200, proofFor(actor));
    return;
  }
...
const server = http.createServer(async (request, response) => {
  try {
    ...
    const actor = authorizeEnterpriseActor(actorFrom(request, permissions), store.snapshot());
...
async function schedulerTick() {
  ...
  for (const mission of due) {
    ...
    } catch (error) { console.error('scheduler', mission.id, error.message); }
  }
}
