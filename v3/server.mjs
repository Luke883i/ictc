...
function runtimePosture(){
  ...
  const base={integrity:store.verifyChain(),safeBinding:true,identityProvider:process.env.ICTC_IDENTITY_MODE==='trusted-header',identityStrategy:state.settings.identity?.strategy||'legacy-role-header'};
  return readinessRuntimeFromDeployment(base,deploymentEvidencePosture(process.env));
}
...
async function handleApi(request,response,url,actor){
  ...
  if(method==='GET'&&pathname==='/api/health'){
    const snapshot=store.snapshot(),enterprise=enterpriseReadiness(snapshot,runtimePosture());
    json(response,200,{ok:true,service:'ictc',version:VERSION,product:PRODUCT_NAME,productEdition:PRODUCT_EDITION,readiness:enterprise.level,services:['monitoring','incidents','objects','coverage','actions','risks','assurance'],supportSurfaces:['standard-proof'],controlPlane:'administration',roles:ROLES,integrity:store.verifyChain(),aiPolicy:aiPolicyProjection(snapshot),enterprise});return;
  }
