...
import { ensureDemoSeed, demoSeedProjection } from './runtime/demo-seed.mjs';
import { ensureDemoRealityContext, demoRealityProjection } from './runtime/demo-reality-context.mjs';
...
async function initializeRuntimeStore(store){await store.init();await ensureEnterpriseState(store);await ensurePrivacyState(store);await ensureDemoSeed(store);await ensureDemoRealityContext(store);return store;}
...
function demoPosture(state){const base=demoSeedProjection(state),reality=demoRealityProjection(state);return base.enabled?{...base,reality,schedulerEnabled:false,schedulerMode:'disabled-in-demo'}:base;}
...
async function schedulerTickCurrentTenant(){
  const tickAt=now();try{await sweepClockReviewNeeds({store,actor:{id:'system:review-clock',role:'system'},validAsOf:tickAt,command:{id:`review-clock-${tickAt}`}});}catch(error){observability.log('review_clock_error',{tenantId:tenantAuthority.current().tenantId,code:error.code||'error'});}const state=store.snapshot();if(!procedureEnabled(state,'monitoring'))return;const due=state.missions.filter(item=>item.state==='active'&&item.nextRunAt&&new Date(item.nextRunAt)<=new Date());for(const mission of due){try{await monitoring.runMission(mission.id,{id:'scheduler',role:'admin',identityMode:'system',identityStrategy:'system',permissions:[...permissions.admin]},{id:`scheduler-${mission.id}-${mission.nextRunAt}`});}catch(error){observability.log('scheduler_error',{tenantId:tenantAuthority.current().tenantId,missionId:mission.id,code:error.code||'error'});}}
}
async function schedulerTick(){if(process.env.ICTC_DEMO_SEED==='1')return;for(const tenantId of tenantAuthority.tenantIds())await tenantAuthority.runTenant(tenantId,schedulerTickCurrentTenant);}
