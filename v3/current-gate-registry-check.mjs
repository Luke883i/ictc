...
assert.deepEqual(CURRENT_BASE_GATE_REGISTRY.map(entry=>entry.gate),[...CURRENT_SEMANTIC],'registry must preserve canonical semantic execution order');
...
const activeOrdered=CURRENT_BASE_GATE_REGISTRY.filter(entry=>entry.class!=='replaced').map(entry=>entry.gate);
const expectedActiveOrdered=CURRENT_SEMANTIC.filter(gate=>!LEGACY_DEMO_GATES.includes(gate));
assert.deepEqual(activeOrdered,expectedActiveOrdered,'classification must not reorder active semantic gates');
const activeSet=[...CURRENT_REQUIRED_BASE,...COMPATIBILITY_REGRESSION_BASE];
assert.equal(new Set(activeSet).size,activeSet.length,'active base classes must not overlap');
assert.deepEqual([...activeOrdered].sort(),[...activeSet].sort(),'ordered active rail must contain exactly the two blocking active classes');
