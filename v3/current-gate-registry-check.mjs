...
import { POLICY_GATES,LEGACY_DEMO_GATES,DEMO_SUITE_GATES,NATIVE_GATES,CURRENT_CONTRACT_GATES,CURRENT_BASE_GATE_REGISTRY,CURRENT_REQUIRED_BASE,COMPATIBILITY_REGRESSION_BASE,REPLACED_BASE,CURRENT_AUTHORITY_VECTOR } from './current-gate-registry.mjs';
...
for(const gate of ['v3/refined-product-check.mjs','v3/procedure-journey-2-1-check.mjs','v3/semantic-closure-2-8-causality-check.mjs'])assert.ok(COMPATIBILITY_REGRESSION_BASE.includes(gate),`known lineage gate must be compatibility-regression: ${gate}`);
...
assert.deepEqual(Object.keys(CURRENT_AUTHORITY_VECTOR),['product','semantic','experience','epistemic','uiComposition','workspaceChrome','uiPresentation','demoProjection','journey','constitution','documentation']);
console.log(JSON.stringify({ok:true,suite:'current-gate-registry',base:{total:CURRENT_SEMANTIC.length,currentRequired:CURRENT_REQUIRED_BASE.length,compatibilityRegression:COMPATIBILITY_REGRESSION_BASE.length,replaced:REPLACED_BASE.length},currentContractOverrides:CURRENT_CONTRACT_GATES.length,policy:POLICY_GATES.length,demoSuite:DEMO_SUITE_GATES.length,native:NATIVE_GATES.length,currentAuthorityVector:CURRENT_AUTHORITY_VECTOR,claimBoundary:'Classification follows current declared contracts before filename/version heuristics. Compatibility-regression gates remain blocking until a later responsibility-coverage contraction proves safe demotion.'}));
