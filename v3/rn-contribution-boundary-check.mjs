...
assert.deepEqual(accepted.map(inferRnSourceClass),RN_SOURCE_CLASSES);for(const [index,item] of accepted.entries()){const normalized=normalizeRnDiscoveredItem(item,{sourceClasses:null});assert.ok(normalized);assert.ok(normalized.rnClassificationEvidence.length);const opts=index===3?{privacyReviewed:true}:undefined,basis=assertRnVerifiableSource(item,opts);assert.equal(basis.verifiable,true);assert.ok(basis.reference);assert.ok(basis.classificationEvidence.length);}
assert.throws(()=>assertRnVerifiableSource(accepted[3]),e=>e.code==='rn-jurisprudence-privacy-review-required');
...
assert.match(monitoring,/sourceClasses:\s*assertRnClosedUniverse/);assert.match(monitoring,/assertRnVerifiableSource\(source,\s*\{\s*privacyReviewed\s*\}\)/);assert.match(monitoring,/current\.rnVerification/);assert.match(monitoring,/mission\.sourceClasses\s*=\s*assertRnClosedUniverse/);
console.log(JSON.stringify({ok:true,control:'RN-CONTRIBUTOR-BOUNDARY',writePaths:['scheduled-ai','human-contribution+ai-enrichment','human-manual-observation'],closedWorld:true,persistedClosedUniverse:true,verificationProvenanceRequired:true,classificationEvidenceIndependent:true,mislabeledSourceRejected:true,jurisprudencePrivacyReviewRequired:true,sourceClasses:RN_SOURCE_CLASSES,outOfNatureExcluded:true}));
