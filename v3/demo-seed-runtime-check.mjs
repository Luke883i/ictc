...
import { YEAR_ONE_EXPECTED_COUNTS } from './runtime/demo-operating-year.mjs';
...
const EXPECTED=YEAR_ONE_EXPECTED_COUNTS;
const VISIBLE_PRIMARY=Object.values(EXPECTED).reduce((sum,value)=>sum+value,0);
...
function assertDemoCounts(boot,grc){
  ...
  assert.equal(boot.experience.demo.reality.showcaseRecords,VISIBLE_PRIMARY,'showcase-total-marker');
  ...
  assert.ok(boot.subjectVersions.totalCount>=VISIBLE_PRIMARY*2,`visible-semantic-successors:${boot.subjectVersions.totalCount}:${VISIBLE_PRIMARY}`);
