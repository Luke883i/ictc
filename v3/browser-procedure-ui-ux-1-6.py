...
try:
    with sync_playwright() as pw:
        ...
        PHASE='proof-to-ep-navigation';before=experience_cycle(page);proof_entry=page.locator('.service-nav [data-service="proof"]');expect(proof_entry).to_be_visible();proof_entry.click();wait_view_owner(page,'#proofView','proof-workspace-3-2.js',before);expect(page.locator('#proofTitle')).to_have_text('Evidenze ICTC');expect(page.locator('#epistemicMetaCard')).to_have_count(0);before=experience_cycle(page);ep_entry=page.locator('.service-nav [data-service="epistemic"]');expect(ep_entry).to_be_visible();ep_entry.click();wait_view_owner(page,'#epistemicView','epistemic-workspace-3-2.js',before);expect(page.locator('#epistemicTitle')).to_have_text('Relazioni tra decisioni, fonti ed evidenze');assert '2970 atomi nella pagina' not in page.locator('#epistemicView').inner_text();no_overflow(page)
        ...
        report={'ok':True,'profile':'native-semantic-lattice-3.2-context-tolerant-ui-oracle','procedures':['RN-01','EC-01','AO-01','MC-01','AP-01'],'readOnly':True,'fixtureAuthority':'none','emptyAndPopulatedStateTolerant':True,'semanticReadiness':'monotonic-final-c01-cycle+declared-owner+observable-projection','primaryActionMax':1,'visibleSupportActionsMax':3,'sharedRecordPrimitive':'procedure-record-card','touchTargetsMinPx':44,'mobileOverflow':False,'proofEpistemicNavigation':'canonical-service-nav','legacyEpistemicMetaCard':False,'writeCount':len(writes),'claimBoundary':'Rendered server-backed UI invariant check; stateful creation and transition coverage belongs to dedicated journey/runtime tests.'}
