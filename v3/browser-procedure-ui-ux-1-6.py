...
PROCESS_READY={
    ...
    'MC-01':"()=>{const r=document.querySelector('#grcWorkspace'),s=r?.querySelector('.grc-kpis[data-editorial-support=\"process-status\"]'),f=r?.querySelector('[data-grc-form=\"mapping\"]'),d=r?.querySelector('.procedure-decision-frame .composition-process-context');return !!(s&&f&&d&&r.dataset.editorialOrderValid==='true');}",
...
try:
    with sync_playwright() as pw:
        ...
        PHASE='MC-navigation';open_process(page,'MC-01');workspace=page.locator('#grcWorkspace');status_context=workspace.locator('.grc-kpis[data-editorial-support="process-status"]')
        ...
        PHASE='MC-status-context-stability';stable=page.evaluate("""async()=>{const before=window.__mcStatusNode;document.dispatchEvent(new CustomEvent('ictc:context-changed',{detail:{surface:'grc',procedureId:'coverage',reason:'browser-owner-stability'}}));await Promise.resolve();await Promise.resolve();return before===document.querySelector('#grcWorkspace .grc-kpis[data-editorial-support="process-status"]')&&document.querySelector('#grcWorkspace')?.dataset.editorialOrderValid==='true';}""");assert stable
        PHASE='MC-kpis';labels=[x.strip() for x in status_context.locator('.grc-kpi small').all_inner_texts()];assert labels[:4]==['Decisioni registrate','Gap','Da decidere','Fuori perimetro'],labels
