...
try:
    with sync_playwright() as pw:
        ...
        PHASE='catalogue';page.goto(BASE+'/?view=processes',wait_until='domcontentloaded');wait_process_catalogue(page);expect(page.locator('#procedureHub .procedure-card')).to_have_count(7);meta=page.locator('#epistemicMetaCard');expect(meta).not_to_be_visible();assert meta.evaluate('e=>e.parentElement?.id')=='proofView';no_overflow(page)
        ...
        PHASE='EP-evidence-entry';before=experience_cycle(page);page.locator('.service-nav [data-service="processes"]').click();wait_experience_cycle(page,before);wait_process_catalogue(page);expect(meta).not_to_be_visible();before=experience_cycle(page);page.locator('.service-nav [data-service="proof"]').click();wait_view_owner(page,'#proofView','proof-workspace-3-2.js',before);expect(meta).to_be_visible();entry=meta.locator('[data-service="epistemic"]');expect(entry).to_be_visible();before=experience_cycle(page);entry.click();wait_view_owner(page,'#epistemicView','epistemic-workspace-3-2.js',before);expect(page.locator('#epistemicTitle')).to_have_text('Relazioni tra decisioni, fonti ed evidenze');assert '2970 atomi nella pagina' not in page.locator('#epistemicView').inner_text();no_overflow(page)
        ...
        report={'ok':True,'profile':'native-semantic-lattice-3.2-context-tolerant-ui-oracle','procedures':['RN-01','EC-01','AO-01','MC-01','AP-01'],'readOnly':True,'fixtureAuthority':'none','emptyAndPopulatedStateTolerant':True,'semanticReadiness':'monotonic-final-c01-cycle+declared-owner+observable-projection','primaryActionMax':1,'visibleSupportActionsMax':3,'sharedRecordPrimitive':'procedure-record-card','touchTargetsMinPx':44,'mobileOverflow':False,'writeCount':len(writes),'claimBoundary':'Rendered server-backed UI invariant check; stateful creation and transition coverage belongs to dedicated journey/runtime tests.'}
