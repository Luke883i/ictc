...
assert.ok(tokens.includes('--chrome-footer-mid:')&&tokens.includes('--landing-start:')&&tokens.includes('--home-action-start:')&&tokens.includes('--ui-row-compact:52px'));
assert.ok(shell.includes("UI_PRESENTATION_MODE='local-owners'")&&shell.includes("uiIcon('arrow-up-right'")&&!shell.includes('UI_FINE_TUNING_VERSION'));
...
assert.ok(enterprise.includes('#homeView .home-hero')&&enterprise.includes('.home-priority-main{display:grid;grid-template-columns:4.2rem'));
assert.ok(chrome.includes('.stable-header')&&chrome.includes('.stable-legal-footer'));
assert.ok(closure.includes('#processesView .processes-head')&&closure.includes('#procedureHub{display:grid!important;grid-template-columns:1fr!important')&&closure.includes('#proofView .proof-head'));
assert.ok(closure.includes('#procedureHub .procedure-card[data-uiux-layout="row"]')&&!closure.includes('repeat(3,minmax(0,1fr))')&&!closure.includes('min-height:226px'));
...
const TASKS=Object.freeze({
 ...
 home:{baseline:{surface:1,rowBorders:1,doubleBorders:0,actionStyle:1,icon:1,inlineAction:1,rowMin:52,semanticOrder:1,overflow:0,mobile:1,owner:1,maxPriority:3},levels:['data','owner','component','separator','action','icon','responsive','accessibility']},
 ...
 processes:{baseline:{rowList:1,singleColumn:1,canonicalChildren:4,localNoise:0,ctaAligned:1,rowMax:64,mobileSemanticWrap:1,overflow:0,owner:1,lateOverride:0},levels:['registry','owner','cascade','row','content','action','responsive','overflow']}
});
const validators={
 ...
 home:s=>s.surface===1&&s.rowBorders===1&&s.doubleBorders===0&&s.actionStyle===1&&s.icon===1&&s.inlineAction===1&&s.rowMin>=44&&s.semanticOrder===1&&s.overflow===0&&s.mobile===1&&s.owner===1&&s.maxPriority<=3,
 ...
 processes:s=>s.rowList===1&&s.singleColumn===1&&s.canonicalChildren===4&&s.localNoise===0&&s.ctaAligned===1&&s.rowMax<=64&&s.mobileSemanticWrap===1&&s.overflow===0&&s.owner===1&&s.lateOverride===0
};
const mutators={
 ...
 home:[s=>s.surface=0,s=>s.rowBorders=0,s=>s.doubleBorders=1,s=>s.actionStyle=0,s=>s.icon=0,s=>s.inlineAction=0,s=>s.rowMin=32,s=>s.semanticOrder=0,s=>s.overflow=1,s=>s.mobile=0,s=>s.owner=0,s=>s.maxPriority=5],
 ...
 processes:[s=>s.rowList=0,s=>s.singleColumn=0,s=>s.canonicalChildren=6,s=>s.localNoise=1,s=>s.ctaAligned=0,s=>s.rowMax=96,s=>s.mobileSemanticWrap=0,s=>s.overflow=1,s=>s.owner=0,s=>s.lateOverride=1]
};
