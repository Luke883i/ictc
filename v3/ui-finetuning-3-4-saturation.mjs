...
assert.ok(tokens.includes('--chrome-footer-mid:')&&css.includes('data-ui-fine-tuning="3.4"'));
assert.ok(shell.includes('home-priority-table')&&!shell.includes('<small>Candidate</small>'));
assert.ok(native.includes('/workspace-finetuning-3-4.css'));
assert.ok(proof.includes('retireDuplicateInvestigation'));
const TASKS=Object.freeze({
 chrome:{baseline:{inlineIcon:1,rasterMount:0,headerGradient:3,footerGradient:3,footerAligned:1,candidate:0,contrast:1,mobile:1},levels:['asset','markup','token','cascade','geometry','copy','accessibility','responsive']},
 home:{baseline:{surface:1,rowBorders:1,doubleBorders:0,actionStyle:1,rowMin:52,semanticOrder:1,overflow:0,mobile:1},levels:['data','markup','component','separator','action','hierarchy','responsive','accessibility']},
...
const validators={
 chrome:s=>s.inlineIcon===1&&s.rasterMount===0&&s.headerGradient>=3&&s.footerGradient>=3&&s.footerAligned===1&&s.candidate===0&&s.contrast===1&&s.mobile===1,
 home:s=>s.surface===1&&s.rowBorders===1&&s.doubleBorders===0&&s.actionStyle===1&&s.rowMin>=44&&s.semanticOrder===1&&s.overflow===0&&s.mobile===1,
...
const mutators={
 chrome:[s=>s.inlineIcon=0,s=>s.rasterMount=1,s=>s.headerGradient=1,s=>s.footerGradient=1,s=>s.footerAligned=0,s=>s.candidate=1,s=>s.contrast=0,s=>s.mobile=0,s=>s.headerGradient=0,s=>s.footerGradient=0],
 home:[s=>s.surface=0,s=>s.rowBorders=0,s=>s.doubleBorders=1,s=>s.actionStyle=0,s=>s.rowMin=32,s=>s.semanticOrder=0,s=>s.overflow=1,s=>s.mobile=0,s=>s.doubleBorders=2,s=>s.rowMin=40],
...
let rng=0x34f17e2d>>>0;const next=()=>{rng^=rng<<13;rng^=rng>>>17;rng^=rng<<5;return rng>>>0;};
...
console.log(JSON.stringify({ok:true,suite:'ui-finetuning-3.4-saturation',generated:totalExpected,killed:totalKilled,survived:0,perTask:PER_TASK,tasks:results,abstractionLevelsPerTask:8,claimBoundary:'Four deterministic one-million model-mutation campaigns tied to source-verified UI invariants; not four million browser sessions or human preference studies.'}));
