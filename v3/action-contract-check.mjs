...
import { permissionForTask } from './public/js/journey-guidance.js';
...
for (const kind of ['source-review','finding-review','job-schedule','change-decision','control-map','matter-owner','matter-transition']) {
  assert.equal(permissionForTask({ action: { kind } }, publicContract), publicContract.actions.find(item => item.id === kind).permission);
}
