...
import { assertRelation, compileProcessRegistry, processDefinitions, processKernelProjection, relationDefinition, validateProcessKernel } from './runtime/process-kernel.mjs';
...
assert.equal(server.includes("./runtime/workbench.mjs"), false, 'legacy workbench graph must not be on the active server path');
console.log(`process-kernel-check: ok (processes=${validation.processCount}, relations=${validation.relationCount}, syntheticCoreEdits=${compiled.expectedCoreEdits})`);
