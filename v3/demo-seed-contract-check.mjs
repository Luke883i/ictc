...
assert.ok(server.includes('projected.experience.demoMode')&&server.includes('demoPosture(snapshot)'),'server must project explicit demo mode and bounded posture');
assert.ok(server.includes("if(process.env.ICTC_DEMO_SEED==='1')return"),'demo mode must suppress autonomous scheduler activity');
for(const token of ['ictcDemoBanner','experience?.demo','DEMO · dati sintetici','scheduler operativo disabilitato','conclusione reale di compliance'])assert.ok(shell.includes(token),`demo shell disclosure missing ${token}`);
