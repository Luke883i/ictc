...
export const COMPATIBILITY_ACTION_SEMANTICS=Object.freeze({
  ...
  'catalog.source.impact.assessed':{families:['decided']},
  ...
  'review.need.resolved':{families:['decided']},
  'insight.human.validated':{families:['decided']}
});
...
export function buildEpistemicStep(event,subjectVersions=[],reviewNeeds=[]){const effects=(subjectVersions||[]).map((version,ordinal)=>{const declared=explicitEffectFor(event,version,ordinal),legacy=legacySemanticsForAction(event?.action),producer=structuredClone(declared?.producerRef||declared?.authorityRef||legacy.producerRef||defaultProducer(event)),families=Array.isArray(declared?.families)&&declared.families.length?[...new Set(declared.families.map(String))]:legacy.families,basisRefs=Array.isArray(declared?.basisRefs)&&declared.basisRefs.length?structuredClone(declared.basisRefs):defaultBasisRefs(event,version,families);return{ordinal:ordinal+1,kind:String(declared?.kind||'subject-versioned'),families,subjectVersionId:version.id,subject:structuredClone(version.subject),payloadSha256:version.payloadSha256,authorityRef:producer,producerRef:producer,basisRefs,relation:declared?.relation?structuredClone(declared.relation):null,classificationSource:declared?'declared-effect':COMPATIBILITY_ACTION_SEMANTICS[String(event?.action)]?'compatibility-action-registry':'safe-action-fallback'};});for(const review of reviewNeeds||[])effects.push({ordinal:effects.length+1,kind:'review-needed',families:['derived'],reviewNeedRef:review.id,subject:structuredClone(review.subject),causeDigests:structuredClone(review.causeDigests||[]),authorityRef:{type:'service',id:'ictc-review-policy'},producerRef:{type:'service',id:'ictc-review-policy'},basisRefs:structuredClone(review.causes||[]),relation:{predicate:'requires-review-of',object:structuredClone(review.subject)},classificationSource:'review-policy'});if(!effects.length){const legacy=legacySemanticsForAction(event?.action),producer=structuredClone(legacy.producerRef||defaultProducer(event)),families=event?.action==='integrity.state-bound'?['attested']:legacy.families;effects.push({ordinal:1,kind:event?.action==='integrity.state-bound'?'policy-recorded':'observation-recorded',families,subject:structuredClone(event?.subject||null),authorityRef:producer,producerRef:producer,basisRefs:defaultBasisRefs(event,null,families),relation:null,classificationSource:COMPATIBILITY_ACTION_SEMANTICS[String(event?.action)]?'compatibility-action-registry':'safe-action-fallback'});}const initiator=actorAuthority(event?.actorId,event?.role),step={schemaVersion:'1.4.0',eventId:event.id,revision:event.revision,transactionAt:event.at,semanticManifestSha256:event?.metadata?.semanticManifestSha256||null,procedureContext:{primaryProcedureId:procedureIdForSubject(event?.subject?.type)||null,procedurePolicyVersion:event?.metadata?.procedurePolicyVersion||null,accessPolicyVersion:event?.metadata?.accessPolicyVersion||null},execution:{initiatorRef:initiator,executorRef:{type:'service',id:'ictc-runtime'},originKind:event?.role==='service'||event?.role==='system'?'system-execution':'human-command'},command:{inputSha256:event.inputSha256||null},effects,correlationId:event?.metadata?.correlationId||null,causationEventId:event?.metadata?.causationEventId||null,limitations:['Epistemic effects describe recorded ICTC operations and authority; they do not upgrade observations, AI proposals or evidence into substantive compliance conclusions.','Compatibility action semantics are exact and bounded. Unknown action names can never promote a record to decided or attested authority; only explicit epistemicEffects or an enumerated compatibility action can do so.','AI-looking compatibility names may still fail safe to proposed, never to human authority. The forward contract remains explicit metadata.epistemicEffects.']};step.stepSha256=sha256(step);return step;}
