...
export const TENANT_ROUTE_CONTEXT=Symbol.for('ictc.tenant.route-context');
...
export function resolveRequestTenant(request,env=process.env,directory=tenantDirectory(env)){
  if(!directory.enabled)return{tenantId:directory.defaultTenantId,routing:'single-tenant',routingIdentity:null};
  ...
  if(mode==='local'){
    ...
    if(requested&&env.ICTC_ALLOW_LOCAL_TENANT_SWITCH==='1'){
      ...
      return{tenantId:tenant.id,routing:'local-explicit-dev',routingIdentity:null};
    }
    ...
    return{tenantId:tenant.id,routing:'local-default',routingIdentity:null};
  }
  ...
  const subject=subjectValue(request?.headers||{},env),groups=groupValues(request?.headers||{},env),routingIdentity={subject:subject||null,groups:[...groups]};if(!subject&&!groups.size)fail(401,'tenant-identity-required','Identità necessaria per risolvere il tenant');
  ...
  const requested=text(request?.headers?.['x-ictc-tenant'],63).toLowerCase();
  if(requested){const selected=matches.find(item=>item.id===requested);if(!selected)fail(403,'tenant-membership-required','L’identità non appartiene al tenant richiesto');return{tenantId:selected.id,routing:'trusted-membership-explicit',routingIdentity};}
  if(matches.length===1)return{tenantId:matches[0].id,routing:'trusted-membership',routingIdentity};
...
export async function createTenantAuthority({runtimeRoot,createStore,initializeStore=async store=>store,env=process.env,maxOpenStores=Number(env.ICTC_MAX_OPEN_TENANT_STORES||32)}){
  ...
  async function runRequest(request,fn){const resolved=resolveRequestTenant(request,env,directory),tenant=tenantById(resolved.tenantId),routeContext={schemaVersion:'1.0.0',enforceMembership:directory.enabled&&env.ICTC_IDENTITY_MODE==='trusted-header',tenant:{id:tenant.id,subjects:[...tenant.subjects],groups:[...tenant.groups]},routingIdentity:resolved.routingIdentity};request[TENANT_ROUTE_CONTEXT]=routeContext;try{return await runTenant(resolved.tenantId,()=>fn({...resolved,tenant}));}finally{delete request[TENANT_ROUTE_CONTEXT];}}
