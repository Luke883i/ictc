...
function enumValue(value, allowed, fallback) {
  const normalized = asString(value, 80).toLowerCase();
...
function isoDate(value) {
  ...
  if (Number.isNaN(date.valueOf())) throw httpError(400, 'Data baseline non valida', 'invalid-baseline-date');
...
export function monitoringJobCandidate(input = {}, previous = {}) {
  ...
  const changeTypes = uniqueStrings(input.changeTypes ?? previous.changeTypes)
    .map(value => value.toLowerCase())
    .filter(value => CHANGE_TYPES.has(value));
  return {
...
function commandPart(command, suffix) {
  return command.id ? { ...command, id: `${command.id}-${suffix}`, expectedRevision: null } : {};
}

async function planJob(store, missionId, actor, command) {
  const mission = findMission(store.snapshot(), missionId);
  const ai = await createMonitoringPlan(store.snapshot().settings, mission);
  return store.mutate(actor, 'monitoring.job.planned', { type: 'mission', id: missionId }, { trace: ai.trace }, draft => {
    ...
    return current;
  }, commandPart(command, 'plan'));
}

async function markPlanFailure(store, missionId, actor, error, command) {
  return store.mutate(actor, 'monitoring.job.plan.deferred', { type: 'mission', id: missionId }, { error: error.message }, draft => {
    const current = findMission(draft, missionId);
    current.state = 'needs-plan';
    current.aiError = error.message;
    current.updatedAt = now();
    return current;
  }, commandPart(command, 'plan-deferred'));
}
...
export function createMonitoringJobHandler({ store, permissions }) {
  return async function handleMonitoringJobs(request, response, pathname, actor) {
    ...
    if (method === 'POST' && pathname === '/api/monitoring-jobs/draft') {
      ...
      const candidate = monitoringJobCandidate(input);
      ...
      const raw = await store.mutate(actor, 'monitoring.job.intent.recorded', { type: 'mission', id: missionId }, candidate, draft => {
        const mission = {
          id: missionId,
          ...candidate,
          plan: null,
      ...
      let planning = null;
      let warning = null;
      try { planning = await planJob(store, missionId, actor, command); }
      catch (error) { warning = error.message; await markPlanFailure(store, missionId, actor, error, command); }
      json(response, 201, { raw, planning, warning, mission: findMission(store.snapshot(), missionId) });
      return true;
    }
    ...
    if (method === 'PUT' && params) {
      ...
      const input = await bodyJson(request);
      const command = commandFrom(request);
      const before = findMission(store.snapshot(), params.id);
      ...
      const candidate = monitoringJobCandidate(input, before);
      if (!candidate.objective) throw httpError(400, 'Descrivi il risultato atteso dal job', 'objective-required');
      if (candidate.noveltyBaseline === 'fixed-date' && !candidate.baselineAt) throw httpError(400, 'Indica la data della baseline', 'baseline-required');
      const updated = await store.mutate(actor, 'monitoring.job.profile.updated', { type: 'mission', id: params.id }, candidate, draft => {
      ...
      let planning = null;
      let warning = null;
      try { planning = await planJob(store, params.id, actor, command); }
      catch (error) { warning = error.message; await markPlanFailure(store, params.id, actor, error, command); }
      json(response, 200, { updated, planning, warning, mission: findMission(store.snapshot(), params.id) });
      return true;
    }
    return false;
  };
