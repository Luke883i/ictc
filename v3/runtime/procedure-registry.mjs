...
export function canonicalProcedureRegistry(){return{schemaVersion:contract.schemaVersion,authority:contract.authority,releaseProfile:contract.releaseProfile,commonSubstrate:structuredClone(contract.commonSubstrate),procedures:procedures.map(structuredClone)};}
...
export function procedureMetricSpecs(id){return procedureContract(id).metricSpecs.map(structuredClone);}export function procedureIds(){return procedures.map(item=>item.id);}
