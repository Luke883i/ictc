...
export function buildClosedDemoEvidenceLattice30(state={},options={}){
  ...
  const actor={id:'local-admin',displayName:'Amministratore locale',role:'admin',identityMode:'local'},epistemic=canonicalEpistemicProjection(state,actor),families=new Set(epistemic.records.flatMap(record=>record.epistemicFamilies||[])),supportedFamilies=new Set(epistemic.metaGrammar?.families||[]),objectTypes=new Set((state.grcObjects||[]).map(item=>item.type)),crossProcedureRelations=crossProcedureCount(graph);
  const ontology={
  ...
  const epistemicSupport=Object.fromEntries(ICTC_CANONICAL_EPISTEMIC_FAMILIES.map(family=>[family,supportedFamilies.has(family)]));
  ...
  const reasons=[...Object.entries(ontology).filter(([,ok])=>!ok).map(([key])=>`ontology:${key}`),...Object.entries(epistemicSupport).filter(([,ok])=>!ok).map(([key])=>`epistemic-support:${key}`),...Object.entries(evidenceCoverage).filter(([,ok])=>!ok).map(([key])=>`evidence-level:${key}`),...(dangling.length?[`dangling:${dangling.length}`]:[])];
  graph.summary={...graph.summary,nodes:graph.nodes.length,edges:graph.edges.length,evidenceLevels:{...graph.summary.evidenceLevels,'E5-portfolio-view':1},ontologyCoverage:ontology,epistemicCoverage,epistemicSupport,evidenceCoverage,crossProcedureRelations};
  graph.digest=sha256({baseDigest:base.digest,portfolio,ontology,epistemicCoverage,epistemicSupport,evidenceCoverage,crossProcedureRelations});
  graph.closure={closed:reasons.length===0,reasons,ontologyContract:'typical-sme-operating-ontology-bounded-by-ictc-vocabulary',ontologyDimensions:DEMO_SME_CLOSED_ONTOLOGY,objectTypes:[...objectTypes].sort(),canonicalEpistemicFamilies:ICTC_CANONICAL_EPISTEMIC_FAMILIES,epistemicSupportContract:'Canonical families must be supported by the projection grammar; instance coverage reports source-state presence and is never synthesized to satisfy closure.',evidenceLevels:DEMO_EVIDENCE_LEVELS,crossProcedureRelations,danglingEdges:dangling.length,claimBoundary:'Closed relative to the declared ICTC SME ontology and epistemic vocabulary; not a universal ontology of every possible SME or legal reality.'};
