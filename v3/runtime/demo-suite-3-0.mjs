...
export function demoSuite30LearningBasis(state={}){const processCounts=Object.fromEntries(PROCEDURES.map(([id,,collection])=>[id,(state?.[collection]||[]).length])),businessRecords=Object.values(processCounts).reduce((a,b)=>a+b,0);return Object.freeze({datasetId:DEMO_SUITE_30_ID,datasetVersion:DEMO_SUITE_30_VERSION,datasetAuthority:'demo-suite-3-0',synthetic:true,businessRecords,processCount:Object.values(processCounts).filter(Boolean).length,processCounts:Object.freeze(processCounts),metadataDigest:demoSuite30MetadataDigest(state),claimBoundary:'Synthetic DEMO basis only; educational wording belongs to the Home copy owner.'});}
...
function promoteMeta(meta={},record,procedureId){const out=clone(meta||{});out.scenarioVersion='3.0';out.datasetAuthority='demo-suite-3-0';out.sourceScenarioVersion=meta?.scenarioVersion||'2.2';out.granular={...(out.granular||{}),contractVersion:'3.0',metadataAuthority:'mechanically-derived',uiProjection:deriveDemoSuite30Metadata(record,procedureId)};return out;}
