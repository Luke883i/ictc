...
const PROJECTION_DESCRIPTOR=Object.freeze({
  monitoring:Object.freeze({title:['jobName','objective'],state:['state'],facts:['cadenceHours','nextRunAt','planVersion']}),
  incidents:Object.freeze({title:['caseTitle','originalNarrative'],state:['state'],facts:['eventKind','operationalSeverity','awarenessAt']}),
  objects:Object.freeze({title:['name'],state:['status'],facts:['type','criticality','owner']}),
  coverage:Object.freeze({title:['requirementRef','requirementLabel'],state:['state'],facts:['universe','targetIds','proposalSource']}),
  actions:Object.freeze({title:['title'],state:['state'],facts:['priority','owner','dueAt','nextStep']}),
  risks:Object.freeze({title:['title'],state:['reviewState','state'],facts:['humanRating','reviewAt','owner']}),
  assurance:Object.freeze({title:['title'],state:['state'],facts:['source','questionCount','owner']})
});
const valueAt=(record,keys)=>{for(const key of keys||[]){const value=record?.[key];if(value!==undefined&&value!==null&&String(value).trim()!=='')return value;}return'';};
const digestible=value=>Array.isArray(value)?value.length:typeof value==='object'&&value!==null?Object.keys(value).length:value;
function projectionBasis(record,procedureId){const copy=clone(record||{}),demo=copy.demo||{};copy.demo={synthetic:true,scenarioId:DEMO_SUITE_30_ID,datasetId:DEMO_SUITE_30_ID,datasetVersion:DEMO_SUITE_30_VERSION,datasetAuthority:'demo-suite-3-0',sourceScenarioId:demo.sourceScenarioId||DEMO_SUITE_22_ID,procedureId};return copy;}
export function deriveDemoSuite30Metadata(record,procedureId){const descriptor=PROJECTION_DESCRIPTOR[procedureId]||{title:['id'],state:['state'],facts:[]},title=String(valueAt(record,descriptor.title)||record?.id||procedureId).trim().slice(0,240),state=String(valueAt(record,descriptor.state)||'recorded').trim().slice(0,100),facts=descriptor.facts.map(key=>({key,value:digestible(record?.[key])})).filter(item=>item.value!==undefined&&item.value!==null&&String(item.value)!=='');const recordDigest=sha256({procedureId,record:projectionBasis(record,procedureId)});return Object.freeze({
  'L0-shell':Object.freeze({datasetId:DEMO_SUITE_30_ID,datasetVersion:DEMO_SUITE_30_VERSION,synthetic:true,objectRole:'record'}),
  'L1-procedure':Object.freeze({procedureId,code:PROCEDURES.find(([id])=>id===procedureId)?.[3]||procedureId}),
  'L2-list':Object.freeze({recordId:String(record?.id||''),title,state,primitive:'RecordRow'}),
  'L3-detail':Object.freeze({recordId:String(record?.id||''),title,state,facts:Object.freeze(facts)}),
  'L4-provenance':Object.freeze({datasetId:DEMO_SUITE_30_ID,datasetVersion:DEMO_SUITE_30_VERSION,sourceScenarioId:DEMO_SUITE_22_ID,sourceSuite:'2.2-deprecated-generator',synthetic:true}),
  'L5-technical':Object.freeze({runtimeAuthority:'native-procedure-runtime',metadataAuthority:'mechanically-derived',stressVisible:false,recordDigest})
});}
export const demoSuite30MetadataDigest=state=>sha256(allBusinessRecords(state).map(({procedureId,record})=>[record.id,deriveDemoSuite30Metadata(record,procedureId)]));
export function demoSuite30LearningScenarios(state={}){const counts=Object.fromEntries(PROCEDURES.map(([id,,collection])=>[id,(state?.[collection]||[]).length])),total=Object.values(counts).reduce((a,b)=>a+b,0),basis=Object.freeze({datasetId:DEMO_SUITE_30_ID,datasetVersion:DEMO_SUITE_30_VERSION,businessRecords:total,processCounts:counts});return Object.freeze([
  Object.freeze({id:'bad-fragmented-compliance',level:'bad',title:'Norme trattate come checklist separate',chain:Object.freeze(['obblighi sovrapposti letti separatamente','oggetti e responsabilita duplicati','controlli ripetuti','evidenze incoerenti e lavoro ridondante']),basis,claimBoundary:'Scenario didattico derivato dalla struttura del DEMO; non descrive un fatto reale o un esito osservato.'}),
  Object.freeze({id:'worst-untraceable-decisions',level:'worst',title:'Decisioni senza catena ricostruibile',chain:Object.freeze(['fonte o requisito senza contesto','decisione senza proprietario','azione senza verifica','evidenza senza provenance','stato finale non falsificabile']),basis,claimBoundary:'Scenario didattico di failure mode; non e una previsione, una conclusione legale o una prova di non conformita.'})
]);}
...
function promoteMeta(meta={},record,procedureId){const out=clone(meta||{});out.scenarioVersion='3.0';out.datasetAuthority='demo-suite-3-0';out.sourceScenarioVersion=meta?.scenarioVersion||'2.2';out.granular={contractVersion:'3.0',metadataAuthority:'mechanically-derived',uiProjection:deriveDemoSuite30Metadata(record,procedureId)};return out;}
function promoteBusinessRecord(record,procedureId){const out=clone(record),legacyDemo=clone(out.demo||{});out.demo={...legacyDemo,synthetic:true,tag:'synthetic-demo',scenarioId:DEMO_SUITE_30_ID,scenarioVersion:'3.0-runtime',datasetId:DEMO_SUITE_30_ID,datasetVersion:DEMO_SUITE_30_VERSION,datasetAuthority:'demo-suite-3-0',sourceScenarioId:legacyDemo.scenarioId||DEMO_SUITE_22_ID,procedureId,legacySource:clone(DEMO_SUITE_30_LEGACY_SOURCE),meta:promoteMeta(legacyDemo.meta||{},out,procedureId),claimBoundary:legacyDemo.claimBoundary||'Dati sintetici DEMO: nessuna conclusione legale o di conformita.'};return out;}
...
function canonicalDigestCollections(state={}){return{missions:(state.missions||[]).map(item=>({planHistory:[],planVersion:item.plan?1:0,...clone(item)})),catalog:(state.catalog||[]).map(item=>({observations:[],decisions:[],...clone(item)})),incidents:(state.incidents||[]).map(item=>({formulationVersions:[],formulationDirty:!item.finalNarrative,...clone(item)})),grcObjects:state.grcObjects||[],grcMappings:state.grcMappings||[],grcActions:state.grcActions||[],grcRisks:state.grcRisks||[],grcAssurance:state.grcAssurance||[],controlTests:state.controlTests||[]};}
export const demoSuite30StateDigest=state=>sha256({datasetId:DEMO_SUITE_30_ID,datasetVersion:DEMO_SUITE_30_VERSION,...canonicalDigestCollections(state)});
...
export function demoSuite30Violations(state={}){
  ...
  for(const{procedureId,record}of records){
    ...
    if(meta.scenarioVersion!=='3.0'||meta.datasetAuthority!=='demo-suite-3-0'||meta?.granular?.contractVersion!=='3.0'||meta?.granular?.metadataAuthority!=='mechanically-derived')out.push(`metadata:${record.id}`);
    const expectedUi=deriveDemoSuite30Metadata(record,procedureId);
    if(!ui||ui?.['L0-shell']?.objectRole!=='record'||ui?.['L2-list']?.primitive!=='RecordRow'||ui?.['L4-provenance']?.datasetId!==DEMO_SUITE_30_ID||ui?.['L5-technical']?.runtimeAuthority!=='native-procedure-runtime'||ui?.['L5-technical']?.metadataAuthority!=='mechanically-derived'||ui?.['L5-technical']?.stressVisible!==false||ui?.['L5-technical']?.recordDigest!==expectedUi['L5-technical'].recordDigest)out.push(`metadata-projection:${record.id}`);
...
export function demoSuite30Projection(state={}){
  ...
  return{enabled:marker.status==='complete',status:marker.status,suiteVersion:DEMO_SUITE_30_VERSION,profile:'suite-3.0',projectionAuthority:'demo-suite-3-0',datasetId:DEMO_SUITE_30_ID,scenarioId:DEMO_SUITE_30_ID,sourceScenarioId:DEMO_SUITE_22_ID,deprecatedSuite:'2.2',legacySourceStatus:'deprecated-generator-only',stateDigest:marker.stateDigest,metadataDigest:demoSuite30MetadataDigest(state),metadataAuthority:'mechanically-derived',organizationName:state?.settings?.organization?.name||manifest.company.name,positiveRecords:marker.positiveRecords||0,primaryRecords:marker.positiveRecords||0,supportRecords:marker.supportRecords||0,uiBusinessRecords:marker.positiveRecords||0,stressFixtures:manifest.stressRecordCount,stressVisible:false,totalModelTarget:manifest.totalModelTarget,counts:clone(marker.counts||{}),metadataStages:['L0-shell','L1-procedure','L2-list','L3-detail','L4-provenance','L5-technical'],learningScenarios:demoSuite30LearningScenarios(state),synthetic:true,coherent:marker.status==='complete'&&violations.length===0,violations,claimBoundary:'Suite 3.0 sintetica: i record business persistiti sono esattamente il corpus DEMO canonico; stress e mutazioni restano test-only; nessuna conclusione legale, prova di conformita o assurance indipendente.',limitations:[...LIMITATIONS]};
}
