...
export function buildEvidenceLineageDocument({ bundle, graph, release = 'ICTC', generatedAt = new Date().toISOString(), actor = null } = {}) {
  ...
  const lineage = (bundle.events || []).map(event => ({
    eventId: event.id || null, revision: Number(event.revision || 0), at: event.at || null, action: event.action || null,
    actorId: event.actorId || null, role: event.role || null, subject: event.subject || null, previousHash: event.previousHash || null,
    hash: event.hash || null, inputSha256: event.inputSha256 || null, resultSha256: event.resultSha256 || null, stateSha256: event.stateSha256 || null,
    semanticManifestSha256: event.metadata?.semanticManifestSha256 || null, epistemicStepSha256: event.metadata?.epistemicStepSha256 || null
  }));
  return {
    schemaVersion: '1.0.0', documentType: 'ictc-evidence-lineage-receipt', product: 'ICTC', release, generatedAt,
    generatedBy: actor?.id || bundle.generatedBy || null, generatedForRole: actor?.role || bundle.generatedForRole || null, authorization: 'same-as-read',
    ...
    integrity: {
      graphSha256: graph.digest || null, subjectSha256: bundle.manifest?.subjectSha256 || null, relatedSha256: bundle.manifest?.relatedSha256 || null,
      eventsSha256: bundle.manifest?.eventsSha256 || null, integrityHead: bundle.integrity?.head || bundle.manifest?.integrityHead || null,
...
export function evidenceMarkdown(doc) {
  ...
  lines.push(`**Generated:** ${scalar(doc.generatedAt)} by ${scalar(doc.generatedBy)} (${scalar(doc.generatedForRole)})`, `**Authorization:** ${doc.authorization}`, '', '## Integrity',
    `- Graph SHA-256: \`${scalar(doc.integrity.graphSha256)}\``, `- Subject SHA-256: \`${scalar(doc.integrity.subjectSha256)}\``, `- Events SHA-256: \`${scalar(doc.integrity.eventsSha256)}\``,
    `- Audit head: \`${scalar(doc.integrity.integrityHead)}\``, `- Canonical state SHA-256: \`${scalar(doc.integrity.canonicalStateSha256)}\``, `- Bound to audit head: **${doc.integrity.stateBoundToAuditHead ? 'yes' : 'no'}**`, '', '## Lineage');
