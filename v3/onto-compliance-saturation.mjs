...
const measures=p=>({
  ...
  saturationContractDrift:(!p.mContract||!p.gContract)?1:0,nativeLatticeDrift:(!p.nativeLattice||!p.globalRuntimeMinimal)?1:0
});
...
const FAULTS=Object.freeze(['procedure-registry-drift','ep01-business-leak','attention-verdict','auditor-to-proof','duplicate-trace-owner','trace-before-work','tablet-overflow','green-zero-attention','green-scope','scope-editor-expanded','scope-option-reset','proposed-blur','small-target','hierarchy-loss','saturation-doc-loss','native-lattice-missing','global-compact-return']);
function inject(base,fault){const p=clone(base);({
  ...
  'native-lattice-missing':()=>p.nativeLattice=false,'global-compact-return':()=>p.globalRuntimeMinimal=false
...
const COMPRESSIONS=Object.freeze([
  ...
  {id:'native-local-composition-owners',applied:sourceProfile.nativeLattice&&sourceProfile.globalRuntimeMinimal,protect:['nativeLatticeDrift']}
]);
for(const c of COMPRESSIONS)assert.ok(c.applied,`declared safe compression not materialized: ${c.id}`);
const G=COMPRESSIONS.length,UNSAFE_COMPRESSIONS=Object.freeze(['remove-claim-boundary','merge-proposed-with-recorded','hide-native-work','remove-role-distinction','drop-local-overflow-owner','shrink-target','drop-procedure-identity','remove-scope-reason','merge-ep01-into-business','remove-trace-owner','restore-global-composition-adapter']);
