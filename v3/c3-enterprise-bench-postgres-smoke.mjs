...
import { enterpriseSubjectAdvisoryLockKey, withTenantReadTransaction, withTenantTransaction } from './runtime/postgres-enterprise-authority.mjs';
const {Pool}=pg;
...
try{
  ...
  if(stage==='advisory'){
    const key=enterpriseSubjectAdvisoryLockKey('probe',{type:'risk',id:'probe-risk'});assert.match(key,/^-?\d+$/);await withTenantTransaction(pool,'probe',client=>client.query('SELECT pg_advisory_xact_lock($1::bigint)',[key]));console.log(JSON.stringify({ok:true,stage,key}));process.exit(0);
  }
  if(stage==='subject-write'){
    const key=enterpriseSubjectAdvisoryLockKey('probe',{type:'risk',id:'probe-risk'});await withTenantTransaction(pool,'probe',async client=>{await client.query('SELECT pg_advisory_xact_lock($1::bigint)',[key]);await client.query(`INSERT INTO ictc_subject_current(tenant_id,subject_type,subject_id,version,payload_sha256,payload_json,bucket) VALUES($1,'risk','probe-risk',1,$2,$3::jsonb,1)`,['probe','a'.repeat(64),JSON.stringify({probe:true})]);await client.query(`INSERT INTO ictc_subject_version(tenant_id,subject_type,subject_id,version,payload_sha256,payload_json,command_id) VALUES($1,'risk','probe-risk',1,$2,$3::jsonb,'probe-command')`,['probe','a'.repeat(64),JSON.stringify({probe:true})]);const count=Number((await client.query("SELECT count(*)::int AS c FROM ictc_subject_current WHERE tenant_id='probe'")).rows[0].c);assert.equal(count,1);});console.log(JSON.stringify({ok:true,stage}));process.exit(0);
  }
