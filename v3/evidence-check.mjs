...
const root=await mkdtemp(path.join(os.tmpdir(),'ictc-evidence-')); let store; let restarted; const actor={id:'admin-test',role:'admin'};
try{
  store=await new Store(root).init();
  ...
  await assert.rejects(()=>store.saveAttachments([{name:'partial.txt',mime:'text/plain',dataBase64:Buffer.from('partial').toString('base64')},{name:'too-large.bin',mime:'application/octet-stream',dataBase64:oversized}]),error=>error.code==='attachment-too-large');
  ...
  applyCatalogDecision(source,'verified','Checked','admin-test'); assert.equal(source.decisions.at(-1).observationSha256.length,64);
  ...
  assert.equal(source.state,'candidate','new observation must require a new human decision'); assert.equal(source.observations.length,2);
  await store.mutate({id:'alice',role:'user'},'contribution.recorded',{type:'contribution',id:'c1'},{},draft=>{draft.contributions.push({id:'c1',links:['https://secret.example/item'],text:'private body',note:'private note',attachments:[{id:'hidden-file',name:'secret.pdf',sha256:'b'.repeat(64)}],state:'enriched',createdAt:new Date().toISOString(),createdBy:'alice'});draft.catalog.push({id:'cat1',title:'Public candidate',identifier:'PUB:1',sourceUrl:'https://public.example',state:'candidate',decisions:[],observations:[{origin:{kind:'contribution',contributionId:'c1'},observedAt:new Date().toISOString()}],origin:{kind:'contribution',contributionId:'c1'}});return {id:'c1'};});
  const bobBundle=store.evidenceBundle('catalog','cat1',{id:'bob',role:'user'}),serialized=JSON.stringify(bobBundle);assert.equal(bobBundle.related.contributions.length,0);assert.equal(bobBundle.related.restrictedContributionCount,1);for(const secret of ['secret.example','private body','private note','secret.pdf','alice']) assert.equal(serialized.includes(secret),false,`catalog bundle leaked ${secret}`);assert.equal(serialized.includes('[restricted]'),true);
  ...
  restarted=await new Store(root).init(); assert.equal(restarted.verifyChain().head,store.verifyChain().head);
...
}finally{restarted?.close();store?.close();await rm(root,{recursive:true,force:true,maxRetries:20,retryDelay:100});}
