...
def footer_clear(page, label):
    metrics = page.evaluate("""async()=>{const f=document.querySelector('#stableLegalFooter');if(!f)return{missing:true};const frame=()=>new Promise(resolve=>requestAnimationFrame(resolve));const waitAnimations=async()=>{const animations=document.getAnimations().filter(a=>{const target=a.effect?.target,timing=a.effect?.getComputedTiming?.();return target?.closest?.('main')&&a.playState!=='finished'&&Number.isFinite(timing?.endTime)});if(animations.length)await Promise.race([Promise.allSettled(animations.map(a=>a.finished)),new Promise(resolve=>setTimeout(resolve,700))]);return animations.length;};const settle=async()=>{const animationsWaited=await waitAnimations();let last=window.scrollY,stable=0;for(let i=0;i<36&&stable<3;i++){await frame();const now=window.scrollY;if(Math.abs(now-last)<.5)stable++;else stable=0;last=now;}return animationsWaited;};const exposed=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);if(!(r.width>0&&r.height>0&&s.visibility!=='hidden'&&s.display!=='none')||e.disabled)return false;if(typeof e.checkVisibility==='function'&&!e.checkVisibility())return false;for(let n=e.parentElement;n;n=n.parentElement){if(n.matches?.('details:not([open])')){const own=n.querySelector(':scope > summary');if(own!==e&&!own?.contains(e))return false;}}return true;};const fr=f.getBoundingClientRect(),pad=parseFloat(getComputedStyle(document.body).paddingBottom)||0;const candidates=[...document.querySelectorAll('main button,main a[href],main input,main select,main textarea,main summary')].filter(exposed);let sampled=0,worst=0,animationsWaited=0;const offenders=[];for(const e of candidates.slice(0,48)){e.focus?.();animationsWaited+=await settle();const r=e.getBoundingClientRect(),ff=f.getBoundingClientRect();const h=Math.max(0,Math.min(r.bottom,ff.bottom)-Math.max(r.top,ff.top)),w=Math.max(0,Math.min(r.right,ff.right)-Math.max(r.left,ff.left)),a=h*w;if(a>0)offenders.push({tag:e.tagName,id:e.id||'',cls:String(e.className||'').slice(0,120),text:(e.textContent||'').trim().replace(/\s+/g,' ').slice(0,100),position:getComputedStyle(e).position,rect:{top:r.top,right:r.right,bottom:r.bottom,left:r.left,width:r.width,height:r.height},footer:{top:ff.top,right:ff.right,bottom:ff.bottom,left:ff.left,width:ff.width,height:ff.height},area:a,scrollY:window.scrollY,maxScroll:Math.max(0,document.documentElement.scrollHeight-innerHeight)});worst=Math.max(worst,a);sampled++;}return{missing:false,footerHeight:fr.height,bodyPaddingBottom:pad,scrollY:window.scrollY,maxScroll:Math.max(0,document.documentElement.scrollHeight-innerHeight),docHeight:document.documentElement.scrollHeight,viewportHeight:innerHeight,sampled,animationsWaited,overlap:offenders.length,worst,offenders};}""")
    assert not metrics.get('missing'), metrics
    if metrics['overlap']:
        o = metrics['offenders'][0]
        raise AssertionError(f"{label}: {o['tag']} text={o['text'][:24]!r} bottom={o['rect']['bottom']:.1f} footerTop={o['footer']['top']:.1f} y={o['scrollY']:.1f}/{o['maxScroll']:.1f} area={o['area']:.1f}")
