...
def open_process(page,code,pid,host):
 ...
 card=page.locator(f'#procedureHub [data-process-code="{code}"]'); expect(card).to_be_visible(); before_cycle=int(page.locator('html').get_attribute('data-experience-cycle') or 0); card.locator(':scope > footer .procedure-primary,:scope > footer .primary').first.click()
 PHASE=f'{code}-owner-ready'; page.wait_for_function("x=>{const h=document.querySelector(x.host),a=h?.querySelector(`:scope > [data-procedure-attention-slot=\"${x.pid}\"]`),rail=h?.querySelector(':scope > .procedure-support-rail'),adv=rail?.querySelector(':scope > [data-editorial-slot=\"advanced-context\"]'),ref=rail?.querySelector(':scope > [data-editorial-slot=\"reference\"]'),cycle=Number(document.documentElement.dataset.experienceCycle||0);return !!(h&&h.offsetParent!==null&&cycle>x.beforeCycle&&h.dataset.editorialOwner===x.owner&&h.dataset.editorialOrderValid==='true'&&a&&adv?.dataset.editorialSlotOwner===x.owner&&ref?.dataset.editorialSlotOwner===x.owner)}",arg={'host':host,'owner':OWNER[pid],'pid':pid,'beforeCycle':before_cycle})
...
def audit(page,code,pid,host):
 ...
 slot=root.locator(f':scope > [data-procedure-attention-slot="{pid}"]'); expect(slot).to_have_count(1); rail=root.locator(':scope > .procedure-support-rail'); adv=rail.locator(':scope > [data-editorial-slot="advanced-context"]'); ref=rail.locator(':scope > [data-editorial-slot="reference"]'); expect(rail).to_have_count(1); expect(adv).to_have_count(1); expect(ref).to_have_count(1); assert adv.evaluate('(a,r)=>a.nextElementSibling===r',ref.element_handle()),f'{pid}: reference not immediately after context in support rail'; assert slot.evaluate('(s,c)=>s.nextElementSibling===c',root.locator(':scope > [data-editorial-slot="controls"]').first.element_handle()) if root.locator(':scope > [data-editorial-slot="controls"]').count() else True; assert root.locator(':scope > [data-editorial-slot="primary"]').first.evaluate('(p,r)=>p.nextElementSibling===r',rail.element_handle()),f'{pid}: support rail not immediately after primary'; assert slot.get_attribute('data-attention-slot-owner')==OWNER[pid],(pid,'attention owner',slot.get_attribute('data-attention-slot-owner'),OWNER[pid])
 direct=root.evaluate("""r=>{const out=[],push=role=>{if(role&&out.at(-1)!==role)out.push(role)},rail=r.querySelector(':scope > .procedure-support-rail');for(const n of r.children){if(n===rail){for(const x of rail.children)push(x.dataset?.editorialSlot);}else push(n.dataset?.editorialSlot);}return out;}"""); expected=[x for x in order if x in direct]; assert direct[:len(expected)]==expected,(pid,direct,expected)
 ...
 if mutations:
  first=mutations[0]; PHASE=f"{code}-post-commit-{first.get('type','mutation')}-{str(first.get('target','node'))[:64]}"[:180]
