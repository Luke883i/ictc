...
def audit(page,code,pid,host):
 ...
 slot=root.locator(f':scope > [data-procedure-attention-slot="{pid}"]'); expect(slot).to_have_count(1); assert frame.evaluate('(f,s)=>f.nextElementSibling===s',slot.element_handle()),f'{pid}: attention not immediately after compact orientation'; assert slot.get_attribute('data-attention-slot-owner')==OWNER[pid],(pid,'attention owner',slot.get_attribute('data-attention-slot-owner'),OWNER[pid])
