...
def audit(page,code,pid,host):
 ...
 slot=root.locator(f':scope > [data-procedure-attention-slot="{pid}"]'); expect(slot).to_have_count(1); rail=frame.locator(':scope > .procedure-support-rail'); adv=rail.locator(':scope > [data-editorial-slot="advanced-context"]'); ref=rail.locator(':scope > [data-editorial-slot="reference"]'); expect(rail).to_have_count(1); expect(adv).to_have_count(1); expect(ref).to_have_count(1); assert adv.evaluate('(a,r)=>a.nextElementSibling===r',ref.element_handle()),f'{pid}: reference not immediately after context in support rail'; assert frame.evaluate('(f,s)=>f.nextElementSibling===s',slot.element_handle()),f'{pid}: attention not immediately after procedure header'; assert slot.get_attribute('data-attention-slot-owner')==OWNER[pid],(pid,'attention owner',slot.get_attribute('data-attention-slot-owner'),OWNER[pid])
 direct=root.evaluate("""r=>{const out=[],rail=r.querySelector(':scope > .procedure-frame > .procedure-support-rail');for(const n of rail?.children||[]){const role=n.dataset?.editorialSlot;if(role)out.push(role);}for(const n of r.children){const role=n.dataset?.editorialSlot;if(role)out.push(role);}return out;}"""); expected=[x for x in order if x in direct]; assert direct[:len(expected)]==expected,(pid,direct,expected)
