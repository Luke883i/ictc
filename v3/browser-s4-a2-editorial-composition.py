...
def audit(page,code,pid,host):
 ...
 direct=root.evaluate("""r=>{const out=[],push=role=>{if(role&&out.at(-1)!==role)out.push(role)},rail=r.querySelector(':scope > .procedure-frame > .procedure-support-rail');for(const n of rail?.children||[])push(n.dataset?.editorialSlot);for(const n of r.children)push(n.dataset?.editorialSlot);return out;}"""); expected=[x for x in order if x in direct]; assert direct[:len(expected)]==expected,(pid,direct,expected)
