...
try:
 with sync_playwright() as p:
  ...
  PHASE='bootstrap'; page.goto(BASE,wait_until='domcontentloaded'); page.wait_for_function("()=>document.documentElement.dataset.nativeSemanticLattice==='3.2.0'",timeout=30000)
  PHASE='identity'; identity=ctx.request.get(f'{BASE}/api/runtime/identity').json(); build=identity.get('build') or {}; assert not EXPECTED_SHA or build.get('sha')==EXPECTED_SHA,(build,EXPECTED_SHA); assert not EXPECTED_SHA or build.get('exact') is True; assert not EXPECTED_SHA or build.get('dirty') is False
