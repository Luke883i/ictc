...
def audit(page,code,pid,host):
 ...
 slot=root.locator(f':scope > [data-procedure-attention-slot="{pid}"]'); expect(slot).to_have_count(1); adv=root.locator(':scope > [data-editorial-slot="advanced-context"]'); ref=root.locator(':scope > [data-editorial-slot="reference"]'); expect(adv).to_have_count(1); expect(ref).to_have_count(1); assert frame.evaluate('(f,a)=>f.nextElementSibling===a',adv.element_handle()),f'{pid}: context not immediately after compact orientation'; assert adv.evaluate('(a,r)=>a.nextElementSibling===r',ref.element_handle()),f'{pid}: reference not immediately after context'; assert ref.evaluate('(r,s)=>r.nextElementSibling===s',slot.element_handle()),f'{pid}: attention not immediately after reference'; assert slot.get_attribute('data-attention-slot-owner')==OWNER[pid],(pid,'attention owner',slot.get_attribute('data-attention-slot-owner'),OWNER[pid])
