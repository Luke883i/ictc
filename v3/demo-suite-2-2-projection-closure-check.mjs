...
const repoRoot=path.dirname(path.dirname(fileURLToPath(import.meta.url))),port=47622;const child=spawn(process.execPath,['v3/server.mjs'],{cwd:repoRoot,env:{...process.env,ICTC_RUNTIME_DIR:runtimeRoot,ICTC_DEMO_SUITE:'2.2',ICTC_SCHEDULER_TICK_MS:'100000',ICTC_PORT:String(port),PORT:String(port),ICTC_HOST:'127.0.0.1'},stdio:['ignore','pipe','pipe']});
let logs='';child.stdout.on('data',d=>logs+=d);child.stderr.on('data',d=>logs+=d);
async function getJson(url){const response=await fetch(url,{headers:{'x-ictc-role':'admin','x-ictc-actor-id':'projection-closure'}});assert.equal(response.ok,true,`${url} HTTP ${response.status}`);return response.json();}
try{let health=null;for(let i=0;i<900;i++){try{health=await getJson(`http://127.0.0.1:${port}/api/health`);break;}catch{await new Promise(r=>setTimeout(r,200));}}assert.ok(health,`Suite 2.2 server did not become ready: ${logs.slice(-2000)}`);assert.equal(health.demo.enabled,true);assert.equal(health.demo.projectionAuthority,'demo-suite-2-2');assert.equal(health.demo.positiveRecords,188);assert.equal(health.demo.stressVisible,false);assert.equal(health.demo.schedulerEnabled,false);const bootstrap=await getJson(`http://127.0.0.1:${port}/api/bootstrap`);assert.equal(bootstrap.experience.demoMode,true);assert.equal(bootstrap.experience.demo.projectionAuthority,'demo-suite-2-2');assert.equal(bootstrap.experience.demo.stateDigest,DEMO_SUITE_22_EXPECTED_DIGEST);assert.deepEqual(bootstrap.experience.demo.counts,DEMO_SUITE_22_SCENARIO.expectedCounts);assert.equal('demoAudit' in bootstrap,false,'legacy demo outcome projection must be absent');}
