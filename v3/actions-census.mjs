...
async function post(state,description){await json(`${api}/repos/${repository}/statuses/${sha}`,{method:'POST',body:JSON.stringify({state,context:'ictc/actions-census',description:String(description).slice(0,140)})});}
...
while(true){
  ...
  const signature=runs.map(run=>`${run.id}:${run.name}:${run.status}:${run.conclusion||''}`).sort().join('|');stablePolls=signature===previousSignature?stablePolls+1:0;previousSignature=signature;
  const failures=runs.filter(run=>conclusionState(run)==='failure'),pending=runs.filter(run=>conclusionState(run)==='pending'),observedFor=Date.now()-started,settled=observedFor>=minObserveMs&&pending.length===0&&stablePolls>=2;
  if(settled||Date.now()>=deadline)break;
  await post('pending',`exact head: ${runs.length} checks, ${failures.length} failed, ${pending.length} pending`);await new Promise(resolve=>setTimeout(resolve,pollMs));
}
const rows=last.map(run=>({id:run.id,name:run.name,status:run.status,conclusion:run.conclusion||null,detailsUrl:run.details_url||run.html_url||null,startedAt:run.started_at||null,completedAt:run.completed_at||null,state:conclusionState(run)})),failures=rows.filter(row=>row.state==='failure'),pending=rows.filter(row=>row.state==='pending'),report={schemaVersion:'1.1.0',authority:'github-check-runs-exact-head',headSha:sha,observedAt:new Date().toISOString(),checkCount:rows.length,failureCount:failures.length,pendingCount:pending.length,allGreen:failures.length===0&&pending.length===0,checks:rows};
...
if(report.allGreen){await post('success',`all ${rows.length} exact-head Actions checks completed without failure`);console.log(JSON.stringify(report));process.exit(0);}
const roots=failures.slice(0,3).map(row=>row.name).join(', '),suffix=roots?` — ${roots}`:'';await post('failure',`exact head: ${failures.length} failed, ${pending.length} pending of ${rows.length}${suffix}`);console.error(JSON.stringify(report));process.exit(1);
