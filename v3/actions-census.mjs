...
while(true){
  ...
  await post('pending',`exact head: ${runs.length} checks, ${failures.length} failed, ${pending.length} pending`,failures[0]?.details_url||failures[0]?.html_url||'');await new Promise(resolve=>setTimeout(resolve,pollMs));
}
const rows=last.map(run=>({id:run.id,name:run.name,status:run.status,conclusion:run.conclusion||null,detailsUrl:run.details_url||run.html_url||null,startedAt:run.started_at||null,completedAt:run.completed_at||null,state:conclusionState(run)})),failures=rows.filter(row=>row.state==='failure'),pending=rows.filter(row=>row.state==='pending'),report={schemaVersion:'1.2.0',authority:'github-check-runs-exact-head',headSha:sha,observedAt:new Date().toISOString(),checkCount:rows.length,failureCount:failures.length,pendingCount:pending.length,allGreen:failures.length===0&&pending.length===0,checks:rows};
...
const roots=failures.slice(0,3).map(row=>row.name).join(', '),suffix=roots?` — ${roots}`:'';await post('failure',`exact head: ${failures.length} failed, ${pending.length} pending of ${rows.length}${suffix}`,failures[0]?.detailsUrl||'');console.error(JSON.stringify(report));process.exit(1);
