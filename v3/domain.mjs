...
export function asString(value,max=20000){return String(value??'').trim().slice(0,max);}
export function asArray(value){return Array.isArray(value)?value:value==null?[]:[value];}
export function uniqueStrings(value,maxItems=100,maxLength=1000){return [...new Set(asArray(value).map(item=>asString(item,maxLength)).filter(Boolean))].slice(0,maxItems);}
...
export function sha256(value){const input=Buffer.isBuffer(value)?value:Buffer.from(typeof value==='string'?value:canonicalJson(value));return createHash('sha256').update(input).digest('hex');}
export function publicSettings(settings){const keyEnv=asString(settings.llm.apiKeyEnv,200);return {organization:structuredClone(settings.organization),llm:{endpoint:settings.llm.endpoint,model:settings.llm.model,apiKeyEnv:keyEnv,temperature:settings.llm.temperature,configured:Boolean(settings.llm.endpoint&&settings.llm.model),ready:Boolean(settings.llm.endpoint&&settings.llm.model&&(!keyEnv||process.env[keyEnv]))},governance:structuredClone(settings.governance||{}),environment:structuredClone(settings.environment||{}),prompts:structuredClone(settings.prompts),updatedAt:settings.updatedAt,updatedBy:settings.updatedBy};}
export function cadenceHours(value){const aliases={daily:24,weekly:168,monthly:720};if(typeof value==='string'&&aliases[value])return aliases[value];const number=Number(value);return Number.isFinite(number)?Math.max(1,Math.min(8760,Math.round(number))):168;}
export function nextRunAt(from,hours){return new Date(new Date(from).getTime()+hours*3600000).toISOString();}
export function reminderDates(awarenessAt){const base=new Date(awarenessAt);if(Number.isNaN(base.valueOf()))return null;return {earlyWarning24h:new Date(base.getTime()+24*3600000).toISOString(),notification72h:new Date(base.getTime()+72*3600000).toISOString(),finalReportOneMonth:new Date(base.getTime()+30*24*3600000).toISOString()};}
export function normalizeUrl(value){const raw=asString(value,4000);if(!raw)return '';try{const url=new URL(raw);if(!['http:','https:'].includes(url.protocol))return '';url.hash='';return url.toString();}catch{return '';}}
export function redactEndpoint(value){try{const url=new URL(value);return `${url.protocol}//${url.host}${url.pathname}`;}catch{return asString(value,500);}}
export function safeFilename(value,fallback='file'){return asString(value,240).replace(/[\r\n"\\/]/g,'_')||fallback;}
