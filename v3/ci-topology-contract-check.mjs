...
check(uiux.includes("details[data-proof-workspace=\"epistemic-investigation\"]")&&uiux.includes("e.parentElement?.firstElementChild===e")&&uiux.includes("investigation.locator('[data-service=\"epistemic\"]')"),'UI/UX oracle must bind the canonical EP-01 Evidence affordance to the first disclosure while retaining global command-palette navigation');
...
console.log(JSON.stringify({ok:true,topology:'native-check-run-authority',acceptance:'required-exact-head-fail-closed',failureRootSelection:'leaf-before-aggregate',browserMatrixIsolation:true,browserMaxParallel:4,requiredBrowserJourneys:10,informationValueExecution:'diagnostic-non-authoritative',browserFailureProvenance:'native-summary+phase-target',shellServices:['home','processes','proof'],epistemicEntryAuthority:'proof-first-row-disclosure+admin-auditor-command-palette-meta-row',epistemicBusinessProcess:false,historicalUiOracle:'read-only+empty-populated-tolerant+final-c01+geometry-owner+proof-first-row+global-ep-navigation',historicalFinetuningOracle:'read-only+empty-populated-tolerant',aoQueueControlAuthority:'single-canonical-pair+legacy-alias',informationValueOwnership:'information-architecture+declared-owner+monotonic-final-c01-cycle',informationValueProcedureLifecycleAuthority:'delegated',informationValueGeometryAuthority:'delegated',browserReadiness:'c01-cycle-after-replay-exhaustion+surface-invariant',readinessMutationGate:10000,productCiMutationGate:'1m+1m',statefulJourneyAuthority:'supported-ui-forms',statusFanout:false,portabilityBoundary:'owned-process-quiescence'}));
