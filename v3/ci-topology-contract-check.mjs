...
check(uiux.includes("#procedureHub .procedure-card")&&uiux.includes("PHASE='proof-to-ep-navigation'")&&uiux.includes("#globalCommandTrigger")&&uiux.includes("data-global-id=\"epistemic\"")&&uiux.includes("data-global-service=\"epistemic\""),'required UI/UX oracle must retain catalogue and canonical Proof-to-EP command-palette navigation');
check(!uiux.includes(".service-nav [data-service=\"epistemic\"]"),'UI/UX oracle must not invent EP-01 as a fourth shell service');
check(uiux.includes("expect(page.locator('#epistemicMetaCard')).to_have_count(0)")&&!uiux.includes("meta.evaluate('e=>e.parentElement?.id')"),'UI/UX oracle must enforce retirement of the legacy epistemic meta-card instead of depending on it');
...
console.log(JSON.stringify({ok:true,topology:'native-check-run-authority',acceptance:'required-exact-head-fail-closed',failureRootSelection:'leaf-before-aggregate',browserMatrixIsolation:true,browserMaxParallel:4,requiredBrowserJourneys:10,informationValueExecution:'diagnostic-non-authoritative',browserFailureProvenance:'native-summary+phase-target',shellServices:['home','processes','proof'],epistemicEntryAuthority:'admin-auditor-command-palette-meta-row',historicalUiOracle:'read-only+empty-populated-tolerant+final-c01+geometry-owner+canonical-proof-ep-navigation',legacyEpistemicMetaCard:false,historicalFinetuningOracle:'read-only+empty-populated-tolerant',aoQueueControlAuthority:'single-canonical-pair+legacy-alias',informationValueOwnership:'information-architecture+declared-owner+monotonic-final-c01-cycle',informationValueProcedureLifecycleAuthority:'delegated',informationValueGeometryAuthority:'delegated',browserReadiness:'c01-cycle-after-replay-exhaustion+surface-invariant',readinessMutationGate:10000,productCiMutationGate:'1m+1m',statefulJourneyAuthority:'supported-ui-forms',statusFanout:false,portabilityBoundary:'owned-process-quiescence'}));
