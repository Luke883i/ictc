...
check(uiux.includes("PHASE='catalogue-epistemic-secondary'")&&uiux.includes("expect(meta).to_have_count(1)")&&uiux.includes("expect(meta).not_to_be_visible()")&&uiux.includes("meta.evaluate('e=>e.parentElement?.id')")&&uiux.includes("=='proofView'"),'UI/UX oracle must keep EP-01 outside the seven-card business catalogue and bind its secondary entry to Proof');
check(uiux.includes("expect(meta).to_be_visible()")&&uiux.includes("meta.locator('[data-service=\"epistemic\"]')"),'UI/UX oracle must prove the Proof-local EP-01 affordance while retaining global command-palette navigation');
...
console.log(JSON.stringify({ok:true,topology:'native-check-run-authority',acceptance:'required-exact-head-fail-closed',failureRootSelection:'leaf-before-aggregate',browserMatrixIsolation:true,browserMaxParallel:4,requiredBrowserJourneys:10,informationValueExecution:'diagnostic-non-authoritative',browserFailureProvenance:'native-summary+phase-target',shellServices:['home','processes','proof'],epistemicEntryAuthority:'proof-secondary+admin-auditor-command-palette-meta-row',epistemicBusinessProcess:false,historicalUiOracle:'read-only+empty-populated-tolerant+final-c01+geometry-owner+proof-secondary+global-ep-navigation',historicalFinetuningOracle:'read-only+empty-populated-tolerant',aoQueueControlAuthority:'single-canonical-pair+legacy-alias',informationValueOwnership:'information-architecture+declared-owner+monotonic-final-c01-cycle',informationValueProcedureLifecycleAuthority:'delegated',informationValueGeometryAuthority:'delegated',browserReadiness:'c01-cycle-after-replay-exhaustion+surface-invariant',readinessMutationGate:10000,productCiMutationGate:'1m+1m',statefulJourneyAuthority:'supported-ui-forms',statusFanout:false,portabilityBoundary:'owned-process-quiescence'}));
