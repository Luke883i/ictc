...
check(sequentialDom.includes('search.dataset.seqQueueSearch=id;stateSelect.dataset.seqQueueState=id;')&&sequentialDom.includes("if(id==='objects'){search.dataset.seqAoSearch='';stateSelect.dataset.seqAoFilter='';}"),'AO queue must keep one canonical filter identity while preserving legacy aliases');
check(rnSequential.includes("ensureQueueWindow($('#missionsList'),'.mission-card',{id:'monitoring-jobs',label:'monitoraggi',budget:6})")&&!rnSequential.includes("compactList($('#missionsList')"),'RN monitoring jobs must use the bounded filter window rather than nested overflow disclosure');
