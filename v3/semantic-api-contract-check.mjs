...
for (const roleId of ['admin','user','auditor']) {
  const role = roles[roleId]; assert.ok(role, `missing product role ${roleId}`);
  ...
  for (const item of procedures) {
    ...
    const process = ontology.processes[item.id]; assert.ok(process, `procedure ${item.id} absent from runtime ontology`);
...
assert.match(contract.ratchetRule, /relation/i);
console.log(`semantic-api-contract-check: ok (${contract.id}, schema ${contract.schemaVersion}, release ${VERSION})`);
