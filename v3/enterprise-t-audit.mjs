...
const [server, http, store, enterprise, ai, network, product, packageJson, ci, security, index, render, compactCss, journeyCss, informationArchitecture] = await Promise.all([
  read('./server.mjs'),
  read('./runtime/http.mjs'),
  read('./store.mjs'),
  read('./enterprise.mjs'),
  read('./ai.mjs'),
  read('./network-policy.mjs'),
  ...
  read('./public/enterprise-compact.css'),
  read('./public/journey-reborn.css'),
  read('../docs/06_INFORMATION_ARCHITECTURE.md'),
]);
...
const facts = {
  ...
  localJsonStore: store.includes("state.json") && store.includes('this.queue = Promise.resolve()'),
...
assert.equal(facts.canonicalRoleCount, 3);
assert.equal(facts.safeBindingPolicy, true);
assert.equal(facts.trustedProxySecret, true);
assert.equal(facts.serverPermissions, true);
assert.equal(facts.localJsonStore, true);
assert.equal(facts.fsyncAndRename, true);
assert.equal(facts.auditChain, true);
assert.equal(facts.deploymentBlockersExplicit, true);
assert.equal(facts.aiTimeoutAndTrace, true);
assert.equal(facts.privateNetworkDenied, true);
assert.equal(facts.singleRoleAwareHome, true);
assert.equal(facts.compactDensityLayer, true);
...
const findings = [
  ...
  { id: 'A02', dimension: 'T05', severity: 'critical', status: facts.localJsonStore ? 'open' : 'resolved', statement: 'Persistence is process-local JSON rather than a transactional multi-instance durable store.' },
  ...
  { id: 'A04', dimension: 'T10', severity: 'high', status: facts.immutableActionPins ? 'resolved' : 'open', statement: 'CI actions use mutable major tags and no signed build provenance is produced.' },
...
const dimensions = model.dimensions.map(item => ({
  id: item.id,
  title: item.title,
  status: item.status,
  logicalAudit: item.logicalAudit,
  technicalAudit: item.technicalAudit,
  functionalAudit: item.functionalAudit,
  blockers: item.blockers,
  standardRefs: item.standardRefs,
  minimumSlice: item.minimumSlice,
  findings: findings.filter(finding => finding.dimension === item.id).map(finding => finding.id),
}));

assert.equal(dimensions.length, 18);
assert.ok(dimensions.every(item => item.logicalAudit && item.technicalAudit && item.functionalAudit));
assert.ok(dimensions.every(item => item.minimumSlice && item.blockers.length));

const report = {
  schemaVersion: '1.0.0',
  model: model.model,
  baselineCommit: model.baselineCommit,
  verdict: openFindings.some(item => item.severity === 'critical') ? 'enterprise-materialization-required' : 'refinement-required',
  facts,
  findingCount: findings.length,
  openFindingCount: openFindings.length,
  findings,
  dimensions,
  minimumRuntimePath: model.minimumRuntimePath,
  certificationBoundary: model.certificationBoundary,
};

await mkdir(new URL('../artifacts/', import.meta.url), { recursive: true });
await writeFile(new URL('../artifacts/enterprise-t-audit.json', import.meta.url), JSON.stringify(report, null, 2));
console.log(`enterprise-t-audit: ok (T=${dimensions.length}, findings=${findings.length}, open=${openFindings.length}, verdict=${report.verdict})`);
