...
check(tokens.includes('--chrome-header-start:#142a46')&&tokens.includes('--chrome-header-mid:#1d4068')&&tokens.includes('--chrome-header-end:#285b84'),'header palette must retain the darker 3.4 continuity tuning');
...
check(shell.includes('OPEN_ACTION_ICON')&&shell.includes('lucide lucide-arrow-up-right')&&!shell.includes('<i aria-hidden="true">→</i>'),'Home actions must replace the text arrow with the canonical Lucide-style SVG icon');
...
check(css.includes('background:var(--workspace-home-action)')&&css.includes('color:var(--home-action-text)'),'Home CTA must consume the shared chrome-continuity action gradient');
