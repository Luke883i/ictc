...
for(const [id,rows] of Object.entries(collections))for(const row of rows)assert.equal(/Officine Aurora/i.test(JSON.stringify(row)),false,`${id}:${row.id} legacy company after reality refinement`);
...
console.log(JSON.stringify({ok:true,control:'DEMO-OPERATING-YEAR',company:DEMO_COMPANY_CONTEXT.name,period:[DEMO_COMPANY_CONTEXT.operatingYear.start,DEMO_COMPANY_CONTEXT.operatingYear.end],showcaseCounts:YEAR_ONE_EXPECTED_COUNTS,stressCorpusSeparated:true,stressCorpusHiddenFromOrdinaryProjection:true,legacyCompanyPayloads:0,aoRequirementOverlap:0,incidentActionLinkRatio:Number(incidentLinkRatio.toFixed(3)),months:12,phases:4,challengeQuestionSets:Object.keys(PROCEDURE_CHALLENGE_QUESTIONS).length}));
