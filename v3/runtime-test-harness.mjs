...
export async function runtimeHarness(prefix='ictc-test') {
  ...
  const spawnOne=(args,env)=>prepareChildProcess(spawn(process.execPath,args,{cwd:root,env:{...process.env,...env},stdio:['ignore','pipe','pipe'],windowsHide:true}));
  ...
  server=spawnOne(['v3/server.mjs'],{PORT:String(apiPort),ICTC_HOST:'127.0.0.1',ICTC_RUNTIME_DIR:dir,ICTC_ALLOW_LOCAL_ACTOR_SWITCH:'1',ICTC_ALLOW_PRIVATE_AI:'1',ICTC_ALLOW_INSECURE_AI:'1',ICTC_LLM_API_KEY:'test-key',ICTC_SCHEDULER_TICK_MS:'100000',ICTC_RATE_LIMIT_BURST:'100000',ICTC_RATE_LIMIT_PER_SECOND:'10000'});await wait(`${base}/api/health`);
