...
console.log(JSON.stringify({ok:true,suite:'current-semantic',baseChecks:CURRENT_SEMANTIC.length,currentRequiredBase:CURRENT_REQUIRED_BASE.length,compatibilityRegressionBase:COMPATIBILITY_REGRESSION_BASE.length,replacedLegacyDemoGates:LEGACY_DEMO_GATES.length,demoSuiteGates:DEMO_SUITE_GATES.length,policyGates:POLICY_GATES,nativeGates:NATIVE_GATES,currentAuthorityVector:CURRENT_AUTHORITY_VECTOR,claimBoundary:'Compatibility regressions remain blocking in this slice; classification is prerequisite evidence for later responsibility-based contraction.'}));
