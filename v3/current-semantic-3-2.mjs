...
import { POLICY_GATES,LEGACY_DEMO_GATES,DEPRECATED_DEMO_SUITE_GATES,DEMO_SUITE_GATES,NATIVE_GATES,CURRENT_BASE_GATE_REGISTRY,CURRENT_REQUIRED_BASE,COMPATIBILITY_REGRESSION_BASE,CURRENT_AUTHORITY_VECTOR } from './current-gate-registry.mjs';

export { POLICY_GATES,LEGACY_DEMO_GATES,DEPRECATED_DEMO_SUITE_GATES,DEMO_SUITE_GATES,NATIVE_GATES };
...
for(const gate of DEMO_SUITE_GATES)runGate(gate,'current-semantic-demo-3.0',240_000);
for(const gate of NATIVE_GATES)runGate(gate,'current-semantic-native');
console.log(JSON.stringify({ok:true,suite:'current-semantic',baseRegistered:CURRENT_SEMANTIC.length,baseActive:CURRENT_SEMANTIC_ACTIVE.length,currentRequiredBase:CURRENT_REQUIRED_BASE.length,compatibilityRegressionBase:COMPATIBILITY_REGRESSION_BASE.length,replacedLegacyDemoGates:LEGACY_DEMO_GATES.length,currentDemoSuiteGates:DEMO_SUITE_GATES.length,deprecatedDemoSuite22Gates:DEPRECATED_DEMO_SUITE_GATES.length,policyGates:POLICY_GATES,nativeGates:NATIVE_GATES,executionOrder:'preserves CURRENT_SEMANTIC order across current-required and compatibility-regression classes; executes Suite 3.0 authority gates before native gates',currentAuthorityVector:CURRENT_AUTHORITY_VECTOR,claimBoundary:'Suite 3.0 is the blocking DEMO acceptance authority. Deprecated Suite 2.2 generator regressions are excluded from this current semantic rail and remain isolated in the dedicated demo workflow.'}));
