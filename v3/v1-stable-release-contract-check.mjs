...
assert.equal(contract.proofSurface.code,'EV-01');assert.equal(contract.proofSurface.businessProcess,false);assert.equal(contract.experience.fullProcessCatalogCopies,1);assert.equal(contract.experience.homeFullCatalog,false);assert.equal(contract.experience.homeAttentionCap,3);assert.equal(contract.experience.primaryActionsPerContext,1);assert.equal(contract.experience.technicalTraceLevel,5);assert.equal(contract.experience.evidenceDetailLevel,4);assert.equal(contract.experience.aiMandatory,false);assert.equal(contract.experience.auditorWrite,false);assert.equal(contract.distribution.repositoryPublicCandidate,true);assert.equal(contract.distribution.npmPublishable,false);assert.equal(packageManifest.private,true);assert.equal(packageManifest.license,contract.distribution.license);
...
console.log('v1-stable-release-contract-check: ok (public stable profile with non-publishable npm package)');
