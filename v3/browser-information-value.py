...
def wait_process_projection(page,selector,surface,owner):
 ...
 if selector=='#grcWorkspace':
  ...
  page.wait_for_function("x=>{const r=document.querySelector(x),f=r?.querySelector(':scope > .procedure-frame[data-procedure-header-contract=\"3.2\"]');return !!(f&&f.offsetParent!==null);}",arg=selector)
...
def open_process(page,code):
 ...
 wait_process_projection(page,*PROCESS_SURFACES[code])

def assert_above_fold(page,selector,ratio=.72):
 global PHASE
 node=page.locator(selector).first
 expect(node).to_be_visible()
 box=node.bounding_box(); viewport=page.viewport_size or {'height':900}
