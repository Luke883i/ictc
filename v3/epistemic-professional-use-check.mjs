...
const [ui,lensUi,browser,seed,reality,lattice,active]=await Promise.all([read('./public/ui/epistemic-lattice.js'),read('./public/ui/epistemic-professional-lenses.js'),read('./browser-epistemic-professional-demo.py'),read('./runtime/demo-seed.mjs'),read('./runtime/demo-reality-context.mjs'),read('./runtime/epistemic-lattice.mjs'),read('./public/ui/active-experience.js')]);
...
const productText=[ui,lensUi,lattice].join('\n');for(const token of ['data-epistemic-lens','epistemicLensContext','Lente professionale','Contesto PMI','businessThread','professionalLenses','conteggi, non score'])assert.ok(productText.includes(token),`professional EP-01 product contract missing ${token}`);
...
assert.ok(seed.includes("DEMO_SEED_ID='pmi-italiana-officine-aurora-v2'"),'base procedure seed must remain compatible v2 instead of hiding reality enrichment behind a silent seed rewrite');
for(const token of ['DEMO_REALITY_ID','BUSINESS_THREADS','evidenceState','operatingPressure','uncertainty','businessThread','PMI-THR-12'])assert.ok(reality.includes(token),`demo reality context missing ${token}`);
...
assert.ok(lattice.includes("schemaVersion:'1.2.0'")&&lattice.includes('epistemicProfessionalDiagnostics'),'lattice projection must expose diagnostics without changing atom authority');
assert.ok(browser.includes('sameProjectionDigestAcrossLenses')&&browser.includes('businessThreads'), 'browser must prove lens digest conservation and synthetic context');
console.log('epistemic-professional-use-check: ok (12 bounded lenses / v2+reality-context synthetic PMI / screenshot matrix contract)');
