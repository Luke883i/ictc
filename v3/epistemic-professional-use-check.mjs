...
const [ui,lensUi,browser,seed,lattice,active]=await Promise.all([read('./public/ui/epistemic-lattice.js'),read('./public/ui/epistemic-professional-lenses.js'),read('./browser-v2-epistemic.py'),read('./runtime/demo-seed.mjs'),read('./runtime/epistemic-lattice.mjs'),read('./public/ui/active-experience.js')]);
...
const productText=[ui,lensUi,lattice].join('\n');for(const token of ['data-epistemic-lens','epistemicLensContext','Lente professionale','Contesto PMI','businessThread','professionalLenses'])assert.ok(productText.includes(token),`professional EP-01 product contract missing ${token}`);
...
assert.ok(lensUi.includes('currentThread')&&lensUi.includes('data-epistemic-basis'),'EP-01 professional context search must route back through canonical atom navigation');
