...
for(let i=0;i<TRIALS;i++){
  ...
  switch(family%8){
    ...
    case 5: violation=variant===7&&words(p.catalogueSummary)>words(p.workspacePurpose);break;
...
console.log(JSON.stringify({ok:true,campaign:'semantic-workspace-closure-3.2.1',trials:TRIALS,failureFamilies:FAMILIES,killed,novel,lastNovel,noNoveltyAfter:TRIALS-lastNovel-1,note:'deterministic model-vocabulary saturation aligned to concise row/list grammar; not browser or code mutation executions'}));
