...
for(const family of ICTC_CANONICAL_EPISTEMIC_FAMILIES)assert.equal(graph.summary.epistemicCoverage[family],true,`ICTC epistemic family missing: ${family}`);
...
assert.ok(handler.includes('buildClosedDemoEvidenceLattice30'));assert.equal(handler.includes('store.mutate'),false,'lattice endpoint must remain read-only');assert.ok(registry.includes("'demo-evidence-lattice'"));assert.ok(workflow.includes('demo-evidence-lattice-3-0-completeness-check.mjs'));assert.ok(workflow.includes('demo-evidence-lattice-3-0-mutation-1m.mjs'));assert.ok(launcher.includes('demo-runtime-2-2'),'launcher must retain isolated Suite 2.2 runtime directory');
console.log(JSON.stringify({ok:true,control:'DEMO-EVIDENCE-LATTICE-3.0-COMPLETENESS',sourceScenarioId:DEMO_SUITE_22_ID,subjects:graph.summary.subjects,ontologyCoverage:graph.summary.ontologyCoverage,epistemicCoverage:graph.summary.epistemicCoverage,evidenceCoverage:graph.summary.evidenceCoverage,crossProcedureRelations:graph.closure.crossProcedureRelations,exclusiveLoad:{businessCollections:PROCEDURE_COLLECTIONS.length,supportReplacement:true,noLegacyMarker:true,dirtyBusinessRejected:true,legacyRejected:true,partialMarkerRejected:true,idempotent:true},claimBoundary:graph.closure.claimBoundary},null,2));
