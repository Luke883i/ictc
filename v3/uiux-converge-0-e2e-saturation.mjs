...
function validateSources(b){
  ...
  if(!b.shell.includes('.slice(0,3)')||!b.shell.includes("title.textContent='Cosa richiede attenzione?'")||!b.shell.includes("homeWorkQueue='3.2'")||!b.shell.includes("html.dataset.uiuxConverge='P1'")||!b.shell.includes("priorities.dataset.uiuxConverge='P1'"))errors.push('home');
  if(b.shell.includes('>→<')||b.shell.includes('<i aria-hidden="true">→</i>')||b.shell.includes('/api/'))errors.push('home:authority');
  const frameLeftIcons=(b.frame.match(/uiIcon\('chevron-left'/g)||[]).length,frameActionIcons=(b.frame.match(/uiIcon\('arrow-up-right'/g)||[]).length;
  if(!b.frame.includes('data-uiux-layout="row"')||!b.frame.includes("host.dataset.uiuxLayout='rows'")||frameLeftIcons<2||frameActionIcons<2||!b.frame.includes('procedure-boundary')||!b.frame.includes('procedure-value')||b.frame.includes('/api/'))errors.push('frame');
  if(!b.frame.includes("$('#monitoringContributionAction')"))errors.push('frame:monitoring-primary-binding');
  ...
  if(!b.homeCss.includes('#homePriorities .home-priority-head>button')||!b.homeCss.includes('#homePriorities .home-priority-clear button')||(b.homeCss.match(/min-height:44px/g)||[]).length<3)errors.push('home-targets-css');
...
add('home-five',b=>replace(b,'shell','.slice(0,3)','.slice(0,5)'));add('home-old-title',b=>replace(b,'shell',"title.textContent='Cosa richiede attenzione?'","title.textContent='Attività di compliance'"));add('home-owner-marker-drift',b=>replace(b,'shell',"homeWorkQueue='3.2'","homeWorkQueue='uiux-converge-0'"));add('shell-p1-marker-missing',b=>replace(b,'shell',"html.dataset.uiuxConverge='P1'","html.dataset.uiuxConverge='legacy'"));add('home-p1-marker-missing',b=>replace(b,'shell',"priorities.dataset.uiuxConverge='P1'","priorities.dataset.uiuxConverge='legacy'"));add('home-api-authority',b=>({...b,shell:b.shell+'\n// /api/uiux-forbidden'}));
add('frame-row-marker-removed',b=>replace(b,'frame','data-uiux-layout="row"','data-uiux-layout="matrix"'));add('frame-host-row-removed',b=>replace(b,'frame',"host.dataset.uiuxLayout='rows'","host.dataset.uiuxLayout='matrix'"));add('frame-left-icon-removed',b=>replace(b,'frame',"uiIcon('chevron-left'","uiIcon('search'"));add('frame-action-icon-removed',b=>replace(b,'frame',"uiIcon('arrow-up-right'","uiIcon('search'"));add('frame-monitoring-primary-crosswire',b=>replace(b,'frame',"$('#monitoringContributionAction')","$('#openContribution')"));add('frame-boundary-removed',b=>replace(b,'frame','procedure-boundary','procedure-claim-hidden'));add('frame-value-removed',b=>replace(b,'frame','procedure-value','procedure-rationale-hidden'));add('frame-api-authority',b=>({...b,frame:b.frame+'\n// /api/uiux-forbidden'}));
...
add('css-matrix-resurrection',b=>({...b,ownerCss:b.ownerCss+'\n#procedureHub{grid-template-columns:repeat(3,minmax(0,1fr))!important;min-height:226px!important}'}));add('css-row-owner-removed',b=>replace(b,'ownerCss','#procedureHub{display:grid!important;grid-template-columns:1fr!important;grid-auto-rows:auto!important','#procedureHub{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;grid-auto-rows:1fr!important'));add('css-desktop-areas-removed',b=>replace(b,'ownerCss','grid-template-areas:"code title purpose action"','grid-template-areas:none'));add('css-mobile-areas-removed',b=>replace(b,'ownerCss','grid-template-areas:"code title" "purpose purpose" "action action"','grid-template-areas:none'));add('css-targets-36',b=>({...b,ownerCss:b.ownerCss.replaceAll('min-height:44px','min-height:36px')}));add('home-css-targets-36',b=>({...b,homeCss:b.homeCss.replaceAll('min-height:44px','min-height:36px')}));
