...
const [operational,semantic,standard,globalTools,native,settings,css,ai,admin,stateProjection,active]=await Promise.all([
  read('./public/ui/operational-surface-a6-ux3.js'),read('./public/ui/semantic-surface-a6-ux4.js'),read('./public/ui/standard-browser.js'),read('./public/ui/global-tools.js'),read('./public/ui/native-workspace-3-2.js'),read('./public/ui/provider-settings-p3.js'),read('./public/semantic-runtime-closure-p3.css'),read('./ai.mjs'),read('./runtime/admin.mjs'),read('./public/ui/semantic-state-projection-p3.js'),read('./public/ui/active-experience.js')
]);
for(const token of ['data-semantic-count-owner="primary"','data-a6-registry-total','retireLegacyRegistryHead',"dataset.enduserPrimitive='ControlRail'"])assert.ok(operational.includes(token),`operational missing ${token}`);
...
for(const token of ['official-public-reference','authorized-licensed','ictc-operational-formulation','reference-only','Apri fonte ufficiale','Usa questo requisito nel mapping','Sintesi specifica ICTC non disponibile'])assert.ok(standard.includes(token),`standard browser missing ${token}`);
...
for(const token of ['normalizeMissingValues','Motivazione · non registrata','stateAxis','organizational-use',"dataset.controlEffect='pagination'"])assert.ok(stateProjection.includes(token),`state projection missing ${token}`);
assert.ok(native.includes('/semantic-runtime-closure-p3.css'));assert.ok(native.includes('installSemanticStateProjectionP3'));assert.ok(css.includes('grid-template-columns:minmax(12rem,1fr)'));assert.ok(css.includes('#procedureHub .procedure-card'));assert.ok(css.includes('.procedure-frame-context'));
assert.ok(active.includes("./provider-settings-p3.js"));assert.ok(!active.includes('settings-1-8'));
...
const custom=resolveAiProvider({endpoint:'http://127.0.0.1:9999/v1',model:'mock',apiKeyEnv:''});assert.equal(custom.provider,'custom-openai-compatible');assert.equal(custom.apiKeyEnv,'');
...
assert.ok(ai.includes('testAiProvider'));assert.ok(ai.includes('buildAiProviderRequest'));assert.ok(admin.includes("'/api/admin/ai/test'"));assert.ok(settings.includes('OpenAI')&&settings.includes('Anthropic')&&settings.includes('DeepSeek')&&settings.includes('data-ai-provider-test'));
