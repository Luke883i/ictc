...
def audit_process(page,role,vp,width,proc,contract,families,revision):
 ...
 context=page.locator(proc['context']);anatomy=page.locator(proc['anatomy']);work=page.locator(proc['work']).first;rail=root.locator(':scope > .procedure-support-rail');expect(rail).to_have_count(1);assert frame.evaluate('(f,r)=>f.nextElementSibling===r',rail.element_handle()),f'{proc["code"]}: support rail must be immediately below header'
 ...
 with sync_playwright() as pw:
  ...
  for role in ROLES:
   for vp,width,height in VIEWPORTS:
    ...
    if width>=1280:
     m=page.evaluate('()=>({inner:innerHeight,html:document.documentElement.scrollHeight,body:document.body.scrollHeight,scrolling:document.scrollingElement?.scrollHeight||0,main:document.querySelector("main")?.getBoundingClientRect(),home:document.querySelector("#homeView")?.getBoundingClientRect(),footer:document.querySelector("#stableLegalFooter")?.getBoundingClientRect(),overflow:getComputedStyle(document.body).overflow})')
     if max(m['html'],m['body'],m['scrolling'])>m['inner']+1:anomaly('home-page-scroll',role,vp,'home',m,'document scrolling element <= viewport+1 without clipping')
     if m.get('home') and m.get('footer') and m['home']['bottom']>m['footer']['top']+1:anomaly('home-footer-overlap',role,vp,'home',m,'home bottom <= footer top')
