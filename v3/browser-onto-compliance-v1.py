...
def process_geometry(page,proc):
    return page.locator(proc['frame']).evaluate("""f=>{const info=e=>{if(!e)return null;const s=getComputedStyle(e),r=e.getBoundingClientRect();return {display:s.display,visibility:s.visibility,opacity:s.opacity,w:+r.width.toFixed(1),h:+r.height.toFixed(1),font:+parseFloat(s.fontSize).toFixed(2),text:e.textContent||''}};const m=f.querySelector('.procedure-frame-main'),p=f.querySelector('.procedure-purpose span'),k=f.querySelector('.procedure-frame-kicker'),b=f.querySelector('.procedure-primary'),w=document.querySelector(arguments[1]);return {frame:info(f),main:info(m),purpose:info(p),kicker:info(k),primary:info(b),workY:w?w.getBoundingClientRect().y:null,frameY:f.getBoundingClientRect().y}}""", proc['work'])
