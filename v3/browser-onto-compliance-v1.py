...
def bootstrap(page,role):return page.evaluate("""async role=>{const r=await fetch('/api/bootstrap',{headers:{'content-type':'application/json','x-ictc-role':role,'x-ictc-actor-id':'onto-visual-audit'}});if(!r.ok)throw new Error('bootstrap '+r.status);return await r.json()}""",role)
...
def scene_metrics(page,args):
 ...
 return{activeView:document.querySelector('main > .view:not([hidden])')?.id||'',h1:vis('h1').map(e=>e.textContent.trim()).filter(Boolean),pageOverflow:{innerWidth,html:document.documentElement.scrollWidth,body:document.body.scrollWidth},primaryAboveFold:primary.map(e=>({text:e.textContent.trim(),rect:rect(e),className:e.className})),actionsAboveFold:vis('button,summary,[role="button"]').filter(e=>e.getBoundingClientRect().top<innerHeight+1).length,frame:rect(frame),work:rect(work),anatomy:rect(anatomy),anatomyBeforeWork:!!(anatomy&&work&&(anatomy.compareDocumentPosition(work)&Node.DOCUMENT_POSITION_FOLLOWING)),duplicateCodeIdentity:identity.map(e=>e.textContent.trim()),framePrimaryText:visible(frame?.querySelector('.procedure-primary'))?frame.querySelector('.procedure-primary').textContent.trim():'',favorableAttentionLanguage:/\bIn ordine\b|processi in ordine/i.test(bodyText),favorableMatches:(bodyText.match(/In ordine|processi in ordine/gi)||[]).slice(0,12),frameRatio:frame?+(frame.getBoundingClientRect().height/innerHeight).toFixed(3):null,scopeTextLength:bodyText.length,openScopeEditors:vis('.market-scope-editor[open]').length,visibleScopeEditors:vis('.market-scope-editor').length,visibleScopeTextareas:vis('.market-scope-editor textarea').length};
 }""",args)
...
def audit_scene(page,role,vp,surface,scope,code='',frame='',work='',anatomy='',contract=None):
 ...
 if code:
  ...
  if len(m['duplicateCodeIdentity'])>1:add_anomaly('duplicate-procedure-identity',role,vp['name'],surface,m['duplicateCodeIdentity'],'procedure code asserted once in identity layer')
  ...
  if contract:
   ...
   if title!=contract.get('label') or code_text!=contract.get('code'):add_anomaly('procedure-contract-identity-mismatch',role,vp['name'],surface,{'title':title,'code':code_text},{'title':contract.get('label'),'code':contract.get('code')})
   ...
   if boundary and boundary not in anatomy_text:add_anomaly('claim-boundary-not-present-in-procedure-trace',role,vp['name'],surface,False,'canonical claim boundary present')
 screenshot(page,role,vp,surface);return m

def hub_metrics(page):return page.locator('#procedureHub .procedure-card').evaluate_all("""nodes=>{const r=nodes.filter(n=>n.getClientRects().length).map(n=>n.getBoundingClientRect()),w=r.map(x=>x.width),h=r.map(x=>x.height);return{count:r.length,widthRange:r.length?Math.max(...w)-Math.min(...w):0,heightRange:r.length?Math.max(...h)-Math.min(...h):0,widths:w.map(x=>+x.toFixed(1)),heights:h.map(x=>+x.toFixed(1))}}""")

def audit_auditor_primary(page,vp,proc):
 primary=page.locator(f'{proc["frame"]} .procedure-primary');text=primary.inner_text().strip()
 if text!='Consulta record':add_anomaly('auditor-primary-language','auditor',vp['name'],proc['code'],text,'Consulta record')
 before=active_view_id(page);primary.click();page.wait_for_timeout(120);after=active_view_id(page)
 if after!=before:add_anomaly('auditor-primary-leaves-procedure','auditor',vp['name'],proc['code'],{'before':before,'after':after},'remain on procedure read surface')
...
try:
 with sync_playwright() as pw:
  ...
  for role in ROLES:
   for vp in VIEWPORTS:
    ctx=browser.new_context(viewport={'width':vp['width'],'height':vp['height']});ctx.add_init_script(f"localStorage.setItem('ictc-role','{role}');localStorage.setItem('ictc-service','home')");page=ctx.new_page();page.set_default_timeout(30000);PHASE=f'{role}-{vp["name"]}-bootstrap';page.goto(BASE+'/?view=home',wait_until='networkidle');data=bootstrap(page,role);registry={x['id']:x for x in data.get('procedureRegistry',{}).get('procedures',[])};assert len(registry)==7,registry.keys()
    audit_scene(page,role,vp,'home','#homeView');page.locator('.service-nav [data-service="processes"]').click();expect(page.locator('#procedureHub .procedure-card')).to_have_count(7);pm=audit_scene(page,role,vp,'processes','#processesView');pm['hub']=hub_metrics(page)
    for proc in PROCEDURES:
     open_process(page,proc['code']);expect(page.locator(proc['view'])).to_be_visible();audit_scene(page,role,vp,proc['code'],proc['view'],proc['code'],proc['frame'],proc['work'],proc['anatomy'],registry.get(proc['id']))
     if role=='auditor' and vp['width'] in (390,1280):audit_auditor_primary(page,vp,proc)
    page.locator('.service-nav [data-service="proof"]').click();expect(page.locator('#proofView')).to_be_visible();audit_scene(page,role,vp,'proof','#proofView');page.locator('.service-nav [data-service="processes"]').click();meta=page.locator('#epistemicMetaCard')
    if role in ('admin','auditor'):
     expect(meta).to_be_visible();meta.locator('[data-service="epistemic"]').click();expect(page.locator('#epistemicView')).to_be_visible();audit_scene(page,role,vp,'EP-01','#epistemicView')
  ...
  report={'ok':not anomalies,'profile':'onto-compliance-horizon-v1-runtime-visual-audit','sceneCount':len(scenes),'screenshotCount':len(screenshots),'anomalyCount':len(anomalies),'uniqueAnomalyCount':len(unique),'anomalies':anomalies,'scenes':scenes,'screenshots':screenshots,'dimensions':{'roles':ROLES,'viewports':VIEWPORTS,'procedures':[x['code'] for x in PROCEDURES]},'boundary':'Server-backed automated visual/geometry/ontology evidence; not independent human usability or assistive-technology assessment.'}
  (ART/'browser-onto-compliance-v1.json').write_text(json.dumps(report,indent=2,ensure_ascii=False),encoding='utf8')
  if anomalies:
   for item in list(unique.values())[:30]:print(f"::error title=onto-visual::{item['kind']}::{item['surface']} {item['role']} {item['viewport']}: {item['measured']}",flush=True)
   raise AssertionError(f'onto-compliance visual audit found {len(anomalies)} observations / {len(unique)} unique signatures')
  print(f'browser-onto-compliance-v1: complete scenes={len(scenes)} screenshots={len(screenshots)} anomalies=0',flush=True);browser.close()
