...
def audit_process(page,role,vp,width,proc,contract,families,revision):
 ...
 except Exception:anomaly('technical-trace-not-visible',role,vp,proc['code'],{'count':anatomy.count(),'html':anatomy.first.evaluate('(e)=>e?.outerHTML?.slice(0,500)||null') if anatomy.count() else None},'one visible procedure anatomy');return
 try:work.wait_for(state='visible',timeout=5000)
 except Exception:anomaly('native-work-not-visible',role,vp,proc['code'],{'count':page.locator(proc['work']).count(),'selector':proc['work']},'first native work anchor visible');return
 ...
 except Exception:anomaly('technical-trace-body-not-visible',role,vp,proc['code'],{'open':anatomy.get_attribute('open'),'text':anatomy.text_content()},'visible anatomy body after disclosure');return
 trace=anatomy.text_content() or '';actual=[x.strip() for x in anatomy.locator('.procedure-anatomy-epistemic span').all_text_contents()]
...
def audit_proof(page,role,vp,width):
 ...
 investigation=page.locator('#proofContent > details[data-proof-workspace="epistemic-investigation"]');trace=page.locator('#proofContent > details[data-proof-workspace="trace-reconstruction"]');decisions=page.locator('#proofContent > details[data-proof-domain="decisions"]')
 for name,disclosure in [('epistemic-investigation',investigation),('trace-reconstruction',trace),('decisions',decisions)]:
  ...
  elif disclosure.get_attribute('open') is not None:anomaly('evidence-workspace-open-by-default',role,vp,'proof',name,'all primary investigation paths collapsed')
 ...
 if order[:3]!=['epistemic-investigation','trace-reconstruction','decisions']:anomaly('evidence-workspace-order',role,vp,'proof',order[:3],['epistemic-investigation','trace-reconstruction','decisions'])
 if decisions.count()==1 and decisions.get_attribute('data-information-role')!='evidence':anomaly('evidence-decision-role',role,vp,'proof',decisions.get_attribute('data-information-role'),'evidence')
 ...
 if reading.count()!=1:anomaly('evidence-progressive-reading-missing',role,vp,'proof',reading.count(),1)
  ...
  if not page.locator('#proofMethodTitle').is_visible():anomaly('evidence-method-not-visible-after-disclosure',role,vp,'proof',False,True)
  if page.locator('.proof-reading-card').count()!=3:anomaly('evidence-reading-card-count',role,vp,'proof',page.locator('.proof-reading-card').count(),3)
 ...
 if missing:anomaly('evidence-benchmark-limit-missing',role,vp,'proof',missing,'each mapping carries explicit limit')
 ...
 if reading.count()==1 and reading.get_attribute('open') is not None:reading.locator(':scope > summary').click();expect(reading).not_to_have_attribute('open','')
...
def audit_ep(page,role,vp,width,expected_ids,revision):
 ...
 if role in ('admin','auditor'):
  page.locator('.service-nav [data-service="proof"]').click();expect(page.locator('#proofView')).to_be_visible();investigation=page.locator('#proofContent > details[data-proof-workspace="epistemic-investigation"]');expect(investigation).to_have_count(1)
