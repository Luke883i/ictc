...
def open_epistemic(page,phase_prefix='open-epistemic'):
 global PHASE
 PHASE=f'{phase_prefix}-processes'; page.goto(BASE+'/?view=processes',wait_until='networkidle'); expect(page.locator('#procedureHub .procedure-card')).to_have_count(7); meta=page.locator('#epistemicMetaCard'); expect(meta).not_to_be_visible(); assert meta.evaluate('e=>e.parentElement?.id')=='proofView'
 PHASE=f'{phase_prefix}-evidence'; page.goto(BASE+'/?view=proof',wait_until='networkidle'); expect(page.locator('#proofView')).to_be_visible(); expect(meta).to_be_visible()
 PHASE=f'{phase_prefix}-entry'; meta.locator('[data-service="epistemic"]').click()
 PHASE=f'{phase_prefix}-surface'; expect(page.locator('#epistemicView')).to_be_visible(); expect(page.locator('#epistemicProfessionalTools')).to_be_visible(); expect(page.locator('#epistemicLens')).to_be_visible()
...
try:
 ...
 with sync_playwright() as pw:
  ...
  open_epistemic(page,'demo-suite-2-2-bootstrap'); PHASE='demo-suite-2-2-bootstrap-data'; boot=api(page,'/api/bootstrap')['payload']; demo=boot['experience']['demo']; assert boot['experience']['demoMode'] is True; assert demo['enabled'] is True; assert demo['suiteVersion']=='2.2'; assert demo['projectionAuthority']=='demo-suite-2-2'; assert demo['positiveRecords']==188; assert demo['stressFixtures']==512; assert demo['stressVisible'] is False; assert demo['schedulerEnabled'] is False; assert demo['stateDigest']=='4266e20a3a89efe65d9e1d050b81fbaaccbccd378e81f28b8e26d0a51c46d61e'; assert 'demoAudit' not in boot; lattice=api(page,'/api/epistemic-lattice?offset=0&limit=80')['payload']; assert lattice['schemaVersion']=='1.3.0'; assert len(lattice['professionalLenses']['lenses'])==12; assert lattice['diagnostics']['counts']['businessAtoms']>0; assert lattice['projection']['rnSemanticAtoms']>=0; digest=lattice['projection']['projectionSha256']; label_before=page.locator('#epistemicPageLabel').inner_text(); assert digest[:12] in label_before
  ...
  PHASE='suite-disclosure'; page.goto(BASE+'/?view=home',wait_until='networkidle'); expect(page.locator('#ictcDemoCard')).to_be_visible(); page.locator('#ictcDemoCard').click(); expect(page.locator('#ictcDemoDialog')).to_be_visible(); expect(page.locator('#ictcDemoDialog')).to_contain_text('188 record positivi'); expect(page.locator('#ictcDemoDialog')).to_contain_text('512 mutanti esclusi'); expect(page.locator('#ictcDemoDialog')).to_contain_text('demo-suite-2-2'); shot(page,'demo-suite-2-2-disclosure.png')
  PHASE='explore-overview'; page.goto(BASE+'/?view=proof',wait_until='networkidle'); page.locator('#epistemicMetaCard [data-service="epistemic"]').click(); expect(page.locator('#epistemicView')).to_be_visible(); page.locator('[data-epistemic-level="overview"]').click(); expect(page.locator('.epistemic-overview')).to_be_visible(); shot(page,'epistemic-use-explore-overview.png')
  ...
  out={'ok':True,'profile':'epistemic-professional-demo-suite-2-2','epistemicSchemaVersion':'1.3.0','businessProcedures':7,'professionalLenses':len(LENSES),'businessThreads':lattice['diagnostics']['counts']['businessThreads'],'sameProjectionDigestAcrossLenses':True,'projectionSha256':digest,'demoSuite':'2.2','demoProjectionAuthority':demo['projectionAuthority'],'demoStateDigest':demo['stateDigest'],'rnSemanticAtoms':lattice['projection']['rnSemanticAtoms'],'metaEntrySurface':'Evidenze ICTC','screenshots':SHOTS,'screenshotCount':len(SHOTS),'claimBoundary':'Server-backed synthetic Suite 2.2 evidence over the declared professional-use taxonomy; not independent assurance, legal applicability, compliance, control effectiveness or universal usability.'}; (ART/'browser-epistemic-professional-demo.json').write_text(json.dumps(out,indent=2),encoding='utf8'); print(f'browser-epistemic-professional-demo: complete screenshots={len(SHOTS)}',flush=True); ctx.close(); browser.close()
