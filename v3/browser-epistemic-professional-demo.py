...
try:
 ...
 with sync_playwright() as pw:
  ...
  open_epistemic(page,'demo-reality-bootstrap'); PHASE='demo-reality-bootstrap-data'; boot=api(page,'/api/bootstrap')['payload']; assert boot['experience']['demoMode'] is True; assert boot['experience']['demo']['reality']['enabled'] is True; assert boot['experience']['demo']['reality']['threads']==12; assert boot['demoAudit']['verdict']=='coherent'; lattice=api(page,'/api/epistemic-lattice?offset=0&limit=80')['payload']; assert lattice['schemaVersion']=='1.2.0'; assert len(lattice['professionalLenses']['lenses'])==12; assert lattice['diagnostics']['counts']['businessThreads']==12; assert lattice['diagnostics']['counts']['contextualizedBusinessAtoms']>0; digest=lattice['projection']['projectionSha256']; label_before=page.locator('#epistemicPageLabel').inner_text(); assert digest[:12] in label_before
