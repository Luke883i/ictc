...
def open_epistemic(page,phase_prefix='open-epistemic'):
 global PHASE
 PHASE=f'{phase_prefix}-processes'; page.goto(BASE+'/?view=processes',wait_until='networkidle'); expect(page.locator('#procedureHub .procedure-card')).to_have_count(7)
 PHASE=f'{phase_prefix}-entry'; expect(page.locator('#epistemicMetaCard')).to_be_visible(); page.locator('#epistemicMetaCard [data-service="epistemic"]').click()
...
try:
 ...
 with sync_playwright() as pw:
  ...
  out={'ok':True,'profile':'epistemic-professional-demo-v1','epistemicSchemaVersion':'1.3.0','businessProcedures':7,'professionalLenses':len(LENSES),'businessThreads':12,'sameProjectionDigestAcrossLenses':True,'projectionSha256':digest,'demoReality':True,'demoAudit':boot['demoAudit']['verdict'],'rnSemanticAtoms':lattice['projection']['rnSemanticAtoms'],'screenshots':SHOTS,'screenshotCount':len(SHOTS),'claimBoundary':'Server-backed synthetic demo evidence over the declared professional-use taxonomy; not independent assurance, legal applicability, compliance, control effectiveness or universal usability.'}; (ART/'browser-epistemic-professional-demo.json').write_text(json.dumps(out,indent=2),encoding='utf8'); print(f'browser-epistemic-professional-demo: complete screenshots={len(SHOTS)}',flush=True); ctx.close(); browser.close()
