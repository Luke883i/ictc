...
try:
 ...
 with sync_playwright() as pw:
  ...
  open_epistemic(page,'demo-reality-bootstrap'); PHASE='demo-reality-bootstrap-data'; boot=api(page,'/api/bootstrap')['payload']; assert boot['experience']['demoMode'] is True; assert boot['experience']['demo']['reality']['enabled'] is True; assert boot['experience']['demo']['reality']['threads']==12; assert boot['demoAudit']['verdict']=='coherent'; lattice=api(page,'/api/epistemic-lattice?offset=0&limit=80')['payload']; assert lattice['schemaVersion']=='1.3.0'; assert len(lattice['professionalLenses']['lenses'])==12; assert lattice['diagnostics']['counts']['businessThreads']==12; assert lattice['diagnostics']['counts']['contextualizedBusinessAtoms']>0; assert lattice['projection']['rnSemanticAtoms']>=0; digest=lattice['projection']['projectionSha256']; label_before=page.locator('#epistemicPageLabel').inner_text(); assert digest[:12] in label_before
  ...
  out={'ok':True,'profile':'epistemic-professional-demo-v1','epistemicSchemaVersion':'1.3.0','businessProcedures':7,'professionalLenses':len(LENSES),'businessThreads':12,'sameProjectionDigestAcrossLenses':True,'projectionSha256':digest,'demoReality':True,'demoAudit':boot['demoAudit']['verdict'],'rnSemanticAtoms':lattice['projection']['rnSemanticAtoms'],'metaEntrySurface':'Evidenze ICTC','screenshots':SHOTS,'screenshotCount':len(SHOTS),'claimBoundary':'Server-backed synthetic demo evidence over the declared professional-use taxonomy; not independent assurance, legal applicability, compliance, control effectiveness or universal usability.'}; (ART/'browser-epistemic-professional-demo.json').write_text(json.dumps(out,indent=2),encoding='utf8'); print(f'browser-epistemic-professional-demo: complete screenshots={len(SHOTS)}',flush=True); ctx.close(); browser.close()
