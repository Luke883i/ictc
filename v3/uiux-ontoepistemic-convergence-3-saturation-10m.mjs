...
const base=()=>({sourceTruth:1,lateRewrite:0,collections:1,missingFallback:true,resolvedReveal:true,orientation:1,auditorWrites:0,recordFilledPrimary:0,epTechnicalFirst:0,aiSettingsOwners:1,footerOverlay:0,mobileOverflow:0,dialogFocus:true,businessWriteAuthorities:1,procedures:7});
function fail(s){const f=[];if(!s.sourceTruth||s.lateRewrite)f.push('R1');if(s.collections!==1||!s.missingFallback||!s.resolvedReveal)f.push('R2');if(s.orientation!==1)f.push('R3');if(s.auditorWrites)f.push('R4');if(s.recordFilledPrimary)f.push('R5');if(s.epTechnicalFirst||s.aiSettingsOwners!==1)f.push('R6');if(s.footerOverlay||s.mobileOverflow||!s.dialogFocus)f.push('R7');if(s.businessWriteAuthorities!==1||s.procedures!==7)f.push('P0');return[...new Set(f)];}
const F=[
 ...
 ['resolved-target-hidden',s=>s.resolvedReveal=false],['orientation-lost',s=>s.orientation=0],['orientation-duplicate',s=>s.orientation=2],['auditor-question-write',s=>s.auditorWrites=1],
