...
const missing=ops.filter((_,i)=>!seen[i]).map(x=>x[0]),result={ok:killed===TRIALS&&!missing.length&&holdoutNovel===0&&atlasKilled===1000&&topologyKilled===1000&&!topologyMissing.length&&topologyPairsSeen.size===topologyPairsPossible,trials:TRIALS,killed,killRate:killed/TRIALS,normalizedFamilies:ops.length,atlasScenarios:{trials:1000,killed:atlasKilled,survivors:1000-atlasKilled,families:atlasOps.length,pairInteractions:atlasPairs.size,classification:'deterministic multi-family model-level repository-orientation mutations'},topologyScenarios:{trials:1000,killed:topologyKilled,survivors:1000-topologyKilled,families:topologyOps.length,pairInteractions:topologyPairsSeen.size,pairInteractionsPossible:topologyPairsPossible,missingFamilies:topologyMissing,classification:'deterministic multi-family same-SHA/workflow/PR/branch semantic mutations'},seed:`0x${SEED.toString(16)}`,lastNovelAt:lastNovel,noNovelAfter:TRIALS-lastNovel-1,holdout:{size:HOLDOUT,newFamilies:holdoutNovel},missing,signature,limitations:['Model-level falsification of declared documentation/runtime invariants.','Repeated traces over normalized failure families are not independent contributor studies or code mutations.','GitHub server-side settings and deployment controls require external observation.']};console.log(JSON.stringify(result,null,2));if(!result.ok)process.exit(1);
