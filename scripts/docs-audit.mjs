...
try {
  ...
  const profiles = [
    {
      id: 'v1',
      marker: '## Versione corrente: ICTC 1.0.0',
      sections: ['## Avvio rapido', '## Scope stabile', '## Guardrail v1', '## Verifica della release', '## Fuori scope'],
      commands: ['./ictc.sh start', './ictc.sh audit', './ictc.sh stop', 'npm run release:check']
    },
    {
      id: 'v3',
      marker: '## Versione corrente proposta: v3 Living Evidence Atlas',
      sections: ['## Cosa si può usare nella v3', '## Confine epistemico', '## Verifica', '## Documentazione v3', '## Requisiti e sicurezza'],
      commands: ['./ictc-v3.sh start', './ictc-v3.sh status', './ictc-v3.sh logs', './ictc-v3.sh audit', './ictc-v3.sh stop']
    },
    {
      id: 'v2',
      marker: '## Avvio rapido',
      sections: ['## Avvio rapido', '## Comandi principali', '## Architettura essenziale', '## Stato del progetto'],
  ...
  assert.ok(profile, 'README senza un profilo operativo supportato: attesi contenuti completi v1, v3 oppure v2');
  ...
  mark('operations-runbook-v3-compatible', 'launcher generazione v3 mantenuto per compatibilità');
  ...
  assert.match(v1Dod, /M=36/);
  assert.match(v1Dod, /scenari 37–136/);
  assert.match(v1Dod, /non significa certificazione di terza parte/i);
  mark('saturation-and-stability-boundary', 'M, M+100 e limite della stabilità espliciti');

  const payload = { schemaVersion: '1.2.0', generatedAt: new Date().toISOString(), result: 'passed', checks };
  ...
  await writeFile(path.join(artifactDir, 'documentation-audit.json'), JSON.stringify({ schemaVersion: '1.2.0', generatedAt: new Date().toISOString(), result: 'failed', checks, error: error.message }, null, 2));
  throw error;
}
