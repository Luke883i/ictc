...
async function publishSummary({base,phase,type,blocker}){const target=String(process.env.GITHUB_STEP_SUMMARY||'').trim();if(!target)return;const safe=value=>String(value||'').replace(/[`\r\n]/g,' ').slice(0,160);await appendFile(target,`\n### ICTC browser failure provenance\n\n- script: \`${safe(base)}\`\n- phase: \`${safe(phase)}\`\n- type: \`${safe(type)}\`\n${blocker?`- blocker: \`${safe(blocker)}\`\n`:''}\nICTC_BROWSER_PHASE=${safe(phase)}\n`);}
