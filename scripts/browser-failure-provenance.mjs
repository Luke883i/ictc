...
function geometryFrom(message){const text=String(message||''),x=text.match(/(?:^|;)x=([0-9.+-]+)/),y=text.match(/(?:^|;)y=([0-9.+-]+)/);return{x:x?.[1]||'',y:y?.[1]||''};}
...
const phase=slug(payload.phase||'unknown',20),type=slug(payload.type||'Error',14),message=String(payload.message||`${type} in ${phase}`),blocker=blockerFrom(message),anomaly=anomalyFrom(payload),callsite=callsiteFrom(payload),geometry=geometryFrom(message);
if(anomaly){
...
const geometrySuffix=[callsite,geometry.x&&`x${Math.round(Number(geometry.x))}`,geometry.y&&`y${Math.round(Number(geometry.y))}`].filter(Boolean).join('-');
const messageSlug=slug(blocker?`covered-${blocker}${geometrySuffix?`-${geometrySuffix}`:''}`:message,48);
await post(`ictc/browser-detail/${slug(base,20)}/${phase}/${type}/${messageSlug}`,`${phase}: ${type}${blocker?` [covered-by ${blocker}]`:''}${callsite?` [${callsite}]`:''}: ${message}`);
console.log(JSON.stringify({browser:base,artifact,phase,type,blocker:blocker||null,callsite:callsite||null,geometry,anomaly:anomaly?{kind:anomaly.kind||null,surface:anomaly.surface||null,role:anomaly.role||null,viewport:anomaly.viewport||null}:null,message:message.slice(0,220)}));
