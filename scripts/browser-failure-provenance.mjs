...
async function publishSummary({base,phase,type,blocker}){const target=String(process.env.GITHUB_STEP_SUMMARY||'').trim();if(!target)return;await appendFile(target,`\n### ICTC browser failure provenance\n\n- script: \`${safe(base)}\`\n- phase: \`${safe(phase)}\`\n- type: \`${safe(type)}\`\n${blocker?`- blocker: \`${safe(blocker)}\`\n`:''}\nICTC_BROWSER_PHASE=${safe(phase)}\n`);}
...
const phase=String(payload.phase||'unknown'),type=String(payload.type||'Error'),message=String(payload.message||`${type} in ${phase}`),blocker=blockerFrom(message),anomaly=anomalyFrom(payload),callsite=callsiteFrom(payload),phaseToken=safe(phase);
console.error(`::error title=browser-provenance ${base}::ICTC_BROWSER_PHASE=${phaseToken}; ${safe(type)}: ${safe(message)}`);
await publishSummary({base,phase,type,blocker});
